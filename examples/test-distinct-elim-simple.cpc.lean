import Cpc.Logos
open Eo
def t1 := (Term.Apply Term._at__at_TypedList_nil Term.Int)
def t2 := (Term.UConst 1 Term.Int)
def t3 := (Term.Apply Term._at__at_TypedList_cons t2)
def t4 := (Term.Apply t3 t1)
def t5 := (Term.UConst 2 Term.Int)
def t6 := (Term.Apply Term._at__at_TypedList_cons t5)
def t7 := (Term.Apply t6 t4)
def t8 := (Term.UConst 3 Term.Int)
def t9 := (Term.Apply Term._at__at_TypedList_cons t8)
def t10 := (Term.Apply t9 t7)
def t11 := (Term.Apply Term.distinct t10)
def t12 := (Term.Apply Term.eq t5)
def t13 := (Term.Apply t12 t2)
def t14 := (Term.Apply Term.or t13)
def t15 := (Term.Apply t14 (Term.Boolean false))
def t16 := (Term.Apply Term.eq t8)
def t17 := (Term.Apply t16 t5)
def t18 := (Term.Apply Term.or t17)
def t19 := (Term.Apply t18 t15)
def t20 := (Term.Apply Term.not t13)
def t21 := (Term.Apply Term.and t20)
def t22 := (Term.Apply t21 (Term.Boolean true))
def t23 := (Term.Apply Term.eq t8)
def t24 := (Term.Apply t23 t2)
def t25 := (Term.Apply Term.not t24)
def t26 := (Term.Apply Term.and t25)
def t27 := (Term.Apply t26 t22)
def t28 := (Term.Apply Term.not t17)
def t29 := (Term.Apply Term.and t28)
def t30 := (Term.Apply t29 t27)
def t31 := (Term.Apply Term.eq t11)

...
\begin{figure}
\begin{lstlisting}
def smt_type_lift (s : native_String) (d : SmtDatatype) : SmtType -> SmtType
  | (SmtType.Datatype s2 d2) => (native_ite (Teq (SmtType.Datatype s d) (SmtType.Datatype s2 d2)) (SmtType.TypeRef s) (SmtType.Datatype s2 (smt_dt_lift s d d2)))
  | T => T

def smt_dtc_lift (s : native_String) (d : SmtDatatype) : SmtDatatypeCons -> SmtDatatypeCons
...
def smt_dt_lift (s : native_String) (d : SmtDatatype) : SmtDatatype -> SmtDatatype
...
def smt_type_substitute (s : native_String) (d : SmtDatatype) : SmtType -> SmtType
  | (SmtType.Datatype s2 d2) => (SmtType.Datatype s2 (native_ite (native_streq s s2) d2 (smt_dt_substitute s (smt_dt_lift s2 d2 d) d2)))
...
def smt_datatype_cons_default (v : SmtValue) : SmtDatatypeCons -> SmtDatatypeCons -> SmtValue
  ...
  | (SmtDatatypeCons.cons TF cF), (SmtDatatypeCons.cons TU cU) =>
    ...
    (native_ite (veq _v0 SmtValue.NotValue) SmtValue.NotValue (smt_datatype_cons_default (SmtValue.Apply v _v0) cF cU))
  | cF, cU => SmtValue.NotValue

def smt_datatype_default (s : native_String) (d : SmtDatatype) (n : Nat) : SmtDatatype -> SmtDatatype -> SmtValue
  | (SmtDatatype.sum cF dF), (SmtDatatype.sum cU dU) =>
    let _v0 := (smt_datatype_cons_default (SmtValue.DtCons s d n) cF cU)
    (native_ite (native_not (veq _v0 SmtValue.NotValue)) _v0 (smt_datatype_default s d (native_nat_succ n) dF dU))
...
def smt_type_default_rec : SmtType -> SmtType -> SmtValue
  ...
  | V, (SmtType.Map T U) =>
    ...
    (native_ite (veq _v0 SmtValue.NotValue) SmtValue.NotValue (SmtValue.Map (SmtMap.default T _v0)))
...
def smt_dt_wf_rec (s : String) (refs : RefList) (haveBase : Bool) : SmtDatatype -> Bool
  | SmtDatatype.null => haveBase
  | (SmtDatatype.sum c d) =>
      (smt_dt_cons_wf_rec c (s :: refs)) &&
      (smt_dt_wf_rec s refs (haveBase || (smt_dt_cons_wf_rec c refs)) d)

def smt_type_wf_rec : SmtType -> RefList -> Bool
  | (SmtType.Datatype s d), refs =>
      if s ∈ refs then false else smt_dt_wf_rec s refs false d

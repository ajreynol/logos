...
- CmdList, a list of Eunoia command, i.e. a proof.
...
- eo_is_closed : Term -> Bool
  - Given a Eunoia term, this returns true iff it is a \emph{closed} term, i.e. has no free variables.
- eo_is_refutation : CmdList -> Bool
  - Returns true if Logos successfully executes the given command list, and is a valid, closed proof of false.
...
This means it is defined solely over the Term datatype.
Second, the speficiation layer above does \emph{not} rely on the Eunoia definitions, e.g. of the Eunoia type system.
Third, the definition of the Eunoia type system is encapsulated as a method, eo_typeof, which returns the type of a given Eunoia term.
All computational side conditions are essentially untyped manipulations over a single deep embedding of Eunoia types and terms, as encapsulated as Term.
...
i.e. eo_typeof for that term returns the Eunoia term Bool.
...
Figure~\ref{} gives the official definition of SMT-LIB types.
This includes an error value (None) denoting not a type,\footnote{
One could alternatively use an Option type instead of allowing None as a first-class constructor.
Throughout, we prefer using an error elements of types.}
the standard atomic SMT-LIB types of Booleans (Bool),
integers (Int), reals (Real) and regular languages (RegLan).
Bitvectors (BitVec) are indexed by a natural number denoting the bitwidth.
Note that in contrast to SMT-LIB, our semantics permit the bitvector of size zero.
...
- The type of sequences (Seq). Note that for uniformity, SMT-LIB strings are represented as a sequence of characters, where the character type (Char) is another atomic type constructor and has a finite range of values determined by the range of Unicode in SMT-LIB (values 0 to 199607).
Note the latter two types are not in the SMT-LIB standard.

The next three types pertain to SMT-LIB datatypes.
The first (Datatype) denotes a (possibly recursive) datatype type.
This definition reflects standard notions in the literature for mu-types, or \emph{recursive} types.
In this definition, the first argument of the datatype denotes the name of that datatype.
The second argument (a SmtDatatype) denotes the list of constructors of that datatype,
where adds a constructor to this list, and null terminates its list of constructors.
Recursive references to the datatype in question are indicated by type references (constructor TypeRef), which
takes the name of the datatype that it references.

For example, a recursive list datatype is represented as:


This definition also permits mutual recursive datatypes, for example:

Below, we will see that values of datatype types are datatype constructors (DtCons)
applied to other values.
We use an curried apply (Apply) constructor
to denote applications of terms.
For example, the value of datatype type

Note that this definition permits two kinds of ill-formed datatypes.
First, a type may contain a type reference that it not within the bounds of the datatype that it references.
In this case, the type is not well-formed.
...
that have that type.
For instance, a datatype containing only recursive constructors is uninhabited.
Other examples include a datatype with no constructors,
or a mutually recursive datatype where all paths to other datatypes
contain a recursive reference to the datatype we are defining.

The next type (DtcAppType) is the type of partially applied constructors.
It is a function-like type that takes the argument and return types of the constructor,
where this type is curried.
...
Note that the type of constructors is intentionally distinct from function types (FunType).

Finally, the last constructor in this list denotes uninterpreted sorts (USort),

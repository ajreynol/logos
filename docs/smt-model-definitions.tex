...
We will additionally require an (arbitrary total) ordering over types,
...
In practice, this required us to define a total ordering over Lean rationals (\code{Rat}) for this
...
def smt_map_canonical : SmtMap -> Bool
  | (SmtMap.default T e) => (smt_map_default_canonical T e && smt_value_canonical e)
  | (SmtMap.cons i e m) =>
...
def smt_value_canonical : SmtValue -> Bool
  | (SmtValue.Binary w n) => if 0 <= w then decide (n = n % (2 ^ Int.toNat w)) else true
...
Firstly, bitvectors values are considered canonical 

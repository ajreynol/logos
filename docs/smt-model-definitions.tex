...
def smt_type_substitute (s : native_String) (d : SmtDatatype) : SmtType -> SmtType
  | (SmtType.Datatype s2 d2) => (SmtType.Datatype s2 (if (native_streq s s2) then d2 else (smt_dt_substitute s (smt_dt_lift s2 d2 d) d2)))
  | (SmtType.TypeRef s2) => (if (native_streq s s2) then (SmtType.Datatype s d) else (SmtType.TypeRef s2))
  | T => T
...
def smt_datatype_default (s : native_String) (d : SmtDatatype) (n : Nat) : SmtDatatype -> SmtDatatype -> SmtValue
  | (SmtDatatype.sum cF dF), (SmtDatatype.sum cU dU) =>
    ...
    (if (native_not (veq _v0 SmtValue.NotValue)) then _v0 else (smt_datatype_default s d (native_nat_succ n) dF dU))

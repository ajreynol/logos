...
In other words, the Logos checker is 8564 lines,

Note the following design decisions.
First, the executable portion of the Logos checker (the third layer) does \emph{not} depend on the SMT-LIB definition or the specification definition.
...
namely that they are total mappings (over both mappings).
...
we require that for all input values of type \code{T},
the Lean function successfully returns a value of type \code{U}.
...
Presently we assume that all values of type Real are rational (Rational),
...
In other words, \code{(cons (Int 0) (Int 1) (default (Int 2)))}
...
for instance
\code{(cons (Int 0) (Int 1) (cons (Int 0) (Int 3) (default (Int 2))))}
...
\code{(cons (Int 0) (Int 1) (default (Int 0)))}
is also redundant.
...
e.g. \code{(const (Int 0) (Int 1) (default (Bool true)))}
stores integer indices but has a different type of value as its default.
...
Given a model \code{M}, the behavior of a function value
...
In other words, our model evaluator 
Canonicity, however, does not apply to values over all types.

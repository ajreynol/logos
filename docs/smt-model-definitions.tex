...
inductive SmtTerm : Type where
  -- literals, variables, and application
  | None : SmtTerm
  ...
  -- representative arithmetic, maps, and bit-vectors
  ...
  -- representative strings, sequences, and regular languages
  ...
  -- representative datatypes, sets, and conversions
...
def native_default_ifun_id : String := "@native_default_ifun"
...
def __smtx_typeof : SmtTerm -> SmtType
  ...
  | ... => ...
\end{lstlisting}
\caption{Type inference for selected value and term forms.}
\end{figure}

\begin{figure}[p]
\begin{lstlisting}
def __smtx_type_substitute (s : String) (d : SmtDatatype) : SmtType -> SmtType
  | (SmtType.Datatype s2 d2) => (SmtType.Datatype s2 (if s = s2 then d2 else __smtx_dt_substitute s d d2))
  | (SmtType.TypeRef s2) => (if s = s2 then SmtType.Datatype s d else SmtType.TypeRef s2)
  | T => T

def __smtx_dtc_substitute (s : String) (d : SmtDatatype) : SmtDatatypeCons -> SmtDatatypeCons
...
def __smtx_dt_substitute (s : String) (d : SmtDatatype) : SmtDatatype -> SmtDatatype
...
def __smtx_typeof_dt_cons_rec (T : SmtType) : SmtDatatype -> Nat -> SmtType
  ...
  | (SmtDatatype.sum (SmtDatatypeCons.cons U c) d), 0 => (SmtType.DtcAppType U (__smtx_typeof_dt_cons_rec T (SmtDatatype.sum c d) 0))
...
def __smtx_typeof : SmtTerm -> SmtType
  | (SmtTerm.DtCons s d i) => let _v0 := (SmtType.Datatype s d); (__smtx_typeof_guard_wf _v0 (__smtx_typeof_dt_cons_rec _v0 (__smtx_dt_substitute s d d) i))
  | (SmtTerm.Apply (SmtTerm.DtSel s d i j) x1) => let _v0 := (__smtx_ret_typeof_sel s d i j); (__smtx_typeof_guard_wf _v0 (__smtx_typeof_apply (SmtType.FunType (SmtType.Datatype s d) _v0) (__smtx_typeof x1)))
  | (SmtTerm.Apply (SmtTerm.DtTester s d i) x1) => let _v0 := (SmtType.Datatype s d); (__smtx_typeof_guard (__smtx_typeof_dt_cons_rec _v0 (__smtx_dt_substitute s d d) i) (__smtx_typeof_apply (SmtType.FunType _v0 SmtType.Bool) (__smtx_typeof x1)))
  | ... => ...
\end{lstlisting}
\caption{Datatype substitution and typing for constructors and selectors.}
\end{figure}

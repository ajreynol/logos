...
\code{"@default"} (named \code{"@native_default_fun"} in the code),
...
Moreover, the function clause is stated in terms of the wrapper
\code{eval_fun_apply} (defined in Figure~\ref{fig:model-eval})
rather than \code{model_fun_lookup} directly,
which special-cases the reserved default function identifier
\code{"@default"} to return the default value of the range type.]}
...
via the wrapper \code{eval_fun_apply} shown in the figure:
this behaves as \code{model_fun_lookup} except on the reserved default
identifier \code{"@default"},
for which it returns the default value of the range type regardless of the
model, guaranteeing that the default function value from

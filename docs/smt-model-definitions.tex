...
\begin{itemize}
  ...
  \item \eoisclosed \code{:} \term \code{->} \bool
    ...
    \end{itemize}
  \item \eoisrefutation \code{:} \cmdlist \code{->} \bool
    \begin{itemize}
...
\end{examplecode}
The datatype type for list is \code{(Datatype "List" D_L)}, where \code{D_L} is the following \code{SmtDatatype}:
\begin{examplecode}
D_L := (sum (cons Int (cons (TypeRef "List") unit)) (sum unit null))
\end{examplecode}
Note that \code{D_L} is a list containing two datatype constructors.
The first (corresponding to \code{listc}) is a list contain \code{Int}
and \code{(TypeRef "List")}.
Intuitively, the latter can be thought of as a variable bound
by \code{(Datatype "List" D_L)}, i.e. it specifies that the child
...
Note that the names of constructors and selectors are not given in this representation.
Instead, constructors will be referenced by natural number indicies in this list,
e.g. 0 will refer to \code{listc} and 1 will refer to \code{listn}.
Similarly, we will use two natural numbers to reference selectors,
e.g. (0, 0) will refer to \code{head} and (0,1) to \code{tail}.

This definition also permits mutual recursive datatypes, for example, consider the SMT-LIB datatype:
\begin{examplecode}
(declare-datatypes ((Tree 0) (ListTree 0))
  (((node (children List)) (leaf))) 
   ((tlistc (head Tree) (tail List)) (tlistn)))
\end{examplecode}
This is captured by the datatype \code{(Datatype "Tree" D_T)}, where \code{D_T} is the following datatype:
\begin{examplecode}
D_T := (sum (cons (Datatype "List" D_S) unit) (sum unit nil))
D_S := (sum (cons (TypeRef "Tree") (cons (TypeRef "List") unit)) nil)
\end{examplecode}
...
Note that this definition permits three kinds of ill-formed datatypes.
...
(Datatype "mutual-no-wf" (sum (cons Int (cons (TypeRef "infinite-list") unit)) null))
\end{examplecode}

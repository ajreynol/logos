...
\noindent\textbf{SMT-LIB definition (roughly 2600 LOC):}
\begin{itemize}
...
\noindent\textbf{Specification definition (roughly 750 LOC):}
\begin{itemize}
...
\noindent\textbf{Logos definitions (roughly 10500 LOC):}
\begin{itemize}
...
Roughly, the first layer is 2600 lines of Lean,
the second layer is 750 lines of Lean (including the shared definition of Eunoia terms),
and the third layer is roughly 10500 lines of Lean.
In other words, the trusted computing base of Logos
is the sum of the first two layers, roughly 3350 lines of Lean.
...
\code{model_wf} for the Lean definition \code{model_total_typed},
and \code{Cmd} and \code{CmdList} for the checker types \code{CCmd} and \code{CCmdList}.
Places where a figure diverges from the code beyond this renaming are marked
with bracketed notes.
...
\noindent\emph{[Note on Figure~\ref{fig:smt-types} vs. the code:
in the Lean sources, the constructors of \code{SmtType} are ordered with
\code{USort} before \code{FunType} and \code{DtcAppType},
and all of these inductive types are declared in a single \code{mutual} block
together with \code{SmtTerm}, \code{SmtValue} and the value payload types.
The figure otherwise matches the code.]}
...
\noindent\emph{[Note on Figure~\ref{fig:smt-values} vs. the code:
in the Lean sources, the base elements of \code{SmtRegLan} are
\smtvalue{}s rather than characters, i.e.
\code{char : SmtValue -> SmtRegLan} and
\code{range : SmtValue -> SmtValue -> SmtRegLan}.
...
well-formed regular languages carry only valid character values as base
elements.]}
...
\noindent\emph{[Note on Figure~\ref{fig:typeof-value} vs. the code:
the \code{Char} case shown in the figure is abbreviated;
in the code it reads
\code{(if (c < 196608) then SmtType.Char else SmtType.None)}.]}
...
(\code{"@native_default_fun"} in the code, abbreviated \code{"@default"} in
Figure~\ref{fig:default-value}), where note that "@" is used as a prefix to
...
\noindent\emph{[Note on Figure~\ref{fig:canonical} vs. the code:
the code contains three further cases of \code{smt_value_canonical}
elided in the figure:
\code{Binary} values are canonical when their payload is reduced modulo
$2^w$ for non-negative bitwidth $w$,
\code{Char} values are canonical when in the legal character range, and
\code{RegLan} values are canonical when all base elements of the regular
expression are valid character values.
The map-default case is also stated slightly differently in the code
(via a helper \code{smt_map_default_canonical}), with the same meaning.]}
...
while (curried) multi-argument function types are permitted.

\noindent\emph{[Note on Figure~\ref{fig:type-wf} vs. the code:
in the code, the function type case of \code{smt_type_wf} is
\code{(smt_type_wf_component T) && (smt_type_wf U)},
i.e. the return type is checked recursively by \code{smt_type_wf} rather
than by \code{smt_type_wf_component}.
This is what permits curried multi-argument function types such as
\code{(FunType Int (FunType Int Bool))}, which the figure's version
would reject.]}
...
\noindent\emph{[Note on Figure~\ref{fig:model-typed} vs. the code:
the Lean definition is named \code{model_total_typed},
and its first clause is split into two conjuncts (typing and canonicity).
Moreover, the function clause is stated in terms of a wrapper
\code{eval_fun_apply} (Lean name \code{native_eval_fun_apply})
rather than \code{model_fun_lookup} directly:
this wrapper behaves as \code{model_fun_lookup} except on the reserved
default function identifier \code{"@native_default_fun"},
for which it returns the default value of the range type
regardless of the model.
This guarantees the default function value from
Figure~\ref{fig:default-value} is well-behaved in every model.
Model evaluation of function applications (Figure~\ref{fig:model-eval})
...
ill-typed.
The embedding additionally contains a constructor
\code{DtTester} (elided in Figure~\ref{fig:smt-terms}) with the same
arguments as \code{DtCons},
denoting the SMT-LIB discriminator (tester) for the constructor with the
given index.

\noindent\emph{[Note on Figure~\ref{fig:smt-terms} vs. the code:
the \code{select} constructor takes two arguments in the code
(\code{select : SmtTerm -> SmtTerm -> SmtTerm});
the figure drops one.
The code also contains further binder-like constructors elided here,
...
\noindent\emph{[Note on Figure~\ref{fig:typeof-term} vs. the code:
the figure writes \code{native_Teq} in some helpers and \code{Teq} in
others; these are the same method (recall the naming conventions above).
The header of \code{smt_typeof_not} contains a stray \code{:=} and should
read \code{def smt_typeof_not : SmtType -> SmtType}.]}
...
\noindent\emph{[Note on Figure~\ref{fig:model-eval} vs. the code:
the free constant constructor is \code{UConst} (not \code{Const}),
and its case reads
\code{(SmtTerm.UConst s T) => (model_lookup M false s T)}.
The definition of \code{re_ext_eq} is abbreviated:
in the code it is (a macro expanding to) a classical case split returning
true iff membership agrees on all valid strings.
Similarly, \code{eval_texists} is realized as a macro over a classical
\code{if}-term in the code, with the meaning shown.
The function-value case of \code{smt_model_eval_apply} calls the wrapper
\code{eval_fun_apply} rather than \code{model_fun_lookup} directly,
...
Note that in the code, the quantification in \code{re_ext_eq} is restricted
to \emph{valid} strings, i.e. those whose characters are within the legal
SMT-LIB character range.
...
we look up its behavior in the model
(via the wrapper \code{eval_fun_apply} discussed in the previous section)
...
\noindent\emph{[Note on Figure~\ref{fig:type-preservation} vs. the code:
the model-existence theorem is named
\code{total_typed_model_nonvacuous} in the code
(recall \code{model_wf} abbreviates \code{model_total_typed}),
and the well-typedness hypotheses of the latter two theorems are phrased via a
definition \code{term_has_non_none_type t},
which unfolds to \code{smt_typeof t} $\neq$ \code{SmtType.None}.]}
...
\noindent\emph{[Note on Figure~\ref{fig:eo-term} vs. the code:
in the code, \code{BitVec} is an \emph{ordinary} operator
(a member of \code{UserOp}, applied to its width via \code{Apply}),
not a member of \code{UserOp1} as shown;
the same holds for \code{Char} and \code{Seq}.
The code's \code{UserOp1} instead contains operators such as
\code{repeat}, \code{zero_extend}, \code{seq_empty} and the
datatype tester and updater operators \code{is} and \code{update},
and there is additionally an enumeration \code{UserOp3} for operators with
three indexed arguments.
The builtin list constructors are named
\code{__eo_List}, \code{__eo_List_nil} and \code{__eo_List_cons}.]}
...
\noindent\emph{[Note on Figure~\ref{fig:eo-to-smt} vs. the code:
the bit-vector type case matches
\code{(Term.Apply (Term.UOp UserOp.BitVec) (Term.Numeral n1))}
in the code, since \code{BitVec} is an ordinary (non-indexed) operator;
the reserved-name check is a helper
(\code{native_reserved_datatype_name}) rather than
\code{s.startsWith "@"}; and the \code{FunType} and other binary cases
evaluate their subterm translations through \code{let} bindings in a
different order than shown. The figure is otherwise representative.]}
...
\noindent\emph{[Note on Figure~\ref{fig:eo-closed} vs. the code:
\code{assumptionCheckGuard} is a presentation-only abbreviation;
in the code the guard
\code{(eo_and (eo_is_bool_type A) (eo_is_closed A))}
appears inline in the \code{assume_push} and input-assumption cases of
command execution (Figure~\ref{fig:checker-exec}).
Also, \code{model_agrees_on_globals} is stated via the constant lookup
method named \code{model_lookup} in Figure~\ref{fig:smt-model}.]}

...
\begin{figure}
\begin{lstlisting}
def smt_type_substitute (s : native_String) (d : SmtDatatype) : SmtType -> SmtType
  | (SmtType.Datatype s2 d2) => (SmtType.Datatype s2 (native_ite (native_streq s s2) d2 (smt_dt_substitute s d d2)))
  | (SmtType.TypeRef s2) => (native_ite (native_streq s s2) (SmtType.Datatype s d) (SmtType.TypeRef s2))
  | T => T

def smt_dtc_substitute (s : native_String) (d : SmtDatatype) : SmtDatatypeCons -> SmtDatatypeCons
...
def smt_dt_substitute (s : native_String) (d : SmtDatatype) : SmtDatatype -> SmtDatatype
...
This is a simple recursive method, where we traverse the constructors of the
...
Otherwise, the substitution is applied to that datatype, which handles
the case where the datatype has mutual recursion.
If \code{T} is a type reference, then if it is the datatype we are substituting,
we replace it by \code{(SmtType.Datatype s d)}, otherwise it is unchanged.
In all other cases, the datatype is unchanged.
...
def vcmp (v1 : SmtValue) (v2 : SmtValue) : native_Bool :=
...
which corresponds to a list of integers:
\begin{examplecode}
DList := (sum (cons Int (cons (TypeRef "List") unit)) (sum unit null))
\end{examplecode}
...
\begin{examplecode}
(Apply (Apply (DtCons "List" DList 0) 0) (DtCons "List" DList 1))
\end{examplecode}
...
def smt_typeof_guard (T : SmtType) (U : SmtType) : SmtType :=
  (native_ite (Teq T SmtType.None) SmtType.None U)
  
def smt_typeof_map_value : SmtMap -> SmtType
  | (SmtMap.cons i e m) => 
    let _v0 := (smt_typeof_map_value m)
    (native_ite (Teq (SmtType.Map (smt_typeof_value i) (smt_typeof_value e)) _v0) _v0 SmtType.None)
...
def smt_typeof_seq_value : SmtSeq -> SmtType
  | (SmtSeq.cons v vs) => 
    ...
    (native_ite (Teq (SmtType.Seq (smt_typeof_value v)) _v0) _v0 SmtType.None)
  | (SmtSeq.empty T) => (SmtType.Seq T)
  
def smt_typeof_dt_cons_value_rec (T : SmtType) : SmtDatatype -> native_Nat -> SmtType
  | (SmtDatatype.sum SmtDatatypeCons.unit d), native_nat_zero => T
  | (SmtDatatype.sum (SmtDatatypeCons.cons U c) d), native_nat_zero => 
    (SmtType.DtcAppType U (smt_typeof_dt_cons_value_rec T (SmtDatatype.sum c d) native_nat_zero))
  | (SmtDatatype.sum c d), (native_nat_succ n) => (smt_typeof_dt_cons_value_rec T d n)
  | d, n => SmtType.None

def smt_typeof_apply_value : SmtType -> SmtType -> SmtType
  | (SmtType.DtcAppType T U), V => (smt_typeof_guard T (native_ite (Teq T V) U SmtType.None))
  | T, U => SmtType.None
...
\subsubsection{Type Finiteness and Value Defaults and Value Canonicity}
...
\begin{figure}
\begin{lstlisting}
def smt_value_dt_substitute (s : native_String) (d : SmtDatatype) : SmtValue -> SmtValue
  | (SmtValue.DtCons s2 d2 i) => (SmtValue.DtCons s2 (smt_dt_substitute s d d2) i)
  | (SmtValue.Apply f v) => (SmtValue.Apply (smt_value_dt_substitute s d f) (smt_value_dt_substitute s d v))
  | v => v
  
def smt_datatype_cons_default (s : native_String) (d : SmtDatatype) (v : SmtValue) : SmtDatatypeCons -> SmtValue
  | SmtDatatypeCons.unit => v
  | (SmtDatatypeCons.cons T c) => 
    let _v0 := (smt_value_dt_substitute s d (smt_type_default T))
    (native_ite (veq _v0 SmtValue.NotValue) SmtValue.NotValue (smt_datatype_cons_default s d (SmtValue.Apply v _v0) c))

def smt_datatype_default (s : native_String) (d0 : SmtDatatype) (n : Nat) : SmtDatatype -> SmtValue
  | SmtDatatype.null => SmtValue.NotValue
  | (SmtDatatype.sum c d) => 
    let _v0 := (smt_datatype_cons_default s d0 (SmtValue.DtCons s d0 n) c)
    (native_ite (native_not (veq _v0 SmtValue.NotValue)) _v0 (smt_datatype_default s d0 (native_nat_succ n) d))
    
def smt_type_default : SmtType -> SmtValue
  | SmtType.Bool => (SmtValue.Boolean false)
  | SmtType.Int => (SmtValue.Numeral 0)
  | (SmtType.Datatype s d) => (smt_datatype_default s d 0 d)
  | (SmtType.Map T U) => (SmtValue.Map (SmtMap.default T (smt_type_default U)))
  ...
  | T => SmtValue.NotValue
\end{lstlisting}
\caption{Finite default value computation.}
...
Our default value method for atomic types returns an arbitrary value,
e.g. false for Booleans, 0 for integers, and so on.
For datatypes, we use a recursive method \code{(smt_datatype_default s d0 n)}
for returning a default datatype value.
This top-level method also carries the string \code{s} and datatype \code{d0} of the current datatype we are considering,
as well as the natural number index \code{n} counting the index of the current constructor.
...
in our definition of \code{smt_type_default}, so it will be treated as an error (\code{SmtValue.NotValue}).

In detail, \code{(smt_datatype_default s d0 n)} scans the list of constructors
and tries to return a value that is built from an application of that constructor,
via the submethod \code{(smt_datatype_cons_default s d0 v)}.
The submethod \code{smt_datatype_cons_default} also carries the string and datatype of the current datatype we are considering,
the value \code{v} corresponding to the (partially applied) datatype constructor of which an
application we are trying to construct.
That method may fail if it contains a field type of the current constructor is not well-formed,
which notice includes recursive references (\code{TypeRef}) to that datatype.
More generally, since the definition of \code{smt_type_default} is mutually recursive,
this includes references to any subfield of a datatype that we have traversed.

Furthermore, when constructing a subfield value via \code{smt_datatype_cons_default}
it is still important that we resolve dangling references to the datatype
we are defining the default value for, since the type we have traversed into
may contain recursive references to the top-level even though we do not construct values for them. 
Resolving dangling reference is done via the recursive method
\code{smt_value_dt_substitute}, which traverses a datatype value and replaces
dangling references to the current datatype.
We demonstrate this via an example. Consider the SMT-LIB datatype:
\begin{examplecode}
(declare-datatypes ((A 0) (B 0))
  (((mkA (b B)))) 
   ((mkB (a A)) (leaf)))
\end{examplecode}
which has the following representation \code{DA}:
\begin{examplecode}
DA := (sum (cons (Datatype "B" DB) unit) nil)
DB := (sum (cons (TypeRef "A") unit) (sum unit nil))
\end{examplecode}
When constructing a default value of type \code{(Datatype "A" DA)},
we call the method \code{(smt_datatype_default "A" DA)},
which in turn calls \code{(smt_datatype_cons_default "A" DA)}
on a constructor value and first constructor of \code{DA}.
This in turn invokes \code{(smt_type_default (Datatype "B" DB))},
which calls \code{(smt_datatype_cons_default "B" DB)},
which in turn calls \code{(smt_type_default  (TypeRef "A"))},
which fails.
We proceed with the second constructor of \code{DB},
which succeeds.
Note that we have not unfolded the datatype \code{DA},
hence we return the constructor value
\code{(SmtValue.DtCons "B" DB 1)}
where note that \code{DB} contains a dangling reference \code{(TypeRef "A")}.
When this is returned to \code{(smt_datatype_cons_default "A" DA)},
we apply the unfolding to replace the dangling references via the call
\code{(smt_value_dt_substitute "A" DA (SmtValue.DtCons "B" DB 1))}.
This used as an argument to the first constructor of \code{DA},
giving us the value:
\begin{examplecode}
(SmtValue.Apply (SmtValue.DtCons "A" DA 0) (SmtValue.DtCons "B" DB2 0))
\end{examplecode}
where \code{DB2} is:
\begin{examplecode}
DB2 := (sum (cons (sum (cons (Datatype "B" DB) unit) nil) unit) (sum unit nil))
\end{examplecode}
(FIXME)
...
if the index type of the map is finite, then the default element stored
must be the one obtained by the method \code{smt_type_default} in the previous section.
To understand this restriction,
...
\subsection{Inhabited and Well-Formedness of Types}

\begin{figure}[p]
\begin{lstlisting}
abbrev RefList := List String

def inhabited_type (T : SmtType) : Bool :=
  (Teq (smt_typeof_value (smt_type_default T)) T) &&
  (smt_value_canonical (smt_type_default T))

def smt_dt_cons_wf_rec : SmtDatatypeCons -> RefList -> Bool
  | (SmtDatatypeCons.cons (SmtType.TypeRef s) c), refs =>
      (s ∈ refs) && (smt_dt_cons_wf_rec c refs)
  | (SmtDatatypeCons.cons T c), refs =>
      (inhabited_type T) && (smt_type_wf_rec T refs) && (smt_dt_cons_wf_rec c refs)
...
def smt_dt_wf_rec : SmtDatatype -> RefList -> Bool
  ...
  | (SmtDatatype.sum c d), refs =>
      (smt_dt_cons_wf_rec c refs) && (smt_dt_wf_rec d refs)
...
def smt_type_wf_rec : SmtType -> RefList -> Bool
  ...
  | (SmtType.Seq x1), refs => (smt_type_wf_component x1)
  | (SmtType.Map x1 x2), refs =>
      (smt_type_wf_component x1) && (smt_type_wf_component x2)
  | (SmtType.Set x1), refs => (smt_type_wf_component x1)
...
\end{lstlisting}
\caption{Inhabitation, and type well-formedness.}
\label{fig:type-wf}
\end{figure}

Figure~\ref{fig:type-wf}
provides two key definitions which we will use when defining the properties
of SMT-LIB models we wish to consider.

First, \code{inhabited_type} is a predicate that is true iff
an SMT-LIB type is inhabited.
We define type inhabitedness operationally.
In particular, a type is considered inhabited if the method \code{smt_type_default}
(from Figure~\ref{fig:default-value})succeeds in generating a value of that type,
where recall that the type of values can be computed by the method \code{smt_typeof_value}.
We additionally require that the default value of the type is canonical.
TODO: this can likely be proven, not assumed.
...
and \smtvaluecanon{v1} = \smtvalue{v2} = true.
In this case, 
...
The method \code{smt_type_wf_component}
returns true if the given type is inhabited
and its component types are well-formed, as defined by the recursive method \code{smt_type_wf_rec}.
This method tracks a list of datatypes we have traversed beneath (\code{RefList}).

For datatype types,
we call the recursive helper \code{smt_dt_wf_rec} that returns true if the datatype is well-formed.
We append the datatype name \code{s} to our list of references when calling this method,
indicating that type references to \code{s} are valid in this context.
This method returns false if the datatype has no constructors.
Otherwise, we visit each constructor
and return true only if all constructors are well formed via \code{smt_dtc_wf_rec}.
Note we have a special case for one constructors to avoid returning false for an empty tail.
We determine whether a constructor is well-formed if
for each of its field types \code{T},
either \code{T} is a type reference in scope (\code{s ∈ refs}),
or otherwise \code{T} is an inhabited type that is well-formed, which we check
recursively, maintaining the list of references.
Note that carrying the reference list 
implies that mutually recursive datatypes are supported.

Note that type references are only permitted as direct children of
constructors.
Unguarded type reference are not well-formed given their case 
in \code{smt_dt_wf_rec}, which returns false.

The next cases in \code{smt_dt_wf_rec}
...
or element types of sequences),
we do not carry the list of reference when recursing
via \code{smt_type_wf_component}.
...
This prevents us from otherwise have e.g. sets of an uninhabited type be
inhabited via the empty set; sequences are guarded for the same reason.

The remaining cases in \code{smt_dt_wf_rec} include exceptions
...
def smt_typeof_eq (T : SmtType) (U : SmtType) : SmtType :=
  (smt_typeof_guard T (native_ite (native_Teq T U) SmtType.Bool SmtType.None))
...
def native_fun_typed (M : SmtModel) : Prop :=
  ∀ fid A B i,
    ...
    smt_typeof_value (native_eval_ifun_apply M fid A B i) = B ∧
      smt_value_canonical (native_eval_ifun_apply M fid A B i) = true

def model_total_typed (M : SmtModel) : Prop :=
  (∀ s T, smt_type_wf T = true -> smt_typeof_value (model_const_lookup M s T) = T) ∧
  (∀ s T, smt_type_wf T = true -> smt_value_canonical (model_const_lookup M s T)) = true ∧
...
open Classical in
noncomputable def re_ext_eq (r1 r2 : native_RegLan) : Bool :=
  if ∀ s : String, native_str_in_re s r1 = native_str_in_re s r2 then true else false
...
\begin{lstlisting}
open Classical in
noncomputable def native_eval_texists (M : SmtModel) (s : String)
    (T : SmtType) (body : SmtTerm) : SmtValue :=
  if ∃ v : SmtValue,
      smt_typeof_value v = T ∧
        smt_value_canonical v = true ∧
        smt_model_eval (model_push M s T v) body = (SmtValue.Boolean true) then
    SmtValue.Boolean true
  else
    SmtValue.Boolean false

open Classical in
noncomputable def native_eval_tforall (M : SmtModel) (s : String)
    (T : SmtType) (body : SmtTerm) : SmtValue :=
  if ∀ v : SmtValue,
      smt_typeof_value v = T ->
        smt_value_canonical v = true ->
...
open Classical in
noncomputable def native_eval_tchoice (M : SmtModel) (s : String)
    (T : SmtType) (body : SmtTerm) : SmtValue :=
  if hSat : ∃ v : SmtValue,
      smt_typeof_value v = T ∧
        smt_value_canonical v = true ∧
  ...
  else if hTy : ∃ v : SmtValue, smt_typeof_value v = T ∧ smt_value_canonical v then
    Classical.choose hTy
  else
    SmtValue.NotValue

noncomputable def native_eval_tchoice_nth (M : SmtModel) (s : String)
    (T : SmtType) (body : SmtTerm) : Nat -> SmtValue
  | 0 => native_eval_tchoice M s T body
  | Nat.succ n =>
      let v := native_eval_tchoice M s T body
      match body with
      | SmtTerm.exists s' T' body' =>
          native_eval_tchoice_nth (model_push M s T v) s' T' body' n
      | _ => SmtValue.NotValue
\end{lstlisting}
\caption{Evaluation of quantifiers and choice.}

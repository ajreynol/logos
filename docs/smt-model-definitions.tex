...
The formalization that follows is faithful to SMT-LIB apart from four
restrictions on the class of models it admits. Each is stated again at the
definition that introduces it; they are collected here so that the list can be
read in one place. Arrays are interpreted as almost-constant maps, values of
type \code{Real} are assumed to be rational, uninterpreted sorts are assumed to
be of infinite cardinality, and sets are finite.

Every model of this semantics is an SMT-LIB model, and the converse does not
hold. Unsatisfiability as defined here is therefore implied by unsatisfiability
in SMT-LIB and does not imply it: a formula satisfiable in SMT-LIB only by a
model this semantics cannot express is treated here as unsatisfiable. Each of
the four is confined to quantified or nonlinear reasoning, and on the
quantifier-free linear fragments the two notions agree. Separately, parametric
datatypes are rejected by the parser rather than interpreted.

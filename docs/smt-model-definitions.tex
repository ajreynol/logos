...
The definitions do not perform that reordering, however, and neither does
anything upstream of them: both front ends carry a declaration block through in
the order the input wrote it, so the assumption is not in fact discharged.
A block that is well-founded but written in another order is rejected, and it
is rejected \emph{as a whole}.

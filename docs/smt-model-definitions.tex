...
The latter tracks a list of datatype names we are currently traversing beneath (\code{RefList}),
recording the recursive references that are permitted in the current context.

For a datatype type \code{(Datatype s d)},
we first check that \code{s} does not already occur in the reference list \code{refs}.
If it does, the datatype aliases an enclosing datatype of the same name,
which we consider to be not well-formed, returning false.
Otherwise, we call the recursive helper \code{smt_dt_wf_rec} on the list of constructors \code{d}.
...
where the name \code{s} of the datatype being defined is added to the reference list (\code{(s :: refs)}),
indicating that type references to \code{s} are valid as fields of \code{c}.
...
In addition, \code{smt_dt_wf_rec} requires that the datatype has at least one
\emph{base} constructor, that is, a constructor that is well-formed
\emph{without} relying on a recursive reference to \code{s}.
This is tracked by the accumulator \code{haveBase}: when processing a constructor \code{c},
we update it based on whether \code{c} is well-formed when \code{s} is \emph{not}
in scope, i.e. \code{(smt_dt_cons_wf_rec c refs)}.
After all constructors have been processed, the \code{null} case returns \code{haveBase}.
In particular, a datatype with no constructors, or one all of whose constructors
recursively reference \code{s}, has no base constructor and is considered not well-formed,
since it would admit no finite values.
...
we do not carry the list of references when recursing

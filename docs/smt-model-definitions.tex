...
\begin{itemize}
  \item \code{SmtTerm}, the type of all SMT-LIB terms,
  \item \code{SmtType}, the type of all SMT-LIB types,
  \item \code{SmtValue}, the type of all SMT-LIB values,
  \item \code{SmtModel}, a type corresponding to an SMT-LIB model,
  \item \code{Term}, the type of all Eunoia terms and types, and
  \item \code{CmdList}, a list of Eunoia command, i.e. a proof.
\end{itemize}
...
\begin{itemize}
  \item \code{eo_typeof : Term -> Term}
    \begin{itemize}
      \item Given a Eunoia term, this returns the Eunoia term that corresponds to its type.
    \end{itemize}
  \item \code{eo_is_closed : Term -> Bool}
    \begin{itemize}
      \item Given a Eunoia term, this returns true iff it is a \emph{closed} term, i.e. has no free variables.
    \end{itemize}
  \item \code{eo_is_refutation : CmdList -> Bool}
    \begin{itemize}
      \item Returns true if Logos successfully executes the given command list, and is a valid, closed proof of false.
    \end{itemize}
\end{itemize}
...
This means it is defined solely over the \code{Term} datatype.
Second, the speficiation layer above does \emph{not} rely on the Eunoia definitions, e.g. of the Eunoia type system.
Third, the definition of the Eunoia type system is encapsulated as a method, \code{eo_typeof}, which returns the type of a given Eunoia term.
All computational side conditions are essentially untyped manipulations over a single deep embedding of Eunoia types and terms, as encapsulated as \code{Term}.
A command (in \code{CmdList}) is either a proof step or an input assumption.\footnote{There are also commands for local assumptions and steps which close them.}
A proof checking step in Eunoia succeeds iff the conclusion of that step has type \code{Bool},
i.e. \code{eo_typeof} for that term returns the Eunoia term \code{Bool}.
An input assumption command succeeds if it is a \emph{closed} Eunoia term of type \code{Bool}.
...
Figure~\ref{fig:smt-types} gives the official definition of SMT-LIB types.
...
The second argument (a \code{SmtDatatype}) denotes the list of constructors of that datatype,
...
Below, we will see that values of datatype types are datatype constructors (\code{DtCons})
applied to other values.
We use an curried apply (\code{Apply}) constructor
...
Second, a datatype itself may be uninhabited, i.e. there are no \code{SmtValue}
that have that type.
...
\code{(DtcAppType Int (DtcAppType (Datatype "list" D) (Datatype "list" D)))}
In detail, this type
is a function that expects an integer,
and returns a function that expects a list and returns a list.

Note that the type of constructors is intentionally distinct from function types (\code{FunType}).

Finally, the last constructor in this list denotes uninterpreted sorts (\code{USort}),

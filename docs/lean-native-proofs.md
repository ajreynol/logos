...
Besides the s-expression (Eunoia) syntax read by `logos` and `logos-mini`, Logos accepts proofs
written directly as Lean evaluation scripts.  These are read by the `-native` executables,
`logos-native` for CPC and `logos-mini-native` for CpcMini.  See the [README](../README.md) for
the s-expression format and for building the executables.

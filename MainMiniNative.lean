...
unsafe def main (args : List String) : IO UInt32 := do
  let [path] := args
    | IO.eprintln "usage: logos-mini-native <path-to-proof-file>"
      return 1
  Lean.initSearchPath (← Lean.findSysroot)
  Lean.enableInitializersExecution
  let env ← Lean.importModules #[`CpcMini.Logos] {} 0 (loadExts := true)
  let coreContext := { fileName := path, fileMap := default }
  let coreState := { env }
  let (res, _, _) ← Lean.Meta.MetaM.toIO (evalLogoExpr path) coreContext coreState
  return res.toUInt32

-- To build, run "lake build logos-mini-native" in this directory.
-- To run, use "lake exe logos-mini-native <path-to-proof-file>".

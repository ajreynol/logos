...
def __eo_to_smt : Term -> SmtTerm
  ...
  | (Term.Apply (Term.Apply (Term.Apply (Term.Apply (Term.UOp UserOp._at_strings_replace_all_result) x1) x2) x3) x4) => 
    let _v0 := (__eo_to_smt x2)
    let _v1 := (__eo_to_smt x1)
    (SmtTerm.str_replace_all (SmtTerm.str_substr _v1 (__eo_to_smt_strings_occur_index _v1 _v0 (__eo_to_smt x4)) (SmtTerm.str_len _v1)) _v0 (__eo_to_smt x3))
  | (Term.Apply (Term.Apply (Term.Apply (Term.Apply (Term.UOp UserOp._at_strings_replace_re_all_result) x1) x2) x3) x4) => 
    let _v0 := (__eo_to_smt x2)
    let _v1 := (__eo_to_smt x1)
    (SmtTerm.str_replace_re_all (SmtTerm.str_substr _v1 (__eo_to_smt_strings_occur_index_re _v1 _v0 (__eo_to_smt x4)) (SmtTerm.str_len _v1)) _v0 (__eo_to_smt x3))

...
def __eo_to_smt : Term -> SmtTerm
  ...
  | (Term.UOp2 UserOp2._at_quantifiers_skolemize (Term.Apply (Term.Apply (Term.UOp UserOp.forall) x1) x2) x3) => (native_ite (__eo_to_smt_nat_is_valid x3) (__eo_to_smt_guard_closed (Term.Apply (Term.Apply (Term.UOp UserOp.forall) x1) x2) (__eo_to_smt_quantifiers_skolemize (__eo_to_smt_exists x1 (SmtTerm.not (__eo_to_smt x2))) (__eo_to_smt_nat x3))) SmtTerm.None)

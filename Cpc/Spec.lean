...
def __eo_to_smt_re_unfold_pos_component (s : SmtTerm) : SmtTerm -> native_Nat -> SmtTerm
  | (SmtTerm.re_concat r1 r2), native_nat_zero => 
    ...
    (SmtTerm.choice_nth "@x" _v0 (SmtTerm.and (SmtTerm.eq s (SmtTerm.str_concat _v2 _v4)) (SmtTerm.and (SmtTerm.str_in_re _v2 r1) (SmtTerm.str_in_re _v4 r2))) native_nat_zero)
...
def __eo_to_smt_type : Term -> SmtType
  ...
  | (Term.UOp UserOp.UnitTuple) => (SmtType.Datatype "@Tuple" (SmtDatatype.sum SmtDatatypeCons.unit SmtDatatype.null))
...
def __eo_to_smt_tuple_app_extend : SmtTerm -> SmtType -> SmtTerm
  ...
  | (SmtTerm.DtCons "@Tuple" (SmtDatatype.sum c SmtDatatype.null) native_nat_zero), T => (SmtTerm.DtCons "@Tuple" (SmtDatatype.sum (__smtx_dtc_concat c (SmtDatatypeCons.cons T SmtDatatypeCons.unit)) SmtDatatype.null) native_nat_zero)
  | s, T => SmtTerm.None
...
def __eo_to_smt_tuple_update : SmtType -> SmtTerm -> SmtTerm -> SmtTerm -> SmtTerm
  | (SmtType.Datatype "@Tuple" d), (SmtTerm.Numeral n), t, u => (native_ite (native_zleq 0 n) (__eo_to_smt_updater (SmtTerm.DtSel "@Tuple" d native_nat_zero (native_int_to_nat n)) t u) SmtTerm.None)
...
def __eo_to_smt : Term -> SmtTerm
  ...
  | (Term._at_array_deq_diff x1 x2) => 
    ...
    let _v2 := (SmtTerm.Var "@x" _v0)
    (SmtTerm.choice_nth "@x" _v0 (SmtTerm.not (SmtTerm.eq (SmtTerm.select (__eo_to_smt x1) _v2) (SmtTerm.select (__eo_to_smt x2) _v2))) native_nat_zero)
  | (Term.Apply (Term.UOp UserOp._at_bvsize) x1) => 
    let _v1 := (__smtx_bv_sizeof_type (__smtx_typeof (__eo_to_smt x1)))
    (native_ite (native_zleq 0 _v1) (SmtTerm.plus (SmtTerm.Numeral _v1) (SmtTerm.Numeral 0)) SmtTerm.None)
  | (Term.Apply (Term.Apply (Term.UOp UserOp.concat) x1) x2) => (SmtTerm.concat (__eo_to_smt x1) (__eo_to_smt x2))
  | (Term.Apply (Term.extract x1 x2) x3) => (SmtTerm.extract (__eo_to_smt x1) (__eo_to_smt x2) (__eo_to_smt x3))
  | (Term.Apply (Term.repeat x1) x2) => (SmtTerm.repeat (__eo_to_smt x1) (__eo_to_smt x2))
  ...
  | (Term.Apply (Term.zero_extend x1) x2) => (SmtTerm.zero_extend (__eo_to_smt x1) (__eo_to_smt x2))
  | (Term.Apply (Term.sign_extend x1) x2) => (SmtTerm.sign_extend (__eo_to_smt x1) (__eo_to_smt x2))
  | (Term.Apply (Term.rotate_left x1) x2) => (SmtTerm.rotate_left (__eo_to_smt x1) (__eo_to_smt x2))
  | (Term.Apply (Term.rotate_right x1) x2) => (SmtTerm.rotate_right (__eo_to_smt x1) (__eo_to_smt x2))
  ...
  | (Term._at_bv x1 x2) => (__eo_to_smt__at_bv (__eo_to_smt x1) (__eo_to_smt x2))
  ...
  | (Term.Apply (Term.re_exp x1) x2) => (SmtTerm.re_exp (__eo_to_smt x1) (__eo_to_smt x2))
  ...
  | (Term.Apply (Term.re_loop x1 x2) x3) => (SmtTerm.re_loop (__eo_to_smt x1) (__eo_to_smt x2) (__eo_to_smt x3))
  ...
  | (Term._at_re_unfold_pos_component x1 x2 x3) => (native_ite (native_teq (__eo_is_z x3) (Term.Boolean true)) (native_ite (native_teq (__eo_is_neg x3) (Term.Boolean false)) (__eo_to_smt_re_unfold_pos_component (__eo_to_smt x1) (__eo_to_smt x2) (__eo_to_smt_nat x3)) SmtTerm.None) SmtTerm.None)
  | (Term._at_strings_deq_diff x1 x2) => 
    let _v0 := (SmtTerm.Numeral 1)
    let _v2 := (SmtTerm.Var "@x" SmtType.Int)
    (SmtTerm.choice_nth "@x" SmtType.Int (SmtTerm.not (SmtTerm.eq (SmtTerm.str_substr (__eo_to_smt x1) _v2 _v0) (SmtTerm.str_substr (__eo_to_smt x2) _v2 _v0))) native_nat_zero)
  | (Term.Apply (Term._at_strings_stoi_result x1) x2) => (SmtTerm.str_to_int (SmtTerm.str_substr (__eo_to_smt x1) (SmtTerm.Numeral 0) (__eo_to_smt x2)))
  | (Term._at_strings_stoi_non_digit x1) => (SmtTerm.str_indexof_re (__eo_to_smt x1) (SmtTerm.re_comp (SmtTerm.re_range (SmtTerm.String "0") (SmtTerm.String "9"))) (SmtTerm.Numeral 0))
  | (Term.Apply (Term._at_strings_itos_result x1) x2) => (SmtTerm.mod (__eo_to_smt x1) (SmtTerm.multmult (SmtTerm.Numeral 10) (__eo_to_smt x2)))
  ...
  | (Term.Apply (Term.Apply (Term._at_witness_string_length x1) x2) x3) => 
    let _v0 := (__eo_to_smt_type x1)
  ...
  | (Term.Apply (Term.is x1) x2) => (SmtTerm.Apply (__eo_to_smt_tester (__eo_to_smt x1)) (__eo_to_smt x2))
  | (Term.Apply (Term.Apply (Term.update x1) x2) x3) => (__eo_to_smt_updater (__eo_to_smt x1) (__eo_to_smt x2) (__eo_to_smt x3))
  ...
  | (Term.Apply (Term.tuple_select x1) x2) => (__eo_to_smt_tuple_select (__eo_to_smt_type (__eo_typeof x2)) (__eo_to_smt x1) (__eo_to_smt x2))
  | (Term.Apply (Term.Apply (Term.tuple_update x1) x2) x3) => (__eo_to_smt_tuple_update (__eo_to_smt_type (__eo_typeof x2)) (__eo_to_smt x1) (__eo_to_smt x2) (__eo_to_smt x3))
  ...
  | (Term.Apply (Term.UOp UserOp.set_choose) x1) => 
    ...
    (SmtTerm.choice_nth "@x" _v0 (SmtTerm.set_member (SmtTerm.Var "@x" _v0) (__eo_to_smt x1)) native_nat_zero)
  ...
  | (Term.Apply (Term.UOp UserOp.set_is_singleton) x1) => 
    let _v0 := (__eo_to_smt_type (__eo_typeof (Term.Apply (Term.UOp UserOp.set_choose) x1)))
    (SmtTerm.exists "@x" _v0 (SmtTerm.eq (__eo_to_smt x1) (SmtTerm.set_singleton (SmtTerm.Var "@x" _v0))))
  ...
  | (Term._at_sets_deq_diff x1 x2) => 
    ...
    let _v2 := (SmtTerm.Var "@x" _v0)
    (SmtTerm.choice_nth "@x" _v0 (SmtTerm.not (SmtTerm.eq (SmtTerm.set_member _v2 (__eo_to_smt x1)) (SmtTerm.set_member _v2 (__eo_to_smt x2)))) native_nat_zero)
  ...
  | (Term.Apply (Term.int_to_bv x1) x2) => (SmtTerm.int_to_bv (__eo_to_smt x1) (__eo_to_smt x2))

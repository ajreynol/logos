...
private theorem eo_to_smt_imp_eq (A B : Term) :
    __eo_to_smt (Term.Apply (Term.Apply Term.imp A) B) =
      theory2 SmtTheoryOp.imp (__eo_to_smt A) (__eo_to_smt B) := by
  rw [__eo_to_smt.eq_def]
...
theorem eo_has_bool_type_and_left (A B : Term) :
  ...
  have hNN : term_has_non_none_type
    ...
    rw [hTy]
    simp
  exact (bool_binop_args_bool_of_non_none (op := theory2 SmtTheoryOp.and)
      ...
      | intro_true htyA hEvalA =>
          have htyB : __smtx_typeof (__eo_to_smt B) = SmtType.Bool := by
            have hNN : term_has_non_none_type
              ...
              rw [htyImp]
              simp
            exact (bool_binop_args_bool_of_non_none (op := theory2 SmtTheoryOp.imp)
...
theorem eo_interprets_not_of_false (M : SmtModel) (t : Term) :
  ...
  | intro_false hTy hEval =>
      refine smt_interprets.intro_true M
          (theory1 SmtTheoryOp.not (__eo_to_smt t)) ?_ ?_
      · rw [typeof_not_eq]

...
import Cpc.Proofs.TypePreservation.TupleAssumptions
...
attribute [local simp] _root_.__eo_to_smt_tuple_cons
attribute [local simp] _root_.__eo_to_smt_tuple_cons_guarded
attribute [local simp] Smtm.__smtx_type_wf_component

private def eo_to_smt_tuple_cons_tail
    (tail : SmtTerm) (T : SmtType) (head : SmtTerm) : SmtTerm :=
  _root_.__eo_to_smt_tuple_cons head T tail

@[simp] private theorem eo_to_smt_tuple_cons_tail_apply
    (f a : SmtTerm) (T : SmtType) (head : SmtTerm) :
    eo_to_smt_tuple_cons_tail (SmtTerm.Apply f a) T head =
      SmtTerm.Apply (eo_to_smt_tuple_cons_tail f T head) a := by
  simp [eo_to_smt_tuple_cons_tail, _root_.__eo_to_smt_tuple_cons]

private def eo_to_smt_tuple_cons_checked_tail
    (tail : SmtTerm) (T : SmtType) (head : SmtTerm) : SmtTerm :=
  let raw := eo_to_smt_tuple_cons_tail tail T head
...
private theorem smtx_tuple_type_wf_tail_of_cons_wf
    {T : SmtType} {c : SmtDatatypeCons}
    (hWf :
      __smtx_type_wf
          (SmtType.Datatype "@Tuple"
  ...
  have hConsWf :
    ...
    have hDtWf :
        __smtx_dt_wf_rec
          ...
          true := by
      have hParts :
          ¬ "@Tuple" ∈ native_reflist_nil ∧
            __smtx_dt_wf_rec
                (SmtDatatype.sum (SmtDatatypeCons.cons T c) SmtDatatype.null)
                (native_reflist_insert native_reflist_nil "@Tuple") =
              true := by
        simpa [fullTy, __smtx_type_wf_rec, native_reflist_contains,
          native_reflist_insert, native_ite] using hFullParts.2
      exact hParts.2
    simpa [__smtx_dt_wf_rec] using hDtWf
  have hTailConsWf :
  ...
  have hTailRec :
      __smtx_type_wf_rec tailTy native_reflist_nil = true := by
    have hNoRec : ¬ "@Tuple" ∈ native_reflist_nil := by
      intro h
      cases h
    have hTailDt :
        __smtx_dt_wf_rec (SmtDatatype.sum c SmtDatatype.null)
            (native_reflist_insert native_reflist_nil "@Tuple") =
          true := by
      simpa [__smtx_dt_wf_rec] using hTailConsWf
    have hParts :
        ¬ "@Tuple" ∈ native_reflist_nil ∧
          __smtx_dt_wf_rec (SmtDatatype.sum c SmtDatatype.null)
              (native_reflist_insert native_reflist_nil "@Tuple") =
            true :=
      And.intro hNoRec hTailDt
    simpa [tailTy, __smtx_type_wf_rec, native_reflist_contains,
      native_reflist_insert, native_ite] using hParts
  have hDefaultNN :=
    smtx_tuple_tail_default_notvalue_of_cons_wf hWf
  have hDefaultTC :=
    Smtm.cpc_tuple_tail_default_typed_canonical_assumption T c hWf
  have hTailInh : native_inhabited_type tailTy = true := by
    let tailD := SmtDatatype.sum c SmtDatatype.null
    let tailDefault :=
      __smtx_datatype_cons_default "@Tuple" tailD
    ...
    have hTy :
      ...
      simpa [tailTy, tailD, tailDefault, hTypeDefault] using hDefaultTC.1
    have hCan :
...
private theorem eo_to_smt_tuple_cons_ne_dt_sel
    (t : SmtTerm) (T : SmtType) (v : SmtTerm) (s : native_String) (d : SmtDatatype)
    (i j : native_Nat) :
    eo_to_smt_tuple_cons_tail t T v ≠ SmtTerm.DtSel s d i j := by
  intro h
  cases t <;>
    simp [eo_to_smt_tuple_cons_tail, _root_.__eo_to_smt_tuple_cons] at h
    ...
    · rename_i c dTail
      ...
      · cases i0
        · by_cases hs : s0 = "@Tuple"
          · subst s0
            simp [eo_to_smt_tuple_cons_tail, _root_.__eo_to_smt_tuple_cons] at h
          · simp [eo_to_smt_tuple_cons_tail, _root_.__eo_to_smt_tuple_cons, hs] at h
        · simp [eo_to_smt_tuple_cons_tail, _root_.__eo_to_smt_tuple_cons] at h
      · simp [eo_to_smt_tuple_cons_tail, _root_.__eo_to_smt_tuple_cons] at h

private theorem eo_to_smt_tuple_cons_ne_dt_tester
    (t : SmtTerm) (T : SmtType) (v : SmtTerm) (s : native_String) (d : SmtDatatype)
    (i : native_Nat) :
    eo_to_smt_tuple_cons_tail t T v ≠ SmtTerm.DtTester s d i := by
  intro h
  cases t <;>
    simp [eo_to_smt_tuple_cons_tail, _root_.__eo_to_smt_tuple_cons] at h
  case DtCons s0 d0 i0 =>
...
private theorem eo_to_smt_tuple_cons_ne_dt_cons
    (t : SmtTerm) (T : SmtType) (v : SmtTerm) (s : native_String) (d : SmtDatatype)
    (i : native_Nat) :
    eo_to_smt_tuple_cons_tail t T v ≠ SmtTerm.DtCons s d i := by
  intro h
  cases t <;>
    simp [eo_to_smt_tuple_cons_tail, _root_.__eo_to_smt_tuple_cons] at h
  case DtCons s0 d0 i0 =>
...
private theorem eo_to_smt_tuple_cons_guarded_ne_term
    (U : SmtType) (t target : SmtTerm)
    (hRaw : t ≠ target) (hNone : SmtTerm.None ≠ target) :
    _root_.__eo_to_smt_tuple_cons_guarded U t ≠ target := by
  intro h
  ...
  case Datatype s d =>
    ...
    · simp [_root_.__eo_to_smt_tuple_cons_guarded, hs] at h
      exact hNone h
  all_goals exact hNone h

private theorem eo_to_smt_tuple_cons_checked_ne_dt_sel
    (t : SmtTerm) (T : SmtType) (v : SmtTerm) (s : native_String) (d : SmtDatatype)
    (i j : native_Nat) :
    eo_to_smt_tuple_cons_checked_tail t T v ≠ SmtTerm.DtSel s d i j := by
  intro h
  unfold eo_to_smt_tuple_cons_checked_tail at h
  exact
    eo_to_smt_tuple_cons_guarded_ne_term (__smtx_typeof t)
      (_root_.__eo_to_smt_tuple_cons_guarded
        (__smtx_typeof (eo_to_smt_tuple_cons_tail t T v))
        (eo_to_smt_tuple_cons_tail t T v))
      (SmtTerm.DtSel s d i j)
      (eo_to_smt_tuple_cons_guarded_ne_term
        (__smtx_typeof (eo_to_smt_tuple_cons_tail t T v))
        (eo_to_smt_tuple_cons_tail t T v) (SmtTerm.DtSel s d i j)
        (eo_to_smt_tuple_cons_ne_dt_sel t T v s d i j)
        (by intro hNone; cases hNone))
      (by intro hNone; cases hNone) h

private theorem eo_to_smt_tuple_cons_checked_ne_dt_tester
    (t : SmtTerm) (T : SmtType) (v : SmtTerm) (s : native_String) (d : SmtDatatype)
    (i : native_Nat) :
    eo_to_smt_tuple_cons_checked_tail t T v ≠ SmtTerm.DtTester s d i := by
  intro h
  unfold eo_to_smt_tuple_cons_checked_tail at h
  exact
    eo_to_smt_tuple_cons_guarded_ne_term (__smtx_typeof t)
      ...
      (eo_to_smt_tuple_cons_guarded_ne_term
        (__smtx_typeof (eo_to_smt_tuple_cons_tail t T v))
        (eo_to_smt_tuple_cons_tail t T v) (SmtTerm.DtTester s d i)
...
private theorem eo_to_smt_tuple_cons_checked_ne_dt_cons
    (t : SmtTerm) (T : SmtType) (v : SmtTerm) (s : native_String) (d : SmtDatatype)
    (i : native_Nat) :
    eo_to_smt_tuple_cons_checked_tail t T v ≠ SmtTerm.DtCons s d i := by
  intro h
  unfold eo_to_smt_tuple_cons_checked_tail at h
  exact
    eo_to_smt_tuple_cons_guarded_ne_term (__smtx_typeof t)
      ...
      (eo_to_smt_tuple_cons_guarded_ne_term
        (__smtx_typeof (eo_to_smt_tuple_cons_tail t T v))
        (eo_to_smt_tuple_cons_tail t T v) (SmtTerm.DtCons s d i)
...
private theorem eo_to_smt_tuple_ne_dt_sel
  ...
  intro h
  have h' :
      eo_to_smt_tuple_cons_checked_tail (__eo_to_smt x)
          (__eo_to_smt_type (__eo_typeof y)) (__eo_to_smt y) =
        SmtTerm.DtSel s d i j := by
    change
      eo_to_smt_tuple_cons_checked_tail (__eo_to_smt x)
          (__eo_to_smt_type (__eo_typeof y)) (__eo_to_smt y) =
...
private theorem smtx_type_context_substitute_self_non_chain_field_apply
  ...
  | SmtType.TypeRef r, refs, _hField, _hT => by
      ...
      · by_cases hSub : r = sub
        ...
        · have hRootNe : root ≠ r := by
            intro h
            exact hRoot h.symm
          have hSubNe : sub ≠ r := by
            intro h
            exact hSub h.symm
          simp [smtx_type_context_substitute_apply,
            smtx_type_substitute_top_apply,
            smtx_chain_type_context_substitute_apply, native_ite,
            native_Teq, native_streq, hRoot, hRootNe, hSub, hSubNe]
  ...
  | SmtType.TypeRef r, refs, hField, hOldT => by
      ...
      · have hRootNe : root ≠ r := by
        ...
        have hTRaw : ¬ dt_cons_chain_result (SmtType.TypeRef r) := by
          simpa [smtx_type_substitute_top_apply, native_ite, native_streq,
            hRoot, hRootNe] using hOldT
        simpa [smtx_type_substitute_top_apply, native_ite, native_streq,
          hRoot, hRootNe] using
...
private theorem smtx_value_dt_substitute_typeof_of_non_chain_apply
  ...
  | SmtValue.Map m, T, hT, hv => by
      simpa [smtx_value_dt_substitute_apply] using hv
  | SmtValue.Fun fid A B, T, hT, hv => by
      simpa [smtx_value_dt_substitute_apply] using hv
...
private theorem smtx_value_dt_substitute_typeof_non_chain_top_apply
  ...
  | TypeRef r =>
      ...
      · have hNe : sub ≠ r := by
          intro h
          exact hEq h.symm
        simpa [smtx_type_substitute_top_apply, native_ite, native_streq, hEq, hNe]
          using hPres
  | None =>
      exact False.elim (hT (by simp [dt_cons_chain_result]))
  | Datatype s d =>
      ...
      have hRecEq :
        ...
        cases U <;>
          simp [field', smtx_dtc_substitute_field_type, __smtx_dt_substitute,
            ...
            smtx_type_substitute_top_apply, native_ite, native_streq, native_Teq]
      rw [hRecEq]
...
private theorem eo_to_smt_typeof_matches_translation_apply_generic_from_valid_ih
  ...
  | inl hFun =>
      ...
      rcases (by
        simpa [eo_type_valid, eo_type_valid_rec] using hFValid' :
...
private theorem eo_to_smt_type_typeof_apply_dt_cons_of_smt_apply_from_ih
  ...
  | inl hFun =>
      ...
      rcases eo_to_smt_type_eq_fun hEoHead with
        ⟨U, V, hHeadEo, hU, hV⟩
      have hHeadValid := hHeadAll.2
      rw [hHeadEo] at hHeadValid
      rcases (by
        simpa [eo_type_valid_rec] using hHeadValid :
...
private theorem eo_to_smt_eq_dt_cons_cases
  ...
  case UOp3 op t r idx =>
    ...
    case _at_witness_string_length =>
      exfalso
      change native_ite (native_teq (__eo_typeof idx) (Term.UOp UserOp.Int))
          (SmtTerm.choice_nth "@x" (__eo_to_smt_type t)
            (SmtTerm.eq
              (SmtTerm.str_len (SmtTerm.Var "@x" (__eo_to_smt_type t)))
              (__eo_to_smt r)) 0) SmtTerm.None =
        SmtTerm.DtCons s d i at hy
      unfold native_ite at hy
...
private theorem eo_to_smt_typeof_matches_translation_apply_purify
    ...
    (ihPurify :
      ...
      __smtx_typeof (__eo_to_smt (Term._at_purify y)) =
        ...
        eo_type_valid (__eo_typeof (Term._at_purify y)))
    (ihX :
      __smtx_typeof (__eo_to_smt x) ≠ SmtType.None ->
      __smtx_typeof (__eo_to_smt x) = __eo_to_smt_type (__eo_typeof x) ∧
        eo_type_valid (__eo_typeof x))
  ...
  have hGeneric :
    ...
    exact generic_apply_type_of_non_special_head _ _
      ...
      (by
        ...
        cases hy)
  exact eo_to_smt_typeof_matches_translation_apply_generic_from_ih_of_valid
    (Term._at_purify y) x ihPurify ihX hGeneric (by rfl) (by rfl) hNonNone
...
private theorem eo_to_smt_type_tuple_eq_tuple_datatype_of_wf
    ...
    ∃ d, __eo_to_smt_type_tuple A B = SmtType.Datatype "@Tuple" d := by
  ...
  case Datatype s d =>
    ...
    · subst s
      cases d with
      | null =>
          simp [__eo_to_smt_type_tuple, __smtx_type_wf,
            __smtx_type_wf_rec, native_and] at hWf
      | sum c rest =>
          cases rest with
          | null =>
              ...
              · exact ⟨
                  ...
                  by
                    simp [__eo_to_smt_type_tuple, hComp, native_ite]⟩
              · exfalso
                simp [__eo_to_smt_type_tuple, hComp, __smtx_type_wf,
                  __smtx_type_wf_component, __smtx_type_wf_rec, native_and,
                  native_ite] at hWf
          | sum c' rest' =>
              simp [__eo_to_smt_type_tuple, __smtx_type_wf,
                __smtx_type_wf_rec, native_and] at hWf
    · cases d <;> simp [__eo_to_smt_type_tuple, hs, __smtx_type_wf,
        __smtx_type_wf_rec, native_and] at hWf
...
private theorem smtx_type_substitute_top_apply_tuple_of_eo_valid_rec
    (base : SmtDatatype) :
    {refs : RefList} -> "@Tuple" ∉ refs -> {T : Term} ->
      eo_type_valid_rec refs T ->
        smtx_type_substitute_top_apply "@Tuple" base (__eo_to_smt_type T) =
          __eo_to_smt_type T
  | refs, hNo, Term.Bool, _hValid => by
      simp [__eo_to_smt_type, smtx_type_substitute_top_apply, native_ite, native_Teq]
  | refs, hNo, Term.DatatypeType s d, hValid => by
      ...
      have hNo' : "@Tuple" ∉ s :: refs := by
        ...
        · exact hNo hMem
      have hDSub :
          __smtx_dt_substitute "@Tuple" base (__eo_to_smt_datatype d) =
      ...
      simp [__eo_to_smt_type, hReserved, smtx_type_substitute_top_apply,
        native_ite, native_streq, hNe, hDSub]
  | refs, hNo, Term.DatatypeTypeRef s, hValid => by
      ...
      have hNe : s ≠ "@Tuple" := by
        ...
        rw [eo_reserved_datatype_name_tuple] at hReserved
        cases hReserved
      have hTupleNe : "@Tuple" ≠ s := by
        intro hs
        exact hNe hs.symm
      simp [__eo_to_smt_type, hReserved, smtx_type_substitute_top_apply,
        native_ite, native_streq, hNe, hTupleNe]
  ...
  | refs, hNo, Term.Apply (Term.UOp UserOp.BitVec) (Term.Numeral n), hValid => by
      ...
      simp [__eo_to_smt_type, hn, smtx_type_substitute_top_apply, native_ite, native_Teq]
  ...
  | refs, hNo, Term.Apply (Term.Apply (Term.UOp UserOp.Tuple) T) U, hValid => by
      ...
      have hWfRaw : __smtx_type_wf (SmtType.Datatype "@Tuple" d) = true := by
        simpa [hRaw] using hWf
      change
        smtx_type_substitute_top_apply "@Tuple" base
            (native_ite
              (__smtx_type_wf
                (__eo_to_smt_type_tuple (__eo_to_smt_type T) (__eo_to_smt_type U)))
              (__eo_to_smt_type_tuple (__eo_to_smt_type T) (__eo_to_smt_type U))
          ...
          native_ite
            ...
            (__eo_to_smt_type_tuple (__eo_to_smt_type T) (__eo_to_smt_type U))
            SmtType.None
      simp [native_ite, hWf, hRaw, hWfRaw, smtx_type_substitute_top_apply,
        native_streq]
  | refs, hNo, T, hValid => by
      ...
      | UOp op =>
          cases op <;> simp [eo_type_valid_rec, __eo_to_smt_type,
            smtx_type_substitute_top_apply, native_ite, native_Teq, native_streq] at hValid ⊢
      | DatatypeType s d =>
          rcases hValid with ⟨hReserved, hD⟩
          have hNe : s ≠ "@Tuple" := by
            intro hs
          ...
          | Apply g y =>
              ...
              | UOp op =>
                  ...
                  case Tuple =>
                    ...
                    rcases eo_to_smt_type_tuple_eq_tuple_datatype_of_wf hWf with ⟨d, hRaw⟩
                    have hWfRaw :
                        __smtx_type_wf (SmtType.Datatype "@Tuple" d) = true := by
                      simpa [hRaw] using hWf
                    change
                      smtx_type_substitute_top_apply "@Tuple" base
                          (native_ite
...
private theorem smtx_dtc_substitute_tuple_of_eo_datatype_cons_valid_rec
    (base : SmtDatatype) :
    {refs : RefList} -> "@Tuple" ∉ refs -> {c : DatatypeCons} ->
      eo_datatype_cons_valid_rec refs c ->
        __smtx_dtc_substitute "@Tuple" base (__eo_to_smt_datatype_cons c) =
...
private theorem smtx_dt_substitute_tuple_of_eo_datatype_valid_rec
    (base : SmtDatatype) :
    {refs : RefList} -> "@Tuple" ∉ refs -> {d : Datatype} ->
      ...
      | UnitTuple =>
          ...
          exact
            smtx_type_substitute_top_apply_tuple_of_eo_valid_rec
              ...
              (by simpa [eo_type_valid] using hValid)
      | RegLan =>
          simp [__eo_to_smt_type, smtx_type_substitute_top_apply, native_ite,
            native_Teq]
      | _ =>
          exfalso
      ...
      · rcases hUnit with ⟨hEq, hD⟩
        ...
        cases j <;>
          simp [__smtx_ret_typeof_sel_rec, __eo_list_nth_rec,
            __eo_to_smt_type, native_nat_to_int, SmtEval.native_nat_to_int]
      · rcases hCons with ⟨y, x0, c, hEq, hTail, hD⟩
        subst T
        subst d
        rcases (by simpa [eo_type_valid_rec] using hValid :
          eo_type_valid_rec [] y ∧ eo_type_valid_rec [] x0 ∧
            __smtx_type_wf
              (__eo_to_smt_type_tuple (__eo_to_smt_type y) (__eo_to_smt_type x0)) =
              true) with ⟨_hy, hx, _hWf⟩
        cases j with
        | zero =>
            simp [__smtx_ret_typeof_sel_rec, __eo_list_nth_rec,
        ...
        | succ j =>
            have hStep :
              ...
              simpa [native_zplus, SmtEval.native_zplus, native_nat_to_int,
                SmtEval.native_nat_to_int] using hInt
            have hNe :
                native_nat_to_int (native_nat_succ j) ≠ (0 : native_Int) := by
              have hInt : ((j : Int) + 1) ≠ 0 := by
                omega
              simpa [native_nat_to_int, SmtEval.native_nat_to_int] using hInt
            have hIH :=
              smtx_ret_typeof_tuple_sel_rec_eq_eo_list_nth_rec_nat
                (T := x0) (d := SmtDatatype.sum c SmtDatatype.null) j hTail hx
            have hNth :
                __eo_list_nth_rec
...
termination_by T d j hT hValid => T
...
theorem eo_type_valid_rec_of_tuple_smt_type
  ...
  cases T <;> try simpa [eo_type_valid] using hValid
  case UOp op =>
    cases op <;> try simpa [eo_type_valid] using hValid
    case RegLan =>
...
private theorem smtx_ret_typeof_tuple_sel_eq_eo_list_nth_rec_nat
    ...
    __smtx_ret_typeof_sel "@Tuple" d native_nat_zero j =
      __eo_to_smt_type (__eo_list_nth_rec T (Term.Numeral (native_nat_to_int j))) := by
  rw [__smtx_ret_typeof_sel]
  rw [smtx_ret_typeof_sel_rec_substitute_apply]
  rw [smtx_ret_typeof_tuple_sel_rec_eq_eo_list_nth_rec_nat j hT hValid]
  exact
    smtx_type_substitute_top_apply_tuple_of_eo_list_nth_rec_nat
      (T := T) (d := d) (base := d) j hT hValid

private def smtx_tuple_cons_type (T : SmtType) : SmtType -> SmtType
  | SmtType.DtcAppType A B => SmtType.DtcAppType A (smtx_tuple_cons_type T B)
  | U =>
      native_ite (__smtx_type_wf (__eo_to_smt_type_tuple T U))
        (__eo_to_smt_type_tuple T U) SmtType.None
...
private theorem smtx_tuple_cons_type_apply
    (T F X : SmtType)
    (hNN :
      __smtx_typeof_apply (smtx_tuple_cons_type T F) X ≠ SmtType.None) :
    __smtx_typeof_apply (smtx_tuple_cons_type T F) X =
      smtx_tuple_cons_type T (__smtx_typeof_apply F X) := by
  cases F with
  | DtcAppType A B =>
      cases A <;> cases X <;>
        simp_all [smtx_tuple_cons_type, __smtx_typeof_apply, __smtx_typeof_guard,
          native_ite, native_Teq]
  | Datatype s d =>
      exfalso
      apply hNN
      cases d with
      | null =>
          simp [smtx_tuple_cons_type, __eo_to_smt_type_tuple, __smtx_typeof_apply,
            native_ite]
      | sum c rest =>
          cases rest with
          | null =>
              ...
              · subst s
                cases hComp :
                    native_and (native_inhabited_type T)
                      (__smtx_type_wf_rec T native_reflist_nil)
                · simp [smtx_tuple_cons_type, __eo_to_smt_type_tuple,
                    __smtx_typeof_apply, native_ite, __smtx_type_wf_component,
                    hComp]
                · cases hWf :
                      __smtx_type_wf
                        (SmtType.Datatype "@Tuple"
                          (SmtDatatype.sum (SmtDatatypeCons.cons T c)
                            SmtDatatype.null)) <;>
                    simp [smtx_tuple_cons_type, __eo_to_smt_type_tuple,
                      __smtx_typeof_apply, native_ite, __smtx_type_wf_component,
                      hComp, hWf]
              · simp [smtx_tuple_cons_type, __eo_to_smt_type_tuple,
                  __smtx_typeof_apply, native_ite, hs]
          | sum cTail restTail =>
              simp [smtx_tuple_cons_type, __eo_to_smt_type_tuple,
                __smtx_typeof_apply, native_ite]
  | _ =>
      exfalso
      apply hNN
      simp [smtx_tuple_cons_type, __eo_to_smt_type_tuple, __smtx_typeof_apply,
...
private theorem smtx_apply_arg_non_none_of_non_none_tuple_cons
    (f x : SmtTerm)
    (hSel : ∀ s d i j, f ≠ SmtTerm.DtSel s d i j)
    (hTester : ∀ s d i, f ≠ SmtTerm.DtTester s d i)
...
private theorem smtx_apply_head_non_none_of_non_none_tuple_cons
    (f x : SmtTerm)
    (hSel : ∀ s d i j, f ≠ SmtTerm.DtSel s d i j)
    (hTester : ∀ s d i, f ≠ SmtTerm.DtTester s d i)
    (hNN : __smtx_typeof (SmtTerm.Apply f x) ≠ SmtType.None) :
...
private theorem smtx_tuple_cons_apply_head_tuple_of_non_none :
    ∀ (tail head : SmtTerm) (T : SmtType),
      __smtx_typeof (eo_to_smt_tuple_cons_tail tail T head) ≠ SmtType.None ->
        ∃ c,
          smtx_apply_head tail =
            SmtTerm.DtCons "@Tuple" (SmtDatatype.sum c SmtDatatype.null)
              native_nat_zero
  | tail, head, T, hNN => by
      cases tail with
      | Apply f a =>
          have hRecNN :
              __smtx_typeof (eo_to_smt_tuple_cons_tail f T head) ≠ SmtType.None :=
            smtx_apply_head_non_none_of_non_none_tuple_cons
              (eo_to_smt_tuple_cons_tail f T head) a
              (eo_to_smt_tuple_cons_ne_dt_sel f T head)
              (eo_to_smt_tuple_cons_ne_dt_tester f T head)
              (by simpa [eo_to_smt_tuple_cons_tail] using hNN)
          exact smtx_tuple_cons_apply_head_tuple_of_non_none f head T hRecNN
      | DtCons s d i =>
          by_cases hs : s = "@Tuple"
          · subst s
            cases d with
            | null =>
                exfalso
                apply hNN
                cases i <;> simp [eo_to_smt_tuple_cons_tail]
            | sum c rest =>
                cases rest with
                | null =>
                    cases i with
                    ...
                    | succ i =>
                        exfalso
                        apply hNN
                        simp [eo_to_smt_tuple_cons_tail]
                | sum cTail restTail =>
                    exfalso
                    apply hNN
                    cases i <;> simp [eo_to_smt_tuple_cons_tail]
          · exfalso
            apply hNN
            cases d <;> cases i <;> simp [eo_to_smt_tuple_cons_tail, hs]
      | _ =>
          exfalso
          apply hNN
          simp [eo_to_smt_tuple_cons_tail]
termination_by tail head T hNN => tail

private theorem smtx_tuple_cons_num_apply_args_of_non_none :
    ∀ (tail head : SmtTerm) (T : SmtType),
      __smtx_typeof (eo_to_smt_tuple_cons_tail tail T head) ≠ SmtType.None ->
        smtx_num_apply_args (eo_to_smt_tuple_cons_tail tail T head) =
          Nat.succ (smtx_num_apply_args tail)
  | tail, head, T, hNN => by
      cases tail with
      | Apply f a =>
          have hRecNN :
          ...
          · subst s
            ...
            | sum c rest =>
                ...
                | null =>
                    cases i with
                    | zero =>
                        simp [eo_to_smt_tuple_cons_tail, smtx_num_apply_args]
                    | succ i =>
                        exfalso
                        apply hNN
                        simp [eo_to_smt_tuple_cons_tail]
...
private theorem smtx_tuple_cons_result_apply_head_tuple_of_non_none :
    ∀ (tail head : SmtTerm) (T : SmtType),
      __smtx_typeof (eo_to_smt_tuple_cons_tail tail T head) ≠ SmtType.None ->
        ∃ c,
          smtx_apply_head (eo_to_smt_tuple_cons_tail tail T head) =
            SmtTerm.DtCons "@Tuple"
              (SmtDatatype.sum (SmtDatatypeCons.cons T c) SmtDatatype.null)
              native_nat_zero
  | tail, head, T, hNN => by
      ...
      | Apply f a =>
          have hRecNN :
            ...
            smtx_apply_head_non_none_of_non_none_tuple_cons
              ...
              (eo_to_smt_tuple_cons_ne_dt_tester f T head)
              (by simpa [eo_to_smt_tuple_cons_tail] using hNN)
          rcases smtx_tuple_cons_result_apply_head_tuple_of_non_none f head T hRecNN with
            ⟨c, hHead⟩
          exact ⟨c, by simpa [eo_to_smt_tuple_cons_tail, smtx_apply_head] using hHead⟩
      | DtCons s d i =>
          by_cases hs : s = "@Tuple"
          · subst s
            cases d with
            ...
            | sum c rest =>
                ...
                | null =>
                    cases i with
                    | zero =>
                        exact ⟨c, by simp [eo_to_smt_tuple_cons_tail, smtx_apply_head]⟩
                    | succ i =>
                        exfalso
                        apply hNN
                        simp [eo_to_smt_tuple_cons_tail]
...
private theorem smtx_dt_cons_chain_type_of_non_none_aux :
    ∀ (n : Nat) (t : SmtTerm) (s : native_String) (d : SmtDatatype) (i : native_Nat),
  ...
  | succ n ih =>
      ...
      have hlt :
          smtx_num_apply_args f < __smtx_dt_num_sels (__smtx_dt_substitute s d d) i := by
        rcases hHeadTy with hHeadTy | hHeadTy
      ...
      have hStep :
        ...
        simpa [R, Rest] using
          dt_cons_applied_type_rec_step s d (__smtx_dt_substitute s d d) i
...
private theorem smtx_term_datatype_type_head_base_apply
    {t : SmtTerm}
    {s sh : native_String}
    {d dh : SmtDatatype}
    {i : native_Nat}
    (hHead : smtx_apply_head t = SmtTerm.DtCons sh dh i)
  ...
  have hle :
      smtx_num_apply_args t ≤
        __smtx_dt_num_sels (__smtx_dt_substitute sh dh dh) i :=
    dt_cons_applied_type_rec_non_none_implies_le sh dh
      (__smtx_dt_substitute sh dh dh) i (smtx_num_apply_args t)
      (by rw [hRes]; simp)
  have hCount :
      smtx_num_apply_args t =
        __smtx_dt_num_sels (__smtx_dt_substitute sh dh dh) i := by
...
private theorem smtx_num_apply_args_eq_dt_num_sels_of_datatype
    {t : SmtTerm}
    {s : native_String}
    {d : SmtDatatype}
    {i : native_Nat}
  ...
  have hle :
    ...
    dt_cons_applied_type_rec_non_none_implies_le s d
      (__smtx_dt_substitute s d d) i (smtx_num_apply_args t)
      (by rw [hRes]; simp)
  have hge :
      __smtx_dt_num_sels (__smtx_dt_substitute s d d) i ≤
        smtx_num_apply_args t :=
...
private theorem smtx_tuple_cons_result_apply_head_of_tail_head :
      ...
      smtx_apply_head tail =
        ...
        smtx_apply_head (eo_to_smt_tuple_cons_tail tail T head) =
          SmtTerm.DtCons "@Tuple"
            (SmtDatatype.sum (SmtDatatypeCons.cons T c) SmtDatatype.null)
            native_nat_zero
  | SmtTerm.Apply f a, head, T, c, hHead => by
      have hHeadF :
          smtx_apply_head f =
            SmtTerm.DtCons "@Tuple" (SmtDatatype.sum c SmtDatatype.null)
              native_nat_zero := by
        simpa [smtx_apply_head] using hHead
      simpa [eo_to_smt_tuple_cons_tail, smtx_apply_head] using
        smtx_tuple_cons_result_apply_head_of_tail_head f head T c hHeadF
  | SmtTerm.DtCons s d i, head, T, c, hHead => by
      simp [smtx_apply_head] at hHead
      rcases hHead with ⟨rfl, hRest⟩
      rcases hRest with ⟨rfl, rfl⟩
      simp [eo_to_smt_tuple_cons_tail, smtx_apply_head]
  | tail, head, T, c, hHead => by
      cases tail with
      | Apply f a =>
          have hHeadF :
              smtx_apply_head f =
      ...
      | _ =>
          simp [smtx_apply_head] at hHead
termination_by tail head T c hHead => tail

private theorem smtx_tuple_cons_num_apply_args_of_tail_head :
    ∀ (tail head : SmtTerm) (T : SmtType) (c : SmtDatatypeCons),
      smtx_apply_head tail =
          SmtTerm.DtCons "@Tuple" (SmtDatatype.sum c SmtDatatype.null)
            native_nat_zero ->
      ...
      have hHeadF :
          smtx_apply_head f =
            SmtTerm.DtCons "@Tuple" (SmtDatatype.sum c SmtDatatype.null)
              native_nat_zero := by
        simpa [smtx_apply_head] using hHead
      simpa [eo_to_smt_tuple_cons_tail, smtx_num_apply_args] using
        smtx_tuple_cons_num_apply_args_of_tail_head f head T c hHeadF
  | SmtTerm.DtCons s d i, head, T, c, hHead => by
      simp [smtx_apply_head] at hHead
      rcases hHead with ⟨rfl, hRest⟩
      rcases hRest with ⟨rfl, rfl⟩
...
private theorem smtx_tuple_cons_tail_head_of_result_head :
    ∀ (tail head : SmtTerm) (T : SmtType) (c : SmtDatatypeCons),
      smtx_apply_head (eo_to_smt_tuple_cons_tail tail T head) =
          SmtTerm.DtCons "@Tuple" (SmtDatatype.sum c SmtDatatype.null)
            native_nat_zero ->
        ∃ cTail,
          c = SmtDatatypeCons.cons T cTail ∧
            smtx_apply_head tail =
              SmtTerm.DtCons "@Tuple" (SmtDatatype.sum cTail SmtDatatype.null)
                native_nat_zero
  | SmtTerm.Apply f a, head, T, c, hHead => by
      have hHeadF :
          smtx_apply_head (eo_to_smt_tuple_cons_tail f T head) =
            SmtTerm.DtCons "@Tuple" (SmtDatatype.sum c SmtDatatype.null)
              native_nat_zero := by
        simpa [eo_to_smt_tuple_cons_tail, smtx_apply_head] using hHead
      rcases smtx_tuple_cons_tail_head_of_result_head f head T c hHeadF with
        ⟨cTail, hC, hTail⟩
      exact ⟨cTail, hC, by simpa [smtx_apply_head] using hTail⟩
  | SmtTerm.DtCons s d i, head, T, c, hHead => by
      by_cases hs : s = "@Tuple"
      · subst s
        cases d with
        | null =>
            cases i <;> simp [eo_to_smt_tuple_cons_tail, smtx_apply_head] at hHead
        | sum cTail rest =>
            cases rest with
            | null =>
                cases i with
                | zero =>
                    have hC' : SmtDatatypeCons.cons T cTail = c := by
                      simpa [eo_to_smt_tuple_cons_tail, smtx_apply_head] using
                        congrArg
                          (fun t =>
                            match t with
                            | SmtTerm.DtCons _ (SmtDatatype.sum c' SmtDatatype.null) _ => c'
                            | _ => c)
                          hHead
                    exact ⟨cTail, hC'.symm, by simp [smtx_apply_head]⟩
                | succ i =>
                    simp [eo_to_smt_tuple_cons_tail, smtx_apply_head] at hHead
            | sum cRest restTail =>
                cases i <;> simp [eo_to_smt_tuple_cons_tail, smtx_apply_head] at hHead
      · cases d <;> cases i <;>
          simp [eo_to_smt_tuple_cons_tail, smtx_apply_head, hs] at hHead
  | tail, head, T, c, hHead => by
      cases tail with
      | Apply f a =>
          have hHeadF :
...
private theorem smtx_tuple_cons_result_args_ge_sels_of_tail_lower
    (tail head : SmtTerm) (T : SmtType) (c : SmtDatatypeCons)
    (hHead :
      smtx_apply_head (eo_to_smt_tuple_cons_tail tail T head) =
        SmtTerm.DtCons "@Tuple" (SmtDatatype.sum c SmtDatatype.null)
    ...
    (hTailLower :
      ∀ cTail : SmtDatatypeCons,
        smtx_apply_head tail =
            SmtTerm.DtCons "@Tuple" (SmtDatatype.sum cTail SmtDatatype.null)
              native_nat_zero ->
          __smtx_dt_num_sels (SmtDatatype.sum cTail SmtDatatype.null)
              native_nat_zero ≤
            smtx_num_apply_args tail) :
    __smtx_dt_num_sels (SmtDatatype.sum c SmtDatatype.null)
        native_nat_zero ≤
      smtx_num_apply_args (eo_to_smt_tuple_cons_tail tail T head) := by
  rcases smtx_tuple_cons_tail_head_of_result_head tail head T c hHead with
    ⟨cTail, hC, hTailHead⟩
  subst c
  have hArgs :=
    smtx_tuple_cons_num_apply_args_of_tail_head tail head T cTail hTailHead
  rw [hArgs]
  exact Nat.succ_le_succ (hTailLower cTail hTailHead)

private theorem smtx_tuple_cons_tail_args_le_sels_of_non_none
    (tail head : SmtTerm) (T : SmtType) (c : SmtDatatypeCons)
    (hTailHead :
      smtx_apply_head tail =
        SmtTerm.DtCons "@Tuple" (SmtDatatype.sum c SmtDatatype.null)
    ...
    (hNN :
      __smtx_typeof (eo_to_smt_tuple_cons_tail tail T head) ≠ SmtType.None) :
    smtx_num_apply_args tail ≤
      __smtx_dt_num_sels (SmtDatatype.sum c SmtDatatype.null) native_nat_zero := by
  let tailD := SmtDatatype.sum c SmtDatatype.null
  let fullD := SmtDatatype.sum (SmtDatatypeCons.cons T c) SmtDatatype.null
  have hResultHead :
      smtx_apply_head (eo_to_smt_tuple_cons_tail tail T head) =
        SmtTerm.DtCons "@Tuple" fullD native_nat_zero := by
    simpa [tailD, fullD] using
      smtx_tuple_cons_result_apply_head_of_tail_head tail head T c hTailHead
  have hResultChain :=
    smtx_dt_cons_chain_type_of_non_none hResultHead hNN
  have hArgs :=
    smtx_tuple_cons_num_apply_args_of_tail_head tail head T c hTailHead
  have hResultLe :
      smtx_num_apply_args (eo_to_smt_tuple_cons_tail tail T head) ≤
        __smtx_dt_num_sels (__smtx_dt_substitute "@Tuple" fullD fullD)
          native_nat_zero :=
    dt_cons_applied_type_rec_non_none_implies_le "@Tuple" fullD
      (__smtx_dt_substitute "@Tuple" fullD fullD) native_nat_zero
      (smtx_num_apply_args (eo_to_smt_tuple_cons_tail tail T head))
      (by
        rw [← hResultChain]
        exact hNN)
  have hFullSels :
    ...
    dt_num_sels_substitute "@Tuple" fullD fullD native_nat_zero
  rw [hArgs, hFullSels] at hResultLe
  have hSucc :
      Nat.succ (smtx_num_apply_args tail) ≤
        Nat.succ (__smtx_dt_num_sels tailD native_nat_zero) := by
    simpa [tailD, fullD, __smtx_dt_num_sels, __smtx_dtc_num_sels] using hResultLe
  exact Nat.le_of_succ_le_succ hSucc

private theorem smtx_tuple_tail_type_of_head_bounds_non_none
    (tail : SmtTerm) (c : SmtDatatypeCons)
    (hTailHead :
      smtx_apply_head tail =
        SmtTerm.DtCons "@Tuple" (SmtDatatype.sum c SmtDatatype.null)
          native_nat_zero)
    ...
    (hTailUpper :
      smtx_num_apply_args tail ≤
        __smtx_dt_num_sels (SmtDatatype.sum c SmtDatatype.null)
          native_nat_zero)
    (hTailNN : __smtx_typeof tail ≠ SmtType.None) :
    __smtx_typeof tail =
  ...
  have hCountPlain :
    ...
    Nat.le_antisymm hTailUpper hTailLower
  have hCount :
      smtx_num_apply_args tail =
        __smtx_dt_num_sels (__smtx_dt_substitute "@Tuple" tailD tailD)
          native_nat_zero := by
    rw [hCountPlain]
    exact (dt_num_sels_substitute "@Tuple" tailD tailD native_nat_zero).symm
  exact
    smtx_term_typeof_full_dt_cons_chain_apply
      (t := tail) (s := "@Tuple") (d := tailD) (i := native_nat_zero)
      (by simpa [tailD] using hTailHead) hCount hTailNN

private theorem eo_to_smt_tuple_unit_head_args_ge_sels
    (c : SmtDatatypeCons)
    (hHead :
      smtx_apply_head (__eo_to_smt (Term.UOp UserOp.tuple_unit)) =
          SmtTerm.DtCons "@Tuple" (SmtDatatype.sum c SmtDatatype.null)
            native_nat_zero) :
    __smtx_dt_num_sels (SmtDatatype.sum c SmtDatatype.null)
        native_nat_zero ≤
      smtx_num_apply_args (__eo_to_smt (Term.UOp UserOp.tuple_unit)) := by
  simp [eo_to_smt_term_tuple_unit, smtx_apply_head, smtx_num_apply_args] at hHead ⊢
  subst c
  simp [__smtx_dt_num_sels, __smtx_dtc_num_sels]

def eo_type_tuple_subst_stable (U : Term) : Prop :=
  ∀ base : SmtDatatype,
    smtx_type_substitute_top_apply "@Tuple" base (__eo_to_smt_type U) =
      __eo_to_smt_type U

theorem eo_type_tuple_subst_stable_of_valid
    {U : Term}
    (hValid : eo_type_valid U) :
    eo_type_tuple_subst_stable U := by
  intro base
  exact smtx_type_substitute_top_apply_tuple_of_eo_valid base hValid

def eo_to_smt_tuple_tail_recoverable (x : Term) : Prop :=
  ∀ (head : SmtTerm) (U : Term),
    eo_type_tuple_subst_stable U ->
      __smtx_typeof
          (eo_to_smt_tuple_cons_tail (__eo_to_smt x) (__eo_to_smt_type U) head) ≠
        SmtType.None ->
        ∃ c,
          __smtx_typeof (__eo_to_smt x) =
            SmtType.Datatype "@Tuple" (SmtDatatype.sum c SmtDatatype.null)

private theorem smtx_typeof_tuple_cons_tail_none
    (head : SmtTerm) (T : SmtType) :
    __smtx_typeof (eo_to_smt_tuple_cons_tail SmtTerm.None T head) =
      SmtType.None := by
  simp [eo_to_smt_tuple_cons_tail]

theorem eo_to_smt_tuple_tail_recoverable_of_none
    {x : Term}
    (h : __eo_to_smt x = SmtTerm.None) :
...
theorem eo_to_smt_tuple_tail_recoverable_of_tuple_cons_none
    {x : Term}
    (h :
      ∀ (head : SmtTerm) (T : SmtType),
        eo_to_smt_tuple_cons_tail (__eo_to_smt x) T head = SmtTerm.None) :
    eo_to_smt_tuple_tail_recoverable x := by
  intro head U _hStable hNN
  exfalso
  apply hNN
...
theorem eo_to_smt_tuple_tail_recoverable_of_tuple_cons_typeof_none
    ...
    (h :
      ∀ (head : SmtTerm) (T : SmtType),
        __smtx_typeof
            (eo_to_smt_tuple_cons_tail (__eo_to_smt x) T head) =
          SmtType.None) :
    eo_to_smt_tuple_tail_recoverable x := by
  intro head U _hStable hNN
...
theorem eo_to_smt_tuple_tail_recoverable_of_checked_tuple_cons
    ...
    (h :
      __eo_to_smt x = eo_to_smt_tuple_cons_checked_tail tail T head0) :
    eo_to_smt_tuple_tail_recoverable x := by
  intro head U _hStable hNN
  rw [h] at hNN ⊢
  unfold eo_to_smt_tuple_cons_checked_tail at hNN ⊢
  have hRawTypeEq :
      __smtx_typeof (_root_.__eo_to_smt_tuple_cons head0 T tail) =
        __smtx_typeof (eo_to_smt_tuple_cons_tail tail T head0) := by
    rfl
  have hRawTermEq :
      _root_.__eo_to_smt_tuple_cons head0 T tail =
        eo_to_smt_tuple_cons_tail tail T head0 := by
    rfl
      ...
      · subst s
        ...
        | null =>
            exact False.elim (hNN (by
              simp [hTail, eo_to_smt_tuple_cons_tail]))
        | sum c rest =>
            cases rest with
            | null =>
                cases hRaw :
                    __smtx_typeof (eo_to_smt_tuple_cons_tail tail T head0) with
                  | Datatype sRaw dRaw =>
                      ...
                      · subst sRaw
                        ...
                        | null =>
                            exact False.elim (hNN (by
                              simp [hTail, hRawTypeEq, hRawTermEq, hRaw,
                                _root_.__eo_to_smt_tuple_cons,
                                smtx_typeof_tuple_cons_tail_none]))
                        | sum cRaw restRaw =>
                            cases restRaw with
                            | null =>
                            ...
                            | sum cRest dRest =>
                                exact False.elim (hNN (by
                                  simp [hTail, hRawTypeEq, hRawTermEq, hRaw,
                                    ...
                                    smtx_typeof_tuple_cons_tail_none]))
                      · exact False.elim (hNN (by
                          simp [hTail, hRawTypeEq, hRawTermEq, hRaw, hRawName,
                            _root_.__eo_to_smt_tuple_cons,
                            smtx_typeof_tuple_cons_tail_none]))
                  | _ =>
                      exact False.elim (hNN (by
            ...
            | sum _ _ =>
                exact False.elim (hNN (by
                  simp [hTail, eo_to_smt_tuple_cons_tail]))
      · exact False.elim (hNN (by
          simp [hTail, hs, eo_to_smt_tuple_cons_tail]))
  | _ =>
      exact False.elim (hNN (by
        simp [hTail, eo_to_smt_tuple_cons_tail]))
...
private theorem smtx_tuple_cons_checked_raw_non_none_of_non_none
    (tail head : SmtTerm) (T : SmtType)
    (hNN :
      __smtx_typeof (eo_to_smt_tuple_cons_checked_tail tail T head) ≠
        SmtType.None) :
    __smtx_typeof (eo_to_smt_tuple_cons_tail tail T head) ≠ SmtType.None := by
  unfold eo_to_smt_tuple_cons_checked_tail at hNN
  ...
  case Datatype s d =>
    ...
    · subst s
      ...
      | sum c rest =>
          cases rest with
          | null =>
              cases hRaw : __smtx_typeof (eo_to_smt_tuple_cons_tail tail T head) with
              | Datatype _ _ =>
                  simp
              | _ =>
                  exfalso
...
theorem eo_to_smt_tuple_tail_recoverable_tuple_unit :
  ...
  intro head U _hStable hNN
  refine ⟨SmtDatatypeCons.unit, ?_⟩
  rw [eo_to_smt_term_tuple_unit]
  exact smtx_typeof_tuple_unit_translation

theorem eo_to_smt_tuple_tail_recoverable_of_tuple_type
    {x : Term} {c : SmtDatatypeCons}
    (hTy :
      __smtx_typeof (__eo_to_smt x) =
        SmtType.Datatype "@Tuple" (SmtDatatype.sum c SmtDatatype.null)) :
    eo_to_smt_tuple_tail_recoverable x := by
  intro _head _U _hStable _hNN
  exact ⟨c, hTy⟩

private theorem smtx_tuple_cons_typeof_of_tail_tuple_type
    (tail head : SmtTerm) (T : SmtType) (c : SmtDatatypeCons)
    (hTailTy :
      __smtx_typeof tail =
        SmtType.Datatype "@Tuple" (SmtDatatype.sum c SmtDatatype.null))
    (hNN :
      __smtx_typeof (eo_to_smt_tuple_cons_tail tail T head) ≠ SmtType.None) :
    __smtx_typeof (eo_to_smt_tuple_cons_tail tail T head) =
      SmtType.Datatype "@Tuple"
        (SmtDatatype.sum (SmtDatatypeCons.cons T c) SmtDatatype.null) := by
  let tailD := SmtDatatype.sum c SmtDatatype.null
  let fullD := SmtDatatype.sum (SmtDatatypeCons.cons T c) SmtDatatype.null
  rcases smtx_tuple_cons_apply_head_tuple_of_non_none tail head T hNN with
    ⟨cHead, hTailHeadRaw⟩
  have hBase :=
    smtx_term_datatype_type_head_base_apply hTailHeadRaw (by
      simpa [tailD] using hTailTy)
  rcases hBase with ⟨_hName, hData⟩
  have hTailHead :
      smtx_apply_head tail =
        SmtTerm.DtCons "@Tuple" tailD native_nat_zero := by
    simpa [tailD, hData] using hTailHeadRaw
  have hResultHead :
      smtx_apply_head (eo_to_smt_tuple_cons_tail tail T head) =
        SmtTerm.DtCons "@Tuple" fullD native_nat_zero := by
    simpa [tailD, fullD] using
  ...
  have hTailCount :
      smtx_num_apply_args tail =
        __smtx_dt_num_sels (__smtx_dt_substitute "@Tuple" tailD tailD)
          native_nat_zero := by
    simpa [tailD] using
      smtx_num_apply_args_eq_dt_num_sels_of_datatype hTailHead
        (by simpa [tailD] using hTailTy)
  have hResultCount :
      smtx_num_apply_args (eo_to_smt_tuple_cons_tail tail T head) =
        __smtx_dt_num_sels (__smtx_dt_substitute "@Tuple" fullD fullD)
          native_nat_zero := by
    have hArgs := smtx_tuple_cons_num_apply_args_of_non_none tail head T hNN
    rw [hArgs, hTailCount]
    have hTailSels :
        __smtx_dt_num_sels (__smtx_dt_substitute "@Tuple" tailD tailD)
            native_nat_zero =
          __smtx_dt_num_sels tailD native_nat_zero := by
      exact dt_num_sels_substitute "@Tuple" tailD tailD native_nat_zero
    have hFullSels :
        __smtx_dt_num_sels (__smtx_dt_substitute "@Tuple" fullD fullD)
            native_nat_zero =
          __smtx_dt_num_sels fullD native_nat_zero := by
      exact dt_num_sels_substitute "@Tuple" fullD fullD native_nat_zero
    rw [hTailSels, hFullSels]
    simp [fullD, tailD, __smtx_dt_num_sels, __smtx_dtc_num_sels]
  have hChainNN :
      dt_cons_applied_type_rec "@Tuple" fullD
          (__smtx_dt_substitute "@Tuple" fullD fullD) native_nat_zero
          (__smtx_dt_num_sels (__smtx_dt_substitute "@Tuple" fullD fullD)
            native_nat_zero) ≠
        SmtType.None := by
  ...
  have hFull :
    ...
    dt_cons_applied_type_rec_full_arity "@Tuple" fullD
      (__smtx_dt_substitute "@Tuple" fullD fullD) native_nat_zero hChainNN
  calc
    __smtx_typeof (eo_to_smt_tuple_cons_tail tail T head) =
        dt_cons_applied_type_rec "@Tuple" fullD
          (__smtx_dt_substitute "@Tuple" fullD fullD) native_nat_zero
          (smtx_num_apply_args (eo_to_smt_tuple_cons_tail tail T head)) := hResultChain
    _ =
        dt_cons_applied_type_rec "@Tuple" fullD
          (__smtx_dt_substitute "@Tuple" fullD fullD) native_nat_zero
          (__smtx_dt_num_sels (__smtx_dt_substitute "@Tuple" fullD fullD)
            native_nat_zero) := by
      rw [hResultCount]
    _ = SmtType.Datatype "@Tuple" fullD := by
      simpa [fullD] using hFull

private theorem smtx_typeof_apply_datatype_head_eq_none
    (f x : SmtTerm) (s : native_String) (d : SmtDatatype)
    (hSel : ∀ s d i j, f ≠ SmtTerm.DtSel s d i j)
    (hTester : ∀ s d i, f ≠ SmtTerm.DtTester s d i)
    (hTy : __smtx_typeof f = SmtType.Datatype s d) :
    __smtx_typeof (SmtTerm.Apply f x) = SmtType.None := by
  cases f with
  | DtSel s0 d0 i0 j0 =>
      exact False.elim (hSel s0 d0 i0 j0 rfl)
  | DtTester s0 d0 i0 =>
...
theorem eo_to_smt_tuple_tail_recoverable_of_generic_apply
    ...
    (hTranslate :
      __eo_to_smt (Term.Apply f x) =
        SmtTerm.Apply (__eo_to_smt f) (__eo_to_smt x))
    (hFRec : eo_to_smt_tuple_tail_recoverable f) :
    eo_to_smt_tuple_tail_recoverable (Term.Apply f x) := by
  intro head U hStable hNN
  have hConsFNN :
      __smtx_typeof
          (eo_to_smt_tuple_cons_tail (__eo_to_smt f) (__eo_to_smt_type U) head) ≠
        SmtType.None :=
    smtx_apply_head_non_none_of_non_none_tuple_cons
      (eo_to_smt_tuple_cons_tail (__eo_to_smt f) (__eo_to_smt_type U) head)
      (__eo_to_smt x)
      (eo_to_smt_tuple_cons_ne_dt_sel (__eo_to_smt f) (__eo_to_smt_type U) head)
      (eo_to_smt_tuple_cons_ne_dt_tester (__eo_to_smt f) (__eo_to_smt_type U) head)
      (by simpa [hTranslate, eo_to_smt_tuple_cons_tail] using hNN)
  rcases hFRec head U hStable hConsFNN with ⟨c, hFTy⟩
  have hConsFTy :
      __smtx_typeof
          (eo_to_smt_tuple_cons_tail (__eo_to_smt f) (__eo_to_smt_type U) head) =
        SmtType.Datatype "@Tuple"
          (SmtDatatype.sum (SmtDatatypeCons.cons (__eo_to_smt_type U) c)
            SmtDatatype.null) :=
    smtx_tuple_cons_typeof_of_tail_tuple_type
      (__eo_to_smt f) head (__eo_to_smt_type U) c hFTy hConsFNN
  exfalso
  apply hNN
  have hApply :
      __smtx_typeof
          (SmtTerm.Apply
            ...
            (__eo_to_smt x)) =
        SmtType.None :=
    smtx_typeof_apply_datatype_head_eq_none
      (eo_to_smt_tuple_cons_tail (__eo_to_smt f) (__eo_to_smt_type U) head)
      (__eo_to_smt x) "@Tuple"
      (SmtDatatype.sum (SmtDatatypeCons.cons (__eo_to_smt_type U) c)
        SmtDatatype.null)
      ...
      (eo_to_smt_tuple_cons_ne_dt_tester (__eo_to_smt f) (__eo_to_smt_type U) head)
      hConsFTy
  simpa [hTranslate, eo_to_smt_tuple_cons_tail] using hApply

private theorem smtx_tuple_dt_cons_non_none_of_wf
    {c : SmtDatatypeCons}
    (hWf :
      __smtx_type_wf
          (SmtType.Datatype "@Tuple" (SmtDatatype.sum c SmtDatatype.null)) =
    ...
    __smtx_typeof
      ...
      SmtType.None := by
  let tailD := SmtDatatype.sum c SmtDatatype.null
  let tailTy := SmtType.Datatype "@Tuple" tailD
  let raw :=
    __smtx_typeof_dt_cons_rec tailTy
      (__smtx_dt_substitute "@Tuple" tailD tailD) native_nat_zero
  have hRawNN : raw ≠ SmtType.None := by
    cases c with
    | unit =>
        simp [raw, tailTy, tailD, __smtx_dt_substitute, __smtx_dtc_substitute,
          __smtx_typeof_dt_cons_rec, native_streq, native_ite, native_Teq]
    | cons U cTail =>
        cases U <;>
          simp [raw, tailTy, tailD, __smtx_dt_substitute, __smtx_dtc_substitute,
            __smtx_typeof_dt_cons_rec, native_streq, native_ite, native_Teq]
  rw [typeof_dt_cons_eq]
  simpa [tailTy, tailD, raw, __smtx_typeof_guard_wf, hWf, native_ite]
    using hRawNN

private theorem smtx_tuple_cons_tail_non_none_dt_cons
    (head : SmtTerm) (T : SmtType) (c : SmtDatatypeCons)
    (hNN :
      __smtx_typeof
          (eo_to_smt_tuple_cons_tail
            (SmtTerm.DtCons "@Tuple" (SmtDatatype.sum c SmtDatatype.null)
              native_nat_zero) T head) ≠
        SmtType.None) :
    __smtx_typeof
        (SmtTerm.DtCons "@Tuple" (SmtDatatype.sum c SmtDatatype.null)
          native_nat_zero) ≠
  ...
  let fullTy := SmtType.Datatype "@Tuple" fullD
  let raw :=
    __smtx_typeof_dt_cons_rec fullTy
      (__smtx_dt_substitute "@Tuple" fullD fullD) native_nat_zero
  have hCtorNN :
      __smtx_typeof (SmtTerm.DtCons "@Tuple" fullD native_nat_zero) ≠
        SmtType.None :=
    smtx_apply_head_non_none_of_non_none_tuple_cons
      (SmtTerm.DtCons "@Tuple" fullD native_nat_zero) head
      (by intro s d i j h; cases h)
      (by intro s d i h; cases h)
      (by simpa [eo_to_smt_tuple_cons_tail, fullD] using hNN)
  have hGuardNN :
      __smtx_typeof_guard_wf fullTy raw ≠ SmtType.None := by
    simpa [typeof_dt_cons_eq, fullTy, fullD, raw] using hCtorNN
  have hFullWf : __smtx_type_wf fullTy = true :=
    Smtm.smtx_typeof_guard_wf_wf_of_non_none fullTy raw hGuardNN
  have hTailWf :
      __smtx_type_wf
          (SmtType.Datatype "@Tuple" (SmtDatatype.sum c SmtDatatype.null)) =
        true := by
    exact smtx_tuple_type_wf_tail_of_cons_wf (T := T) (c := c)
      (by simpa [fullTy, fullD] using hFullWf)
  exact smtx_tuple_dt_cons_non_none_of_wf hTailWf

private theorem smtx_tuple_cons_type_none (T : SmtType) :
    smtx_tuple_cons_type T SmtType.None = SmtType.None := by
  simp [smtx_tuple_cons_type, __eo_to_smt_type_tuple, __smtx_type_wf,
    __smtx_type_wf_rec, native_and, native_ite]

private theorem smtx_tuple_cons_head_non_none_of_non_none :
    ∀ (tail head : SmtTerm) (T : SmtType),
      __smtx_typeof (eo_to_smt_tuple_cons_tail tail T head) ≠ SmtType.None ->
      ...
      | Apply f a =>
          have hRecNN :
            ...
            smtx_apply_head_non_none_of_non_none_tuple_cons
              ...
              (eo_to_smt_tuple_cons_ne_dt_tester f T head)
              (by simpa [eo_to_smt_tuple_cons_tail] using hNN)
          exact smtx_tuple_cons_head_non_none_of_non_none f head T hRecNN
      | DtCons s d i =>
          by_cases hs : s = "@Tuple"
          · subst s
            cases d with
            ...
            | sum c rest =>
                ...
                | null =>
                    ...
                    | zero =>
                        exact
                          smtx_apply_arg_non_none_of_non_none_tuple_cons
                            (SmtTerm.DtCons "@Tuple"
                              (SmtDatatype.sum (SmtDatatypeCons.cons T c) SmtDatatype.null)
                              native_nat_zero)
                            head
      ...
      · subst s
        ...
        | Numeral n =>
            ...
            · have hTranslate :
              ...
              have hArgTy :
                  __smtx_typeof (__eo_to_smt x) = SmtType.Datatype "@Tuple" d :=
                dt_sel_arg_datatype_of_non_none hApplyNN
              have hXNN : __smtx_typeof (__eo_to_smt x) ≠ SmtType.None := by
                rw [hArgTy]
                simp
              have hTyFromIH :
                  __eo_to_smt_type (__eo_typeof x) = SmtType.Datatype "@Tuple" d := by
              ...
              have hTupleWf :
                  __smtx_type_wf (SmtType.Datatype "@Tuple" d) = true :=
                Smtm.smt_datatype_wf_of_non_none_type
                  (__eo_to_smt x) "@Tuple" d hArgTy
              ...
              have hXValidRec : eo_type_valid_rec [] (__eo_typeof x) :=
                eo_type_valid_rec_of_tuple_smt_type hTyFromIH hXValidTop
              ...
              simpa [hNatInt] using
                smtx_ret_typeof_tuple_sel_eq_eo_list_nth_rec_nat
                  (T := __eo_typeof x) (d := d) (j := native_int_to_nat n)
                  hTyFromIH hXValidRec
        | _ =>
            exact eo_to_smt_typeof_matches_translation_of_smt_none
              (Term.Apply (Term.UOp1 UserOp1.tuple_select y) x)
              (by
...
private theorem eo_to_smt_typeof_matches_translation_apply_tuple_of_tail_type
  ...
  have hCheckedNN :
      __smtx_typeof
          (eo_to_smt_tuple_cons_checked_tail (__eo_to_smt x) headTy (__eo_to_smt y)) ≠
        SmtType.None := by
    change
      __smtx_typeof
          (eo_to_smt_tuple_cons_checked_tail (__eo_to_smt x) headTy (__eo_to_smt y)) ≠
    ...
    exact hNonNone
  have hTupleNN :
      __smtx_typeof (eo_to_smt_tuple_cons_tail (__eo_to_smt x) headTy (__eo_to_smt y)) ≠
        SmtType.None :=
  ...
  have hChecked :
      __eo_to_smt (Term.Apply (Term.Apply (Term.UOp UserOp.tuple) y) x) =
        eo_to_smt_tuple_cons_tail (__eo_to_smt x) headTy (__eo_to_smt y) := by
    have hTerm :
        __eo_to_smt (Term.Apply (Term.Apply (Term.UOp UserOp.tuple) y) x) =
          eo_to_smt_tuple_cons_checked_tail (__eo_to_smt x) headTy (__eo_to_smt y) := by
      change
        eo_to_smt_tuple_cons_checked_tail (__eo_to_smt x) headTy (__eo_to_smt y) =
          eo_to_smt_tuple_cons_checked_tail (__eo_to_smt x) headTy (__eo_to_smt y)
      rfl
    have hCheckedRaw :
        eo_to_smt_tuple_cons_checked_tail (__eo_to_smt x) headTy (__eo_to_smt y) =
          eo_to_smt_tuple_cons_tail (__eo_to_smt x) headTy (__eo_to_smt y) := by
      unfold eo_to_smt_tuple_cons_checked_tail
      simp [hTailTy, hRawSmt, fullD]
    exact hTerm.trans hCheckedRaw
  have hSmt :
      __smtx_typeof (__eo_to_smt (Term.Apply (Term.Apply (Term.UOp UserOp.tuple) y) x)) =
        SmtType.Datatype "@Tuple" fullD := by
    rw [hChecked]
  ...
  have hXTailType :
      __eo_to_smt_type (__eo_typeof x) =
        SmtType.Datatype "@Tuple" tailD := by
    exact hXEq.symm.trans (by simpa [tailD] using hTailTy)
  ...
  have hRaw :
      __eo_to_smt_type_tuple headTy (__eo_to_smt_type (__eo_typeof x)) =
        SmtType.Datatype "@Tuple" fullD := by
    have hFullWfFromRaw : __smtx_type_wf (SmtType.Datatype "@Tuple" fullD) = true :=
      Smtm.smt_datatype_wf_of_non_none_type
  ...
  have hFullWf : __smtx_type_wf (SmtType.Datatype "@Tuple" fullD) = true :=
    Smtm.smt_datatype_wf_of_non_none_type
      (__eo_to_smt (Term.Apply (Term.Apply (Term.UOp UserOp.tuple) y) x))
      "@Tuple" fullD hSmt
  have hRawWf :
  ...
  have hEo :
    ...
    simp [__eo_requires, hXTailLenOk, hYNotStuck, hXNotStuck, headTy, hRaw,
  ...
  have hCheckedNN :
    ...
    change
      __smtx_typeof
        ...
        SmtType.None at hNonNone
    exact hNonNone
  unfold eo_to_smt_tuple_cons_checked_tail at hCheckedNN
  ...
  have hEo :
    ...
    | DtcAppType A B =>
        ...
        have hYValidDtc : eo_type_valid_rec [] (Term.DtcAppType Y1 Y2) := by
          ...
          simpa [eo_type_valid] using hYValidTop'
        have hYDtcTrans :
  ...
  case set_is_empty =>
    exact eo_to_smt_typeof_matches_translation_apply_apply_generic_of_head_from_ih
      ...
      (let _v0 := __eo_to_smt y
       SmtTerm.eq _v0 (SmtTerm.set_empty (__smtx_typeof _v0))) ihFAll ihXAll (by rfl)
      (by rfl) (by rfl)
      (by intro s d i j h; cases h)
      (by intro s d i h; cases h)
      hNonNone
...
private theorem eo_to_smt_typeof_matches_translation_apply_apply_apply_bvite_from_valid_ih
  ...
  have hzEo :
    ...
    simpa [native_nat_to_int, SmtEval.native_nat_to_int] using hzEo'
  have hYTrans : __eo_to_smt_type (__eo_typeof y) = T := by
    rw [← (ihY hYNN).1, hY]
  have hXTrans : __eo_to_smt_type (__eo_typeof x) = T := by
    rw [← ihX hXNN, hX]
  ...
  | succ k ih =>
      have hRecNN :
        ...
        smtx_apply_head_non_none_of_non_none
          ...
          (by
            simpa [__eo_to_smt_updater_rec] using hNN)
      exact ih acc hAccSel hAccTester hRecNN
...
private theorem eo_to_smt_updater_rec_type_of_non_none
    ...
    __smtx_typeof
        (__eo_to_smt_updater_rec (SmtTerm.DtSel s d i j) n t u
          (SmtTerm.DtCons s d i)) =
      dt_cons_applied_type_rec s d (__smtx_dt_substitute s d d) i n := by
  induction n with
  | zero =>
      let raw :=
        __smtx_typeof_dt_cons_rec (SmtType.Datatype s d)
  ...
  | succ k ih =>
      ...
      have hRecTy :
          __smtx_typeof recTerm =
            dt_cons_applied_type_rec s d (__smtx_dt_substitute s d d) i k := by
        simpa [recTerm] using ih hRecNN
      rcases typeof_apply_non_none_cases hApplyNN with
        ⟨A, B, hHead, hArg, hA, _hB⟩
      have hlt :
          k < __smtx_dt_num_sels (__smtx_dt_substitute s d d) i := by
        rcases hHead with hHead | hHead
        ...
        · have hArgs := congrArg dt_cons_type_num_args hHead
          ...
          simp [dt_cons_type_num_args] at hArgs
          omega
      let R := __smtx_ret_typeof_sel_rec (__smtx_dt_substitute s d d) i k
      let Rest :=
        dt_cons_applied_type_rec s d (__smtx_dt_substitute s d d) i (Nat.succ k)
      have hStep :
          dt_cons_applied_type_rec s d (__smtx_dt_substitute s d d) i k =
            SmtType.DtcAppType R Rest := by
        simpa [R, Rest] using
          dt_cons_applied_type_rec_step s d (__smtx_dt_substitute s d d) i k hlt
      ...
      · subst s
        ...
        have hXRetSmt :
          ...
          eo_to_smt_updater_rec_update_arg_type_of_non_none
            ...
            (__smtx_dt_num_sels d native_nat_zero) (__eo_to_smt y) (__eo_to_smt x)
            hIdx hRecNN
        have hXNN : __smtx_typeof (__eo_to_smt x) ≠ SmtType.None := by
          exact eo_to_smt_updater_rec_update_arg_non_none_of_non_none
            "@Tuple" d native_nat_zero (native_int_to_nat n)
        ...
        have hTupleWf :
            __smtx_type_wf (SmtType.Datatype "@Tuple" d) = true :=
          Smtm.smt_datatype_wf_of_non_none_type
            (__eo_to_smt y) "@Tuple" d hYTy
        have hYValidTop : eo_type_valid (__eo_typeof y) :=
          eo_type_valid_of_smt_wf (__eo_typeof y) (by
            simpa [hTy] using hTupleWf)
        have hYValidRec : eo_type_valid_rec [] (__eo_typeof y) :=
          eo_type_valid_rec_of_tuple_smt_type hTy hYValidTop
        have hList :
            __eo_is_list (Term.UOp UserOp.Tuple) (__eo_typeof y) =
              Term.Boolean true :=
          eo_tuple_is_list_true_of_smt_tuple_type hTy
        have hnNonneg : (0 : Int) ≤ n := by
        ...
        have hRetNth :
          ...
          simpa [hNatInt] using
            smtx_ret_typeof_tuple_sel_eq_eo_list_nth_rec_nat
              (T := __eo_typeof y) (d := d) (j := native_int_to_nat n)
              hTy hYValidRec
        have hIdxNat :
        ...
        have hNthNN :
          ...
          rw [← hRetNth]
          exact hRetNN
        have hEo : __eo_to_smt_type (__eo_typeof t) = SmtType.Datatype "@Tuple" d := by
          have hCore :=
...
private theorem eo_to_smt_typeof_matches_translation_apply_binary_application_head_obligation
  ...
  case UOp op =>
    ...
    case multmult =>
      exact eo_to_smt_typeof_matches_translation_apply_apply_apply_generic_non_special_head
        UserOp.multmult z y x ihFAll ihXAll
    ...
    case multmult_total =>
      exact eo_to_smt_typeof_matches_translation_apply_apply_apply_generic_non_special_head
        UserOp.multmult_total z y x ihFAll ihXAll
...
private theorem eo_to_smt_typeof_matches_translation_apply_apply_head
  ...
  case Apply f z =>
    exact eo_to_smt_typeof_matches_translation_apply_binary_application_head f z y x ihFAll
      (ihFArgAll f z rfl) ihYAll ihXAll hNonNone
  case FunType =>
    have hTranslate :
    ...
    case _at_witness_string_length =>
      ...
      exact eo_to_smt_typeof_matches_translation_apply_generic_from_ih_of_valid
        ...
        (generic_apply_type_of_non_special_head _ _
          (by
            intro s d i j h
            change native_ite (native_teq (__eo_typeof w) (Term.UOp UserOp.Int))
                (SmtTerm.choice_nth "@x" (__eo_to_smt_type y)
                  (SmtTerm.eq
                    (SmtTerm.str_len (SmtTerm.Var "@x" (__eo_to_smt_type y)))
                    (__eo_to_smt z)) 0) SmtTerm.None =
              SmtTerm.DtSel s d i j at h
            unfold native_ite at h
            split at h <;> cases h)
          (by

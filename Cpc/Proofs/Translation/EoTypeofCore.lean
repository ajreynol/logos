...
private theorem native_eo_to_smt_guard_closed_ne_numeral
    (x : Term) (body : SmtTerm) (n : native_Int) :
    body ≠ SmtTerm.Numeral n ->
    native_eo_to_smt_guard_closed x body ≠ SmtTerm.Numeral n := by
  intro hBody h
  unfold native_eo_to_smt_guard_closed __eo_to_smt_guard_closed at h
  cases hc : native_eo_to_smt_closed x <;>
    simp [native_ite, hc] at h
  exact hBody h
...
private theorem eo_to_smt_qs_choice_ne_numeral
    (t : SmtTerm) (k : native_Nat) (n : native_Int) :
    __eo_to_smt_qs_choice t k ≠ SmtTerm.Numeral n := by
  intro h
  cases t <;> simp [__eo_to_smt_qs_choice] at h
  case not t =>
    cases t <;> simp at h

private theorem eo_to_smt_quantifiers_skolemize_ne_numeral
    (t : Term) (k : native_Nat) (n : native_Int) :
    __eo_to_smt_quantifiers_skolemize t k ≠ SmtTerm.Numeral n := by
  intro h
  cases t <;> try (simp [__eo_to_smt_quantifiers_skolemize] at h)
  case Apply f body =>
    ...
    case Apply g vars =>
      ...
      case UOp op =>
        ...
        case «forall» =>
          exact eo_to_smt_qs_choice_ne_numeral
...
private theorem eo_to_smt_apply_ne_numeral
  ...
  case UOp1 op idx =>
    ...
    case _at_strings_stoi_result =>
      exact False.elim
        (native_eo_to_smt_guard_closed_ne_numeral idx
          (SmtTerm.str_to_int
            (SmtTerm.str_substr (__eo_to_smt idx) (SmtTerm.Numeral 0)
              (__eo_to_smt x)))
          n (by intro hBody; cases hBody) h)
  ...
  case Apply g y =>
    ...
    case UOp op =>
      cases op <;> try cases h
      case set_insert =>
        exact False.elim (eo_to_smt_apply_set_insert_ne_numeral y x n (by
          rw [__eo_to_smt.eq_def]
          exact h))
      case «forall» =>
        exact False.elim (eo_to_smt_apply_forall_ne_numeral y x n (by
          rw [__eo_to_smt.eq_def]
          exact h))
      case «exists» =>
        exact False.elim (eo_to_smt_apply_exists_ne_numeral y x n (by
          rw [__eo_to_smt.eq_def]
          exact h))
      case tuple =>
...
theorem eo_to_smt_eq_numeral
  ...
  | UOp1 op x =>
    ...
    case _at_purify =>
      exact False.elim
        (native_eo_to_smt_guard_closed_ne_numeral x
          (SmtTerm._at_purify (__eo_to_smt x)) n
          (by intro hBody; cases hBody) h)
    case seq_empty =>
  ...
  | UOp2 op x y =>
    ...
    case _at_array_deq_diff =>
      exact False.elim
        (native_eo_to_smt_guard_closed_ne_numeral x
          ...
          n
          (native_eo_to_smt_guard_closed_ne_numeral y
            (__eo_to_smt_array_deq_diff
              (__eo_to_smt x) (__smtx_typeof (__eo_to_smt x))
              (__eo_to_smt y) (__smtx_typeof (__eo_to_smt y)))
            n
            (eo_to_smt_array_deq_diff_ne_numeral
              (__eo_to_smt x) (__eo_to_smt y) n))
          h)
    ...
    case _at_quantifiers_skolemize =>
      have hTop :
        ...
        rw [__eo_to_smt.eq_def]
        exact h
      exact False.elim (eo_to_smt_quantifier_term_ne_numeral x y n hTop)
  | Var name T =>
      rw [__eo_to_smt.eq_def] at h
      cases name <;> cases h
  | DtCons s d i =>
            ...
            (native_eo_to_smt_guard_closed_ne_numeral y
              ...
              (by
                ...
                cases hz : __eo_to_smt_nat_is_valid z <;>
                  simp [native_ite, hz] at hBody
                exact eo_to_smt_re_unfold_pos_component_ne_numeral
                  (__eo_to_smt x) (__eo_to_smt y) (__eo_to_smt_nat z) n hBody))
            h)
          ...
          (native_eo_to_smt_guard_closed_ne_numeral y
            (native_eo_to_smt_guard_closed z
              (native_ite
                ...
                SmtTerm.None))
            n
            (native_eo_to_smt_guard_closed_ne_numeral z
              (native_ite
                (native_Teq (__smtx_typeof (__eo_to_smt z)) SmtType.Int)
                (SmtTerm.choice_nth (native_string_lit "@x") (__eo_to_smt_type x)
                  (SmtTerm.eq

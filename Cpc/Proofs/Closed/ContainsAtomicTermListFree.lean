...
theorem eo_var_env_of_l1_get_unused_vars_ne_stuck
...
theorem eo_var_env_of_get_unused_vars_same_quant_body_ne_stuck
...
theorem contains_atomic_term_list_free_rec_false_except_ne_stuck
    {t except bound : Term}
    (hNoFree :
...
theorem eo_var_env_of_get_unused_vars_same_quant_body_of_contains_false
    {Q x F y : Term} {xVars yVars : List EoVarKey}
    (hQ : Q ≠ Term.Stuck)
    (hF : F ≠ Term.Stuck)
    (hX : EoVarEnv x xVars)
...
theorem eo_var_env_of_get_unused_vars_quant_branch_of_contains_false
    {Q x F Q₂ y F₂ : Term}
    {xVars : List EoVarKey}
    (hQ : Q = Term.UOp UserOp.forall ∨ Q = Term.UOp UserOp.exists)
    (hX : EoVarEnv x xVars)
...
theorem eo_var_env_of_get_unused_vars_of_contains_false
    {Q x F G : Term} {xVars : List EoVarKey}
    (hQ : Q = Term.UOp UserOp.forall ∨ Q = Term.UOp UserOp.exists)
    (hX : EoVarEnv x xVars)
...
theorem eo_var_env_of_get_unused_vars_of_quant_has_smt_translation
    {Q x F G : Term}
    (hQ : Q = Term.UOp UserOp.forall ∨ Q = Term.UOp UserOp.exists)
    (hLeftTrans :
...
theorem eo_var_env_perm_of_get_unused_vars_of_quant_has_smt_translation
    {Q x F G : Term}
    (hQ : Q = Term.UOp UserOp.forall ∨ Q = Term.UOp UserOp.exists)
    (hLeftTrans :
      eoHasSmtTranslation (Term.Apply (Term.Apply Q x) F))

              ...
              | nil =>
                  ...
                  · exact eoHasSmtTranslation.of_has_bool_type
                      (RuleProofs.eo_has_bool_type_of_interprets_true M _
                        hResultTrue)
                      (by
                        change eoUOpIndicesClosed (__eo_prog_arith_mult_pos m F)
                        have hmClosed : eoUOpIndicesClosed m := hmTrans0.indices_closed
                        have hFClosed : eoUOpIndicesClosed F := hFTrans0.indices_closed
                        cases m <;> cases F <;>
                          simp [__eo_prog_arith_mult_pos, __mk_arith_mult_pos,
                            __eo_mk_apply, __eo_nil, __eo_nil_mult,
                            __arith_mk_one, __arith_mk_zero, eoUOpIndicesClosed]
                            at hmClosed hFClosed ⊢)
              | cons _ _ =>
                  change Term.Stuck ≠ Term.Stuck at hProg
                  exact False.elim (hProg rfl)
          | cons _ _ =>

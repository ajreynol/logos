...
private theorem native_char_to_lower_digitChar_of_lt_10
    {d : Nat} (hd : d < 10) :
    native_char_to_lower (Char.toNat (Nat.digitChar d)) =
      Char.toNat (Nat.digitChar d) := by
  unfold native_char_to_lower Nat.digitChar
  by_cases h0 : d = 0
  · simp [h0]
  · by_cases h1 : d = 1
    · simp [h1]
...
private theorem map_native_char_to_lower_toDigitsCore :
    ∀ fuel n ds,
      List.map native_char_to_lower (ds.map Char.toNat) = ds.map Char.toNat ->
        List.map native_char_to_lower
  ...
  | fuel + 1, n, ds, hds => by
      ...
      have hDigit :
          native_char_to_lower (Char.toNat (Nat.digitChar (n % 10))) =

...
private def bvOpAnd : BvOpSpec where
  ...
  htypeof := fun X Y => by
    rw [show __eo_to_smt (Term.Apply (Term.Apply (Term.UOp UserOp.bvand) X) Y)
          = SmtTerm.bvand (__eo_to_smt X) (__eo_to_smt Y) from rfl, __smtx_typeof.eq_bvand]
  hvalN := bvand_valN
...
private def bvOpOr : BvOpSpec where
  ...
  htypeof := fun X Y => by
    rw [show __eo_to_smt (Term.Apply (Term.Apply (Term.UOp UserOp.bvor) X) Y)
          = SmtTerm.bvor (__eo_to_smt X) (__eo_to_smt Y) from rfl, __smtx_typeof.eq_bvor]
  hvalN := bvor_valN
...
private def bvOpXor : BvOpSpec where
  ...
  htypeof := fun X Y => by
    rw [show __eo_to_smt (Term.Apply (Term.Apply (Term.UOp UserOp.bvxor) X) Y)
          = SmtTerm.bvxor (__eo_to_smt X) (__eo_to_smt Y) from rfl, __smtx_typeof.eq_bvxor]
  hvalN := bvxor_valN

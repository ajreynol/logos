...
private theorem native_seq_extract_len_tail_of_bounds
    (xs : List SmtValue) (i : native_Int)
    (hNonneg : 0 ≤ i)
    (hLeLen : i ≤ native_seq_len xs) :
    native_seq_extract xs i (native_seq_len xs) = xs.drop (Int.toNat i) := by
  ...
  · have hStartLtNat : Int.toNat i < xs.length :=
    ...
    have hINotGeLen : ¬ i ≥ (Int.ofNat xs.length : Int) := by
      ...
      have hGeNat : xs.length ≤ Int.toNat i :=
        Int.ofNat_le.mp hGeInt
      omega
    have hLenNotLeI : ¬ (Int.ofNat xs.length : Int) ≤ i :=
      hINotGeLen
    have hLenPos : xs.length ≠ 0 := by
      omega
    have hXsNe : xs ≠ [] := by
      intro hNil
      subst xs

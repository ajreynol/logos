...
private theorem str_leq_eval_rec_strCharChain :
    ∀ s t : native_String,
      native_string_valid s = true ->
      native_string_valid t = true ->
        __str_leq_eval_rec (strCharChain s) (strCharChain t) =
  ...
  | c :: cs, d :: ds, hs, ht => by
      rcases native_string_valid_cons_parts_local hs with ⟨hc, hcs⟩
      rcases native_string_valid_cons_parts_local ht with ⟨hd, hds⟩
      have hTail := str_leq_eval_rec_strCharChain cs ds hcs hds
      rw [show strCharChain (c :: cs) =
        Term.Apply (Term.Apply (Term.UOp UserOp.str_concat) (Term.String [c]))
          (strCharChain cs) by
        rfl]
      rw [show strCharChain (d :: ds) =
        Term.Apply (Term.Apply (Term.UOp UserOp.str_concat) (Term.String [d]))
          (strCharChain ds) by
        rfl]
      unfold __str_leq_eval_rec
      rw [eo_to_z_singleton hc, eo_to_z_singleton hd]
      by_cases hcd : c = d
      · subst d
...
private theorem str_leq_result_strings
    {s t : native_String}
  ...
  rw [hGuard, eo_ite_true]
  rw [str_flatten_nary_intro_string_char_chain s]
  rw [str_flatten_nary_intro_string_char_chain t]
  exact str_leq_eval_rec_strCharChain s t hs ht

private theorem str_leq_result_non_strings
    {s t : Term}
    (hNotBoth :
  ...
  rw [eo_mk_apply_eq_apply_of_args_ne_stuck
    (Term.UOp UserOp.str_leq) s (by intro h; cases h) hsNe]
  exact eo_mk_apply_eq_apply_of_args_ne_stuck
...
private theorem smt_value_rel_model_eval_str_leq_of_rel
    (a b c d : SmtValue) :
...
private theorem smt_value_rel_model_eval_str_replace_of_rel
...
private theorem smt_value_rel_model_eval_int_log2_of_rel
    (a b : SmtValue) :
    RuleProofs.smt_value_rel a b ->
    RuleProofs.smt_value_rel
...
private theorem smtx_typeof_binary_eq_bitvec_parts
...
private theorem native_nat_to_int_of_int_to_nat_eq
...
private theorem model_eval_bitvec_term_binary
...
private theorem native_qsub_lt_zero_eq_eval
    (q1 q2 : native_Rat) :
...
private theorem eo_leq_int_result_eq
    (n1 n2 : native_Int) :
    __eo_or
        (__eo_is_neg
...
private theorem eo_leq_real_result_eq
    (q1 q2 : native_Rat) :
    __eo_or
        (__eo_is_neg
...
private theorem eo_to_q_int_arg_of_nonstuck
    (x : Term)
...
private theorem eo_to_q_real_arg_of_nonstuck
    (x : Term)
...
private theorem eo_to_z_real_arg_of_nonstuck
    (x : Term)
    (hxTy : __smtx_typeof (__eo_to_smt x) = SmtType.Real)
...
private theorem eo_is_int_result_rel
...
private theorem eo_lt_int_run_args_of_nonstuck
    (x y : Term)
    (hxTy : __smtx_typeof (__eo_to_smt x) = SmtType.Int)
    ...
    ∃ nx : native_Int, ∃ ny : native_Int,
      x = Term.Numeral nx ∧ y = Term.Numeral ny := by
  rcases eo_is_neg_arg_arith_of_nonstuck
  ...
  · rcases hInnerInt with ⟨n, hInner⟩
    have hInnerTy :
      ...
      rw [hInner]
      rfl
    rcases eo_add_args_numeral_of_typeof_int x (__eo_neg y)
        hInnerTy with
      ⟨nx, nNegY, hx, hNegY⟩
    have hNegYTy : __eo_typeof (__eo_neg y) =
      ...
      rw [hNegY]
      rfl
    rcases eo_neg_arg_numeral_of_typeof_int y hNegYTy with
      ⟨ny, hy⟩
    exact ⟨nx, ny, hx, hy⟩
  · rcases hInnerReal with ⟨q, hInner⟩
    have hInnerTy :
      ...
      rw [hInner]
      rfl
    rcases eo_add_args_rational_of_typeof_real x (__eo_neg y)
        hInnerTy with
      ⟨qx, _qNegY, hx, _hNegY⟩
    rw [hx] at hxTy
...
private theorem eo_lt_real_run_args_of_nonstuck
    (x y : Term)
    (hxTy : __smtx_typeof (__eo_to_smt x) = SmtType.Real)
  ...
  · rcases hInnerReal with ⟨q, hInner⟩
    ...
    have hNegYTy : __eo_typeof (__eo_neg y) =
      ...
      rw [hNegY]
      rfl
    rcases eo_neg_arg_rational_of_typeof_real y hNegYTy with
      ⟨qy, hy⟩
    exact ⟨qx, qy, hx, hy⟩
...
private theorem eo_zdiv_args_binary_of_typeof_bitvec
    (x y : Term) (w : native_Int) :
...
private theorem eo_sbv_to_int_msb_zero_eq_of_pos
...
private theorem native_int_pow2_lt_of_lt_nonneg
  ...
  have hWPos : 0 < native_int_pow2 w :=
    native_int_pow2_pos_of_nonneg hw
...
private theorem native_mod_div_pow2_eq_zero_of_lt
    {w k x : native_Int} (hw : 0 <= w) (hlt : w < k)
  ...
  have hPowLt : native_int_pow2 w < native_int_pow2 k :=
    native_int_pow2_lt_of_lt_nonneg hw hlt
...
private theorem eo_signed_bv_value_eq_stuck_of_not_binary
    (x : Term)
    (hNotBinary : ¬ ∃ w n : native_Int, x = Term.Binary w n)
    (hNotString : ¬ ∃ s : native_String, x = Term.String s) :
    eo_signed_bv_value x = Term.Stuck := by
  cases x
  case String s =>
  ...
  all_goals
    simp [eo_signed_bv_value, __eo_extract, __eo_eq, __eo_ite,
      native_ite, native_teq] at hNotBinary ⊢

private def smt_bv_msb_set (w n : native_Int) : native_Bool :=
  native_zeq
    (native_mod_total
      (native_div_total n (native_int_pow2 (w - 1))) 2) 1

private theorem smtx_model_eval_bvsgt_binary_eq_formula
    (w n1 n2 : native_Int) :
    __smtx_model_eval_bvsgt (SmtValue.Binary w n1) (SmtValue.Binary w n2) =
      SmtValue.Boolean
        (native_or
          (native_and
            (native_not (smt_bv_msb_set w n1))
            (smt_bv_msb_set w n2))
          (native_and
            (native_veq
              (SmtValue.Boolean (smt_bv_msb_set w n1))
              (SmtValue.Boolean (smt_bv_msb_set w n2)))
            (native_zlt n2 n1))) := by
  simp [__smtx_model_eval_bvsgt, __smtx_model_eval__,
    ...
    smt_bv_msb_set, __smtx_bv_sizeof_value, native_binary_extract,
    native_zplus, SmtEval.native_zplus, native_zneg,
    SmtEval.native_zneg]
...
private theorem smt_bv_msb_false_eq_mod_of_pos
    {w n : native_Int}
    (hwpos : 0 < w)
    (hw0 : native_zleq 0 w = true)
    (hCanon :
  ...
  · have hSignTrue : smt_bv_msb_set w n = true := by
      dsimp [smt_bv_msb_set, q, p] at hq ⊢
      rw [hq]
      simp [native_mod_total, native_zeq, SmtEval.native_zeq]
    rw [hSign] at hSignTrue
    cases hSignTrue

private theorem smt_bv_msb_true_eq_pow_add_mod_of_pos
    {w n : native_Int}
    (hwpos : 0 < w)
    (hw0 : native_zleq 0 w = true)
    (hCanon :
  ...
  have hDivMod : p * q + r = n := by
    simpa [q, r, p, native_div_total, native_mod_total] using
      Int.mul_ediv_add_emod n p
  rcases hqCases with hq | hq
  · have hSignFalse : smt_bv_msb_set w n = false := by
      dsimp [smt_bv_msb_set, q, p] at hq ⊢
      rw [hq]
      simp [native_mod_total, native_zeq, SmtEval.native_zeq]
...
private theorem native_mod_pow2_pred_range_of_pos
    {w n : native_Int} (hwpos : 0 < w) :
  ...
  have hpPos : 0 < native_int_pow2 (w - 1) :=
    native_int_pow2_pos_of_nonneg hwp0
  constructor
...
private theorem smt_bvsgt_formula_eq_signed_gt
    {w n1 n2 : native_Int}
    (hw0 : native_zleq 0 w = true)
    (hCanon1 :
      native_zeq n1
  ...
  · let p := native_int_pow2 (w - 1)
    ...
    have hUts1 :
        (if smt_bv_msb_set w n1 then
            native_zplus (native_zneg p) r1
          else r1) =
          native_binary_uts w n1 := by
      simpa [smt_bv_msb_set, p, r1, Int.sub_eq_add_neg] using
        sign_payload_eq_uts_core (w := w) (n := n1) hw0 hCanon1
    have hUts2 :
        (if smt_bv_msb_set w n2 then
            native_zplus (native_zneg p) r2
          else r2) =
          native_binary_uts w n2 := by
      simpa [smt_bv_msb_set, p, r2, Int.sub_eq_add_neg] using
        sign_payload_eq_uts_core (w := w) (n := n2) hw0 hCanon2
    have hr1Range :
        0 <= r1 ∧ r1 < p := by
      simpa [r1, p] using
        native_mod_pow2_pred_range_of_pos (w := w) (n := n1) hwpos
    have hr2Range :
        0 <= r2 ∧ r2 < p := by
      simpa [r2, p] using
        native_mod_pow2_pred_range_of_pos (w := w) (n := n2) hwpos
    cases hSign1 : smt_bv_msb_set w n1 <;>
      cases hSign2 : smt_bv_msb_set w n2
    · have hN1 :
          n1 = r1 :=
...
private theorem native_veq_binary_eq_signed_teq_of_canonical
    {w n1 n2 : native_Int}
    (hw0 : native_zleq 0 w = true)
    (hCanon1 :
      native_zeq n1
  ...
  have hIff :=
    native_binary_uts_eq_iff_canonical
...
private theorem smtx_model_eval_bvsge_binary_eq_uts
    {w n1 n2 : native_Int}
    (hw0 : native_zleq 0 w = true)
    (hCanon1 :
      native_zeq n1
  ...
  rw [smtx_model_eval_bvsgt_binary_eq_uts hw0 hCanon1 hCanon2]
  simp [__smtx_model_eval_or, __smtx_model_eval_eq]
  rw [native_veq_binary_eq_signed_teq_of_canonical
    hw0 hCanon1 hCanon2]

private theorem native_teq_term_numeral_comm
    (n1 n2 : native_Int) :
...
private theorem smtx_model_eval_bvsle_binary_eq_uts
    {w n1 n2 : native_Int}
    (hw0 : native_zleq 0 w = true)
    (hCanon1 :
      native_zeq n1
  ...
  rw [smtx_model_eval_bvsge_binary_eq_uts
    (w := w) (n1 := n2) (n2 := n1) hw0 hCanon2 hCanon1]
  rw [native_teq_term_numeral_comm]
...
private def eo_eval_sbv_to_int_rhs (x : Term) : Term :=
...
private theorem eo_eval_sbv_to_int_rhs_binary_eq_uts
    {w n : native_Int}
    (hw0 : native_zleq 0 w = true)
    (hCanon :
      native_zeq n (native_mod_total n (native_int_pow2 w)) = true) :
    eo_eval_sbv_to_int_rhs (Term.Binary w n) =
      Term.Numeral (native_binary_uts w n) := by
  by_cases hwz : w = 0
  · subst w
    have hRange := bitvec_payload_range_of_canonical hw0 hCanon
  ...
  · have hw0Int : 0 <= w := by
    ...
    have hPowExpr :
      ...
      eo_int_pow2_literal_eq w
    dsimp [eo_eval_sbv_to_int_rhs]
    change
    ...
    rw [hEqW0, eo_ite_false,
      eo_sbv_to_int_msb_zero_eq_of_pos (w := w) (n := n) hwpos]
    cases hSignZero :
        native_zeq
          (native_mod_total
            (native_div_total n (native_int_pow2 (w - 1))) 2)
    ...
    · rw [eo_ite_false, hPowExpr]
      ...
      simpa [hSignZero] using
        sbv_to_int_payload_eq_uts_core
          (w := w) (n := n) hw0 hCanon
    · rw [eo_ite_true]
      exact congrArg Term.Numeral
        (by
...
private theorem eo_eval_sbv_to_int_rhs_arg_binary_of_pos_typeof_int
    (x : Term) {w n : native_Int}
    (hRun : __run_evaluate x = Term.Binary w n)
    (hwpos : 0 < w)
    (hTy : __eo_typeof (eo_eval_sbv_to_int_rhs x) =
      Term.UOp UserOp.Int) :
  ...
  all_goals
    exfalso
    dsimp [eo_eval_sbv_to_int_rhs] at hTy
...
private theorem eo_eval_sbv_to_int_rhs_eq_zero_of_run_typeof_zero
    (x : Term)
    (hRunTy :
      __eo_typeof (__run_evaluate x) =
        Term.Apply (Term.UOp UserOp.BitVec) (Term.Numeral 0)) :
    eo_eval_sbv_to_int_rhs x = Term.Numeral 0 := by
  dsimp [eo_eval_sbv_to_int_rhs]
  rw [hRunTy]
  rfl

private theorem eo_eval_sbv_to_int_rhs_arg_binary_of_pos_run_typeof_int
    (x : Term) {k : native_Int}
    (hRunTy :
      __eo_typeof (__run_evaluate x) =
...
private theorem eo_concat_args_binary_of_typeof_bitvec
    (x y : Term) (w : native_Int) :
    __eo_typeof (__eo_concat x y) =
      Term.Apply (Term.UOp UserOp.BitVec) (Term.Numeral w) ->
...
private theorem eo_len_seq_arg_of_nonstuck
    (x : Term) {T : SmtType}
  ...
  case String str =>
    rcases smt_string_seq_type_inv hTy with ⟨hValid, hT⟩
...
private theorem eo_extract_same_seq_int_args_of_nonstuck
    (x n : Term) {T : SmtType}
...
private theorem eo_extract_seq_char_numeral_args_of_nonstuck
  ...
  cases x <;> simp [__eo_extract] at hExt
  case String str =>
    exact ⟨str, rfl, native_string_valid_of_string_type hxTy⟩
  case Binary w nval =>
...
private theorem eo_find_to_str_seq_args_of_nonstuck
    (x y : Term) {T : SmtType}
    (hxTy : __smtx_typeof (__eo_to_smt x) = SmtType.Seq T)
    (hyTy : __smtx_typeof (__eo_to_smt y) = SmtType.Seq T)
...
private theorem str_replace_run_repl_string_of_nonneg
  ...
  case String repl =>
    exact ⟨repl, rfl, native_string_valid_of_string_type hZTy⟩

private theorem smt_model_eval_seq_of_type_local
...
private theorem native_string_valid_substr_local
...
private theorem native_seq_extract_pack_string
  ...
  simp only [native_pack_string, native_seq_extract, native_str_substr,
    native_str_len, native_unpack_pack_seq_local]
  by_cases h : (i < 0 ∨ n ≤ 0) ∨ ↑s.length ≤ i
  · simp [h]
  · simp [h, List.map_take, List.map_drop]

private theorem native_seq_extract_pack_string_one
  ...
  native_seq_extract_pack_string s i 1

private theorem native_pack_string_injective :
...
private theorem smtx_model_eval_str_leq_pack_string
    (s t : native_String) :
  ...
  · have hPackNe : native_pack_string s ≠ native_pack_string t := by
      intro hPack
      exact hEq (native_pack_string_injective hPack)
    simp [__smtx_model_eval_eq, __smtx_model_eval_or, native_veq,
...
private theorem native_string_prefix_eq_length_le
    (pat s : native_String)
...
private theorem native_str_indexof_zero_of_prefix
    (s pat : native_String)
    (hPrefix : native_string_prefix_eq pat s = true) :
  ...
  | cons p ps =>
      have hPatLe : (p :: ps).length ≤ s.length :=
        native_string_prefix_eq_length_le (p :: ps) s hPrefix
...
private def native_str_replace_eval_result
...
private theorem native_str_substr_prefix_take_local
...
private theorem native_str_substr_suffix_drop_local
...
private theorem native_seq_replace_pack_string
    ...
    native_pack_seq SmtType.Char
      ...
      native_pack_string (native_str_replace_eval_result s pat repl) := by
  rw [show native_unpack_seq (native_pack_string s) =
      s.map SmtValue.Char by
    simp [native_pack_string, native_unpack_pack_seq_local]]
  ...
  cases pat with
  | nil =>
      simp [native_seq_replace, native_str_replace_eval_result,
  ...
  | cons p ps =>
      ...
      · simp only [native_seq_replace, List.map_cons]
        rw [hIdx']
        simp [native_str_replace_eval_result, hNeg, native_pack_string]
      · simp only [native_seq_replace, List.map_cons]
        rw [hIdx']
        simp [native_str_replace_eval_result, hNeg, native_pack_string,
          List.map_append, List.map_take, List.map_drop]

private theorem str_replace_result_strings
    (s pat repl : native_String) :
    __eo_ite (__eo_is_neg
          (__eo_find (__eo_to_str (Term.String s))
            (__eo_to_str (Term.String pat))))
      ...
      Term.String (native_str_replace_eval_result s pat repl) := by
  let idx := native_str_indexof s pat 0
  simp only [__eo_find, __eo_to_str]
  unfold native_str_replace_eval_result
  by_cases hNeg : idx < 0
  · have hGuard :
  ...
  · have hNonneg : 0 ≤ idx := Int.le_of_not_gt hNeg
    ...
    rw [show native_str_substr s 0 (native_str_indexof s pat 0) =
      ...
      rw [← hIdxNat]
      exact native_str_substr_prefix_take_local s n]
    rw [show
        native_str_substr s
...
private theorem native_str_replace_all_eval_result_nil
    (s repl : native_String) :
    native_str_replace_all_eval_result s [] repl = s := by
  unfold native_str_replace_all_eval_result
  cases s <;>
    simp [native_str_replace_all_eval_aux]

private theorem native_str_replace_all_eval_aux_prefix_cons
    (fuel : Nat) (s repl : native_String) (p : native_Char)
    (ps : native_String)
    (hPrefix : native_string_prefix_eq (p :: ps) s = true) :
    native_str_replace_all_eval_aux (fuel + 1) (p :: ps) repl s =
      repl ++
        native_str_replace_all_eval_aux fuel (p :: ps) repl
...
private theorem smtx_model_eval_str_replace_all_pack_string_nil
    (s repl : native_String) :
    __smtx_model_eval_str_replace_all
        (SmtValue.Seq (native_pack_string s))
  ...
  rw [native_str_replace_all_eval_result_nil]

private def native_seq_update_string_result
...
private theorem native_string_valid_seq_update_string_result
    ...
    native_string_valid (native_seq_update_string_result s i repl) = true := by
...
private theorem native_seq_update_pack_string
    ...
    native_pack_seq SmtType.Char
        (native_seq_update
          ...
          (native_unpack_seq (native_pack_string repl))) =
      native_pack_string (native_seq_update_string_result s i repl) := by
  rw [show native_unpack_seq (native_pack_string s) =
      s.map SmtValue.Char by
    simp [native_pack_string, native_unpack_pack_seq_local]]
  rw [show native_unpack_seq (native_pack_string repl) =
      repl.map SmtValue.Char by
    simp [native_pack_string, native_unpack_pack_seq_local]]
  unfold native_seq_update native_seq_update_string_result
...
private theorem smtx_model_eval_str_update_pack_string
    (s repl : native_String) (i : native_Int) :
...
private theorem eo_extract_string_suffix_window
...
private theorem native_int_log2_of_neg
...
private theorem eo_int_log2_result_arg_typeof_int
    (x : Term) :
    __eo_typeof
        (__eo_ite (__eo_is_z x)
          (__eo_ite (__eo_is_neg x) (Term.Numeral 0)
...
private theorem eo_int_log2_eval_numeral_to_smt
    (n : native_Int) :
    __eo_to_smt
        (__eo_ite (__eo_is_z (Term.Numeral n))
          (__eo_ite (__eo_is_neg (Term.Numeral n)) (Term.Numeral 0)
  ...
  · simp [__eo_is_z, __eo_is_z_internal, __eo_is_neg, __eo_ite,
      ...
      native_not, hNeg, native_int_log_two_eq_log2]
    rfl
...
private theorem run_evaluate_sound_apply_to_real_core
    (M : SmtModel) (hM : model_total_typed M)
    (x : Term)
    (rec :
      ∀ A : Term,
        sizeOf A < sizeOf (Term.Apply (Term.UOp UserOp.to_real) x) ->
          RunEvaluateSoundGoal M A) :
  RunEvaluateSoundGoal M (Term.Apply (Term.UOp UserOp.to_real) x) := by
  intro hATrans hEvalTy
  have hToRealNN :
  ...
  · have hXTrans : RuleProofs.eo_has_smt_translation x := by
    ...
    have hXProgTy : __eo_typeof (__eo_prog_evaluate x) = Term.Bool :=
      eo_prog_evaluate_typeof_bool_of_smt_type_int x hXTrans hXInt
  ...
  · have hXTrans : RuleProofs.eo_has_smt_translation x := by
      ...
      simp
    have hXProgTy : __eo_typeof (__eo_prog_evaluate x) = Term.Bool :=
      eo_prog_evaluate_typeof_bool_of_smt_type_real x hXTrans hXReal
    rcases run_evaluate_rec_apply_arg M
        (Term.UOp UserOp.to_real) x rec hXTrans hXProgTy with
      ⟨hXSameTy, hXRel⟩
    have hRunXSmtTy :
...
private theorem run_evaluate_sound_apply_to_int_core
    (M : SmtModel) (hM : model_total_typed M)
    (x : Term)
    (rec :
      ∀ A : Term,
        sizeOf A < sizeOf (Term.Apply (Term.UOp UserOp.to_int) x) ->
          RunEvaluateSoundGoal M A) :
  RunEvaluateSoundGoal M (Term.Apply (Term.UOp UserOp.to_int) x) := by
  intro hATrans hEvalTy
  have hToIntNN :
  ...
  have hRunXSmtTy :
    ...
    rw [← hXSameTy]
    exact hXReal
  rcases eo_to_z_real_arg_of_nonstuck
...
private theorem run_evaluate_sound_apply_is_int_core
    (M : SmtModel) (hM : model_total_typed M)
    (x : Term)
    (rec :
      ∀ A : Term,
        sizeOf A < sizeOf (Term.Apply (Term.UOp UserOp.is_int) x) ->
          RunEvaluateSoundGoal M A) :
  RunEvaluateSoundGoal M (Term.Apply (Term.UOp UserOp.is_int) x) := by
  intro hATrans hEvalTy
  have hIsIntNN :
  ...
  · have hXRelValue :
    ...
    rw [show
      ...
      rw [__smtx_model_eval.eq_def] <;> simp only]
    rw [hXEval]
    exact eo_is_int_result_rel M runQ
...
private theorem run_evaluate_sound_apply_int_log2_core
    (M : SmtModel) (hM : model_total_typed M)
    (b : Term)
    (rec :
      ∀ A : Term,
        sizeOf A < sizeOf (Term.Apply (Term.UOp UserOp.int_log2) b) ->
          RunEvaluateSoundGoal M A) :
  RunEvaluateSoundGoal M (Term.Apply (Term.UOp UserOp.int_log2) b) := by
  intro hATrans hEvalTy
  have hLogNN :
  ...
  have hRunBEoType :
      __eo_typeof (__run_evaluate b) = Term.UOp UserOp.Int :=
    eo_int_log2_result_arg_typeof_int (__run_evaluate b) hRunLogEoInt
  have hIntTypeNe : Term.UOp UserOp.Int ≠ Term.Stuck := by
    intro h
    cases h
  have hBProgTy : __eo_typeof (__eo_prog_evaluate b) = Term.Bool :=
  ...
  | Numeral runN =>
      constructor
      · rw [eo_int_log2_eval_numeral_to_smt]
  ...
  | _ =>
      ...
      · rw [hRunLogToSmt]
        ...
        exact smt_value_rel_model_eval_int_log2_of_rel
          (__smtx_model_eval M (__eo_to_smt b))
          (__smtx_model_eval M (__eo_to_smt (__run_evaluate b))) hBRel
...
private theorem run_evaluate_sound_apply_concat_core
    (M : SmtModel) (hM : model_total_typed M)
    (a b : Term)
    (rec :
      ∀ A : Term,
  ...
  rcases eo_concat_args_binary_of_typeof_bitvec
  ...
  have hAProgTy : __eo_typeof (__eo_prog_evaluate a) = Term.Bool :=
    eo_prog_evaluate_typeof_bool_of_smt_type_bitvec a wa hATransA hATy
  have hBProgTy : __eo_typeof (__eo_prog_evaluate b) = Term.Bool :=
    eo_prog_evaluate_typeof_bool_of_smt_type_bitvec b wb hBTrans hBTy
  ...
  · rw [hConcatTerm]
    rw [typeof_concat_eq]
    rw [hATy, hBTy]
    rw [smtx_typeof_binary_mod_of_nonneg _ _ hRunWidthNonneg]
...
private theorem run_evaluate_sound_apply_bvudiv_core
    (M : SmtModel) (hM : model_total_typed M)
    (a b : Term)
    (rec :
      ∀ A : Term,
  ...
  rcases bv_binop_args_of_non_none
      ...
      (by rw [__smtx_typeof.eq_49]) hBvUdivNN with
    ⟨w, hATy, hBTy⟩
  have hATransA : RuleProofs.eo_has_smt_translation a := by
    unfold RuleProofs.eo_has_smt_translation
    rw [hATy]
  ...
  case Binary runWB runB =>
    ...
    · subst runB
      ...
      · rcases model_eval_bitvec_term_binary M hM a w hATy with
        ...
        rcases model_eval_bitvec_term_binary M hM b w hBTy with
    ...
    · have hZeroFalse : native_zeq 0 runB = false := by
      ...
      rcases eo_zdiv_args_binary_of_typeof_bitvec
...
private theorem run_evaluate_sound_apply_bvurem_core
    (M : SmtModel) (hM : model_total_typed M)
    (a b : Term)
    (rec :
      ∀ A : Term,
  ...
  rcases bv_binop_args_of_non_none
      ...
      (by rw [__smtx_typeof.eq_50]) hBvUremNN with
    ⟨w, hATy, hBTy⟩
  have hATransA : RuleProofs.eo_has_smt_translation a := by
    unfold RuleProofs.eo_has_smt_translation
    rw [hATy]
  ...
  case Binary runWB runB =>
    ...
    · have hZeroFalse : native_zeq 0 runB = false := by
      ...
      rcases eo_zmod_args_binary_of_typeof_bitvec
          (__run_evaluate a) (Term.Binary runWB runB)
...
private theorem run_evaluate_sound_apply_bvult_core
    (M : SmtModel) (hM : model_total_typed M)
    (a b : Term)
    (rec :
      ∀ A : Term,
  ...
  rcases bv_binop_ret_args_of_non_none
      ...
      (by rw [__smtx_typeof.eq_55]) hBvUltNN with
    ⟨w, hATy, hBTy⟩
  have hATransA : RuleProofs.eo_has_smt_translation a := by
    unfold RuleProofs.eo_has_smt_translation
    rw [hATy]
...
private theorem run_evaluate_sound_apply_bvugt_core
    (M : SmtModel) (hM : model_total_typed M)
    (a b : Term)
    (rec :
      ∀ A : Term,
  ...
  rcases bv_binop_ret_args_of_non_none
      ...
      (by rw [__smtx_typeof.eq_57]) hBvUgtNN with
    ⟨w, hATy, hBTy⟩
  have hATransA : RuleProofs.eo_has_smt_translation a := by
    unfold RuleProofs.eo_has_smt_translation
    rw [hATy]
  ...
  have hBvUltSwapNN :
    ...
    unfold term_has_non_none_type
    rw [__smtx_typeof.eq_55, hBTy, hATy]
    simp [__smtx_typeof_bv_op_2_ret, native_ite, native_nateq]
  ...
  have hBvUltSwapEvalTy :
      __eo_typeof
        (__eo_prog_evaluate
          (Term.Apply (Term.Apply (Term.UOp UserOp.bvult) b) a)) =
        Term.Bool := by
    apply eo_prog_evaluate_typeof_bool_of_typeof_bool_and_run_typeof_bool
    · intro h
      cases h
  ...
  have recSwap :
    ...
    exact hLt
  have hSwap :=
    run_evaluate_sound_apply_bvult_core M hM b a recSwap
  ...
  · have hTypeEq :
      ...
      rw [__smtx_typeof.eq_57, __smtx_typeof.eq_55, hATy, hBTy]
    rw [hTypeEq]
    exact hSwap.1
  · have hEvalEq :
        __smtx_model_eval M
...
private theorem run_evaluate_sound_apply_bvuge_core
    (M : SmtModel) (hM : model_total_typed M)
    (a b : Term)
    (rec :
      ∀ A : Term,
...
private theorem run_evaluate_sound_apply_bvule_core
    (M : SmtModel) (hM : model_total_typed M)
    (a b : Term)
    (rec :
      ∀ A : Term,
...
private theorem run_evaluate_sound_apply_bvsgt_core
    (M : SmtModel) (hM : model_total_typed M)
    (a b : Term)
    (rec :
      ∀ A : Term,
  ...
  have hBvSgtEoType :
    ...
    simp [__eo_typeof_bvult, __eo_requires, __eo_eq, native_ite,
      native_teq, native_not]
  let runCmp :=
    __eo_gt (eo_signed_bv_value (__run_evaluate a))
      (eo_signed_bv_value (__run_evaluate b))
  have hRunCmpNe : runCmp ≠ Term.Stuck := by
    intro hStuck
    change
      __eo_typeof
  ...
  case Binary runWA runA =>
    ...
    case Binary runWB runB =>
      ...
      · rw [show
        ...
        rw [hAEval, hBEval]
        rw [hRunCmpSmt]
        rw [smtx_model_eval_bvsgt_binary_eq_uts
    ...
    all_goals
      ...
      have hRunBNotString :
        ...
        cases hEq
      have hSignedB :
          eo_signed_bv_value (__run_evaluate b) = Term.Stuck :=
        eo_signed_bv_value_eq_stuck_of_not_binary
          (__run_evaluate b) hRunBNotBinary hRunBNotString
      have hRunCmpStuck : runCmp = Term.Stuck := by
        dsimp [runCmp]
        rw [hSignedB]
        cases eo_signed_bv_value (__run_evaluate a) <;>
          simp [__eo_gt]
      exact False.elim (hRunCmpNe hRunCmpStuck)
  case Numeral runN =>
    rw [hRunA] at hRunASmtTy
  ...
  all_goals
    ...
    have hRunANotString :
      ...
      cases hEq
    have hSignedA :
        eo_signed_bv_value (__run_evaluate a) = Term.Stuck :=
      eo_signed_bv_value_eq_stuck_of_not_binary
...
private theorem run_evaluate_sound_apply_bvslt_core
    (M : SmtModel) (hM : model_total_typed M)
    (a b : Term)
    (rec :
      ∀ A : Term,
  ...
  have hBvSltEoType :
    ...
    simp [__eo_typeof_bvult, __eo_requires, __eo_eq, native_ite,
      native_teq, native_not]
  let runCmp :=
    __eo_gt (eo_signed_bv_value (__run_evaluate b))
      (eo_signed_bv_value (__run_evaluate a))
  have hRunCmpNe : runCmp ≠ Term.Stuck := by
    intro hStuck
    change
      __eo_typeof
  ...
  have recSwap :
    ...
    exact hLt
  have hSwap :=
    run_evaluate_sound_apply_bvsgt_core M hM b a recSwap
...
private theorem run_evaluate_sound_apply_bvsge_core
    (M : SmtModel) (hM : model_total_typed M)
    (a b : Term)
    (rec :
      ∀ A : Term,
  ...
  let runCmp :=
    __eo_or
      (__eo_gt (eo_signed_bv_value (__run_evaluate a))
        (eo_signed_bv_value (__run_evaluate b)))
      (__eo_eq (eo_signed_bv_value (__run_evaluate a))
        (eo_signed_bv_value (__run_evaluate b)))
  have hRunCmpNe : runCmp ≠ Term.Stuck := by
    intro hStuck
    change
...
private theorem run_evaluate_sound_apply_bvsle_core
    (M : SmtModel) (hM : model_total_typed M)
    (a b : Term)
    (rec :
      ∀ A : Term,
  ...
  case Binary runWA runA =>
    ...
    case Binary runWB runB =>
      ...
      · rw [show
        ...
        rw [hAEval, hBEval]
        rw [hRunCmpSmt]
        rw [smtx_model_eval_bvsle_binary_eq_uts
          hRunWANonneg hRunACanon hRunBCanon]
        rw [__smtx_model_eval.eq_1]
        exact RuleProofs.smt_value_rel_refl _
    case Numeral runN =>
  ...
  all_goals
    ...
    have hRunCmpStuck : runCmp = Term.Stuck := by
      dsimp [runCmp]
      rw [hSignedA]
      cases eo_signed_bv_value (__run_evaluate b) <;>
        simp [__eo_or, __eo_gt, __eo_eq]
    exact False.elim (hRunCmpNe hRunCmpStuck)
...
private theorem run_evaluate_sound_apply_bvlshr_core
    (M : SmtModel) (hM : model_total_typed M)
    (a b : Term)
    (rec :
      ∀ A : Term,
      ...
      · rcases model_eval_bitvec_term_binary M hM a w hATy with
        ...
        have hZeroMod :
            native_mod_total
                ...
                (native_int_pow2 (native_nat_to_int w)) =
              0 :=
          native_mod_div_pow2_eq_zero_of_lt hWNonneg hGtInt
            hARange.1 hARange.2
        rw [show
            __smtx_model_eval M
                (SmtTerm.bvlshr (__eo_to_smt a) (__eo_to_smt b)) =
    ...
    · have hGtFalse :
      ...
      have hPowZeroFalse :
        ...
        have hPowPos : 0 < native_int_pow2 runB :=
          native_int_pow2_pos_of_nonneg hRunBRange.1
        simp [native_zeq, SmtEval.native_zeq]
...
private theorem run_evaluate_sound_apply_lt_core
    (M : SmtModel) (hM : model_total_typed M)
    (a b : Term)
    (rec :
      ∀ A : Term,
  ...
  · rcases hArgsInt with ⟨hATyInt, hBTyInt⟩
    ...
    have hRunBSmtTy :
      ...
      rw [← hBSameTy]
      exact hBTyInt
    rcases eo_lt_int_run_args_of_nonstuck
        (__run_evaluate a) (__run_evaluate b)
    ...
    · change
      ...
      rw [native_zsub_lt_zero_eq_eval]
      ...
      change
        RuleProofs.smt_value_rel
          ...
          (__smtx_model_eval M
            (SmtTerm.Boolean
              (native_zlt (native_zplus runA (native_zneg runB)) 0)))
      rw [native_zsub_lt_zero_eq_eval, __smtx_model_eval.eq_1]
      exact RuleProofs.smt_value_rel_refl _
  · rcases hArgsReal with ⟨hATyReal, hBTyReal⟩
    have hATransA : RuleProofs.eo_has_smt_translation a := by
      unfold RuleProofs.eo_has_smt_translation
    ...
    have hRunBSmtTy :
      ...
      rw [← hBSameTy]
      exact hBTyReal
    rcases eo_lt_real_run_args_of_nonstuck
        (__run_evaluate a) (__run_evaluate b)
        hRunASmtTy hRunBSmtTy (by simpa [runLt] using hRunLtNe) with
      ⟨runA, runB, hRunA, hRunB⟩
    change
    ...
    · change
      ...
      rw [native_qsub_lt_zero_eq_eval]
      simp [__smtx_typeof_arith_overload_op_2_ret, __smtx_typeof]
    · have hARelValue :
          RuleProofs.smt_value_rel
            (__smtx_model_eval M (__eo_to_smt a))
        ...
        RuleProofs.smt_value_rel
          ...
          (__smtx_model_eval M
            (SmtTerm.Boolean
              (native_qlt (native_qplus runA (native_qneg runB))
                (native_mk_rational 0 1))))
      rw [native_qsub_lt_zero_eq_eval, __smtx_model_eval.eq_1]
      exact RuleProofs.smt_value_rel_refl _

private theorem run_evaluate_sound_apply_leq_core
    (M : SmtModel) (hM : model_total_typed M)
    (a b : Term)
    (rec :
      ∀ A : Term,
  ...
  · rcases hArgsInt with ⟨hATyInt, hBTyInt⟩
    ...
    dsimp [runLeq]
    rw [hRunA, hRunB, eo_leq_int_result_eq]
    constructor
    · change
        __smtx_typeof (SmtTerm.leq (__eo_to_smt a) (__eo_to_smt b)) =
    ...
    change
      __smtx_typeof (SmtTerm.leq (__eo_to_smt a) (__eo_to_smt b)) =
        ...
        RuleProofs.smt_value_rel
          ...
          (__smtx_model_eval M (__eo_to_smt runLeq))
    dsimp [runLeq]
    rw [hRunA, hRunB, eo_leq_real_result_eq]
    constructor
    · change
        __smtx_typeof (SmtTerm.leq (__eo_to_smt a) (__eo_to_smt b)) =
...
private theorem run_evaluate_sound_apply_gt_core
    (M : SmtModel) (hM : model_total_typed M)
    (a b : Term)
    (rec :
      ∀ A : Term,
...
private theorem run_evaluate_sound_apply_geq_core
    (M : SmtModel) (hM : model_total_typed M)
    (a b : Term)
    (rec :
      ∀ A : Term,
...
private theorem run_evaluate_sound_apply_str_at_core
    (M : SmtModel) (hM : model_total_typed M)
    (a b : Term)
    (rec :
      ∀ A : Term,
  ...
  have hBTrans : RuleProofs.eo_has_smt_translation b := by
    ...
    simp
  have hAProgTy : __eo_typeof (__eo_prog_evaluate a) = Term.Bool :=
    eo_prog_evaluate_typeof_bool_of_smt_type_seq a T hATransA hATy
  have hBProgTy : __eo_typeof (__eo_prog_evaluate b) = Term.Bool :=
    eo_prog_evaluate_typeof_bool_of_smt_type_int b hBTrans hBTy
  ...
  rcases eo_extract_same_seq_int_args_of_nonstuck runA runB
  ...
  have hRunAtEq :
    ...
    dsimp [runAt]
    rw [hRunA, hRunB]
    exact eo_extract_string_same_index str i
  have hSubValid :
      native_string_valid (native_str_substr str i 1) = true :=
    native_string_valid_substr_local i 1 hValid
  change
  ...
  · have hStrEval :
    ...
    have hBEval :
      ...
      smt_value_rel_numeral_eq _ i hBRelNumeral
    have hSubEval :
        __smtx_model_eval M
    ...
    rw [show
        __smtx_elem_typeof_seq_value (native_pack_string str) =
          SmtType.Char by
      simp [native_pack_string, elem_typeof_pack_seq_local]]
    rw [native_seq_extract_pack_string_one]
    exact RuleProofs.smtx_model_eval_eq_refl _

private theorem run_evaluate_sound_apply_str_prefixof_core
    (M : SmtModel) (hM : model_total_typed M)
    (a b : Term)
    (rec :
      ∀ A : Term,
  ...
  have hRunSliceNeConcrete :
    ...
    simpa [runSlice, runEnd, runLen, hRunA, __eo_len, __eo_add]
      using hRunSliceNe
  rcases eo_extract_seq_char_numeral_args_of_nonstuck runB 0
  ...
  have hRunSliceEq :
    ...
    rw [hRunA, hRunB]
    exact eo_extract_string_zero_len_minus_one sx sy
  have hRunPrefixEq :
      runPrefix =
        Term.Boolean
          (native_teq
  ...
  · have hSxEval :
    ...
    rw [show
      ...
      simp [native_pack_string, native_unpack_pack_seq_local, native_seq_len,
        native_str_len]]
    rw [show
    ...
    · have hSxSubNe :
      ...
      have hPackNe :
        ...
        intro hPack
        exact hSxSubNe (native_pack_string_injective hPack)
      simp [__smtx_model_eval_eq, native_veq, native_teq, hSubEq,
        hPackNe]

private theorem run_evaluate_sound_apply_str_suffixof_core
    (M : SmtModel) (hM : model_total_typed M)
    (a b : Term)
    (rec :
      ∀ A : Term,
  ...
  have hRunSliceEq :
    ...
    rw [hRunA, hRunB]
    exact eo_extract_string_suffix_window sx sy
  have hRunSuffixEq :
      runSuffix =
        Term.Boolean
          (native_teq
...
private theorem run_evaluate_sound_apply_str_leq_core
  ...
  have hRunANe : runA ≠ Term.Stuck := by
    intro hStuck
    rw [hStuck] at hRunATy
    rw [show __eo_to_smt Term.Stuck = SmtTerm.None by rfl] at hRunATy
    rw [show __smtx_typeof SmtTerm.None = SmtType.None by
      rw [__smtx_typeof.eq_def]] at hRunATy
    cases hRunATy
  have hRunBNe : runB ≠ Term.Stuck := by
    intro hStuck
    rw [hStuck] at hRunBTy
    rw [show __eo_to_smt Term.Stuck = SmtTerm.None by rfl] at hRunBTy
    rw [show __smtx_typeof SmtTerm.None = SmtType.None by
      rw [__smtx_typeof.eq_def]] at hRunBTy
    cases hRunBTy
  have hARelRun :
      RuleProofs.smt_value_rel
        (__smtx_model_eval M (__eo_to_smt a))
  ...
  · rcases hStrings with ⟨sx, sy, hRunA, hRunB⟩
    ...
    have hRunStringBTy :
      ...
      exact hRunBTy
    have hSxValid : native_string_valid sx = true :=
      native_string_valid_of_string_type hRunStringATy
    have hSyValid : native_string_valid sy = true :=
      native_string_valid_of_string_type hRunStringBTy
    let leqBool := native_or (decide (sx = sy)) (native_str_lt sx sy)
    have hRunLeqEq :
        runLeq = Term.Boolean leqBool := by
      dsimp [runLeq, leqBool]
      rw [hRunA, hRunB]
      exact str_leq_result_strings hSxValid hSyValid
    rw [hRunLeqEq]
    constructor
    · change
        __smtx_typeof (SmtTerm.str_leq (__eo_to_smt a) (__eo_to_smt b)) =
    ...
    · have hSxEval :
      ...
      rw [show
        ...
        rw [__smtx_model_eval.eq_1]]
      dsimp [leqBool]
      rw [smtx_model_eval_str_leq_pack_string sx sy]
      exact RuleProofs.smtx_model_eval_eq_refl _
  · have hRunLeqEq :
        runLeq =
          Term.Apply (Term.Apply (Term.UOp UserOp.str_leq) runA) runB := by
      dsimp [runLeq]
      exact str_leq_result_non_strings hStrings hRunANe hRunBNe
    rw [hRunLeqEq]
    constructor
    · change
        __smtx_typeof (SmtTerm.str_leq (__eo_to_smt a) (__eo_to_smt b)) =
      ...
      simp [hATyChar, hBTyChar, hRunATy, hRunBTy, native_Teq,
        native_ite]
    · have hRelLeq :=
        smt_value_rel_model_eval_str_leq_of_rel
          (__smtx_model_eval M (__eo_to_smt a))
          (__smtx_model_eval M (__eo_to_smt b))
...
private theorem run_evaluate_sound_apply_str_replace_core
    (M : SmtModel) (hM : model_total_typed M)
    (s pat repl : Term)
    (rec :
      ∀ A : Term,
  ...
  have hPatProgTy : __eo_typeof (__eo_prog_evaluate pat) = Term.Bool :=
    eo_prog_evaluate_typeof_bool_of_smt_type_seq pat T hPatTrans hPatTySeq
  have hReplProgTy : __eo_typeof (__eo_prog_evaluate repl) = Term.Bool :=
    eo_prog_evaluate_typeof_bool_of_smt_type_seq repl T hReplTrans hReplTySeq
  ...
  let runReplace :=
    __eo_ite (__eo_is_neg runIdx) runS
      (__eo_concat
        (__eo_concat
  ...
  have hRunReplTy :
      __smtx_typeof (__eo_to_smt runRepl) = SmtType.Seq T := by
    simpa [runRepl] using hReplSameTy.symm.trans hReplTySeq
  have hRunIdxNe : runIdx ≠ Term.Stuck := by
    intro hIdx
    apply hRunReplaceNe
    dsimp [runReplace]
    rw [hIdx]
    simp [__eo_is_neg, __eo_ite, native_ite, native_teq]
  rcases eo_find_to_str_seq_args_of_nonstuck runS runPat hRunSTy
  ...
  · have hRunReplaceEq : runReplace = Term.String str := by
    ...
    · have hRelReplace :=
        smt_value_rel_model_eval_str_replace_of_rel
          (__smtx_model_eval M (__eo_to_smt s))
          (__smtx_model_eval M (__eo_to_smt pat))
          (__smtx_model_eval M (__eo_to_smt repl))
      ...
      have hEvalRunReplace :
        ...
        | cons p ps =>
            ...
            rw [native_seq_indexof_map_char_zero str (p :: ps)]
      ...
      exact hRelReplace
  · rcases str_replace_run_repl_string_of_nonneg str patStr runRepl
        hRunReplTyChar hIdxNeg (by
          dsimp [runReplace, runIdx] at hRunReplaceNe
    ...
    have hRunReplaceEq :
        runReplace =
          Term.String
            (native_str_replace_eval_result str patStr replStr) := by
      dsimp [runReplace, runIdx]
      rw [hRunS, hRunPat, hRunRepl]
      exact str_replace_result_strings str patStr replStr
    rw [hRunReplaceEq]
    constructor
    · change
        __smtx_typeof
          ...
          __smtx_typeof
            (SmtTerm.String
              (native_str_replace_eval_result str patStr replStr))
      rw [typeof_str_replace_eq]
      have hResultValid :
          native_string_valid
              (native_str_replace_eval_result str patStr replStr) = true := by
        unfold native_str_replace_eval_result
      ...
      have hEvalRunReplace :
        ...
        rw [show
            __smtx_elem_typeof_seq_value (native_pack_string str) =
              SmtType.Char by
          simp [native_pack_string, elem_typeof_pack_seq_local]]
        rw [native_seq_replace_pack_string str patStr replStr]
      have hEvalReplace :
          __smtx_model_eval M
              (SmtTerm.str_replace (__eo_to_smt s) (__eo_to_smt pat)
                (__eo_to_smt repl)) =
...
private theorem run_evaluate_sound_apply_str_len_core
    ...
    (rec :
      ∀ A : Term,
        sizeOf A < sizeOf (Term.Apply (Term.UOp UserOp.str_len) b) ->
          RunEvaluateSoundGoal M A) :
  RunEvaluateSoundGoal M (Term.Apply (Term.UOp UserOp.str_len) b) := by
  intro hATrans hEvalTy
  have hLenNN :
  ...
  have hRunLenNe : runLen ≠ Term.Stuck := by
    ...
    change Term.Stuck = Term.Bool at hEvalTy
    cases hEvalTy
  rcases run_evaluate_rec_apply_arg M (Term.UOp UserOp.str_len) b rec
      hBTrans hBProgTy with
    ⟨hSameTy, hRel⟩
  have hSameTyRun :
      __smtx_typeof (__eo_to_smt b) =
  ...
  rcases eo_len_seq_arg_of_nonstuck runArg hRunTySeq
      (by simpa [runLen] using hRunLenNe) with
  ...
  · have hRelRun :
    ...
    have hRelString :
      ...
      exact hRelRun
    have hRelLen :=
      smt_value_rel_model_eval_str_len_of_rel
        (__smtx_model_eval M (__eo_to_smt b))
        (__smtx_model_eval M (__eo_to_smt (Term.String str)))
        hRelString
    have hEvalLenB :
    ...
    have hEvalLenString :
      ...
      simp [__smtx_model_eval_str_len, native_pack_string,
        native_unpack_pack_seq_local, native_seq_len, native_str_len]
    rw [hEvalLenB, hEvalLenString]
    exact hRelLen

private theorem run_evaluate_sound_apply_str_to_code_core
    (M : SmtModel) (hM : model_total_typed M)
    (b : Term)
    (rec :
      ∀ A : Term,
        sizeOf A < sizeOf (Term.Apply (Term.UOp UserOp.str_to_code) b) ->
          RunEvaluateSoundGoal M A) :
  RunEvaluateSoundGoal M (Term.Apply (Term.UOp UserOp.str_to_code) b) := by
  intro hATrans hEvalTy
  have hCodeNN :
  ...
  rcases run_evaluate_rec_apply_arg M (Term.UOp UserOp.str_to_code) b rec
      hBTrans hBProgTy with
    ⟨hSameTy, hRel⟩
  have hSameTyRun :
      __smtx_typeof (__eo_to_smt b) =
  ...
  rcases eo_len_seq_arg_of_nonstuck runArg hRunTyChar
      (by simpa [runLen] using hRunLenNe) with
  ...
  have hRunCode :
    ...
    rw [hRunArg]
    exact str_to_code_result_string hValid
  change
  ...
  · have hRelRun :
    ...
    have hRelString :
      ...
      exact hRelRun
    have hRelCode :=
      smt_value_rel_model_eval_str_to_code_of_rel
        (__smtx_model_eval M (__eo_to_smt b))
        (__smtx_model_eval M (__eo_to_smt (Term.String str)))
        hRelString
    have hEvalCodeB :
...
private theorem run_evaluate_sound_apply_str_from_code_core
    (M : SmtModel) (hM : model_total_typed M)
    (x : Term)
    (rec :
      ∀ A : Term,
  ...
  __run_evaluate (Term.Apply (Term.UOp UserOp.str_from_code) x) ≠
      Term.Apply (Term.UOp UserOp.str_from_code) x ->
  RunEvaluateSoundGoal M (Term.Apply (Term.UOp UserOp.str_from_code) x) := by
  ...
  have hXEoInt : __eo_typeof x = Term.UOp UserOp.Int :=
    TranslationProofs.eo_to_smt_type_eq_int
      (hXMatch.symm.trans hxSmtTy)
  let runFrom := eo_eval_str_from_code_rhs x
  have hRunFromActive :
  ...
  case Numeral runN =>
    have hRunFromString :
      ...
      eo_eval_str_from_code_rhs_run_numeral_eq_of_active
  ...
  all_goals
    ...
    dsimp [runFrom, eo_eval_str_from_code_rhs]
    rw [hRunX]
...
private theorem run_evaluate_sound_apply_ubv_to_int_core
  ...
  rcases bv_unop_ret_arg_of_non_none
      ...
      (by rw [__smtx_typeof.eq_132]) hUbvToIntNN with
    ⟨w, hxSmtTy⟩
  have hXTrans : RuleProofs.eo_has_smt_translation x := by
    unfold RuleProofs.eo_has_smt_translation
    rw [hxSmtTy]
...
private theorem run_evaluate_sound_apply_sbv_to_int_core
    (M : SmtModel) (hM : model_total_typed M)
    (x : Term)
    (rec :
      ∀ A : Term,
        sizeOf A < sizeOf (Term.Apply (Term.UOp UserOp.sbv_to_int) x) ->
          RunEvaluateSoundGoal M A) :
  RunEvaluateSoundGoal M (Term.Apply (Term.UOp UserOp.sbv_to_int) x) := by
  intro hATrans hEvalTy
  have hSbvToIntNN :
      term_has_non_none_type
  ...
  have hSbvToIntEoType :
    ...
    rw [hXEoBv]
    rfl
  let runToInt := eo_eval_sbv_to_int_rhs x
  have hRunToIntNe : runToInt ≠ Term.Stuck := by
    intro hStuck
    change
      __eo_typeof
  ...
  · subst w
    have hRunToIntEq : runToInt = Term.Numeral 0 := by
      ...
      simpa [native_nat_to_int, SmtEval.native_nat_to_int] using
        eo_eval_sbv_to_int_rhs_eq_zero_of_run_typeof_zero x hRunEoBv
    rcases model_eval_bitvec_term_binary M hM (__run_evaluate x) 0
        hRunXSmtTy with
      ...
      rw [show
          __smtx_model_eval M
            ...
            __smtx_model_eval_int_to_bv
              (SmtValue.Numeral i)
              (__smtx_model_eval M (__eo_to_smt x)) by
        rw [__smtx_model_eval.eq_131, __smtx_model_eval.eq_2]]
      rw [hXEval]
      change
        RuleProofs.smt_value_rel
          (SmtValue.Binary i

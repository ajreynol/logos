...
/-- Inverse of `listConcatRecSmtType`: if the concatenation of two n-ary XOR
    lists is well-typed as a bit-vector, then so are both operands. -/
theorem listConcatRecSmtType_inv (a z : Term) (w : Nat) :
    __eo_is_list (Term.UOp UserOp.bvxor) a = Term.Boolean true ->
    __eo_is_list (Term.UOp UserOp.bvxor) z = Term.Boolean true ->
    __smtx_typeof (__eo_to_smt (__eo_list_concat_rec a z)) =
        SmtType.BitVec w ->
  ...
  | case3 f x xs z hZNe ih =>
      ...
      have hXsList :
        ...
        eo_is_list_tail_true_of_cons_self
          (Term.UOp UserOp.bvxor) x xs hAList
      have hZNeStuck : z ≠ Term.Stuck := is_list_true_ne_stuck hZList
      have hTailNe : __eo_list_concat_rec xs z ≠ Term.Stuck :=
        eo_list_concat_rec_ne_stuck_of_list
          (Term.UOp UserOp.bvxor) xs z hXsList hZNeStuck
      rw [eo_list_concat_rec_cons_eq_of_tail_ne_stuck
        (Term.UOp UserOp.bvxor) x xs z hTailNe] at hTy
      have hArgs := bvxor_args_of_bitvec_type x (__eo_list_concat_rec xs z) w
        (by simpa [xor] using hTy)
      have hRec := ih hXsList hZList hArgs.2
      refine ⟨?_, hRec.2⟩
      have hConsTy :
          __smtx_typeof
              (__eo_to_smt
        ...
        rw [__smtx_typeof.eq_43]
        simp [__smtx_typeof_bv_op_2, hArgs.1, hRec.1,
          native_nateq, native_ite]
      exact hConsTy
  | case4 nil z hNil hZNe hNot =>
      intro hTy
      have hNilTrue :
        ...
        exact eo_requires_eq_of_ne_stuck
          (__eo_is_list_nil (Term.UOp UserOp.bvxor) nil)
          (Term.Boolean true) nil hReq
      have hNilZ : __eo_list_concat_rec nil z = z := by
        cases nil <;> cases z <;>
          simp [__eo_is_list_nil, __eo_list_concat_rec] at hNilTrue ⊢
      rw [hNilZ] at hTy
      have hNilTy : __smtx_typeof (__eo_to_smt nil) = SmtType.BitVec w := by
        cases nil <;> cases z <;>
          simp_all [__eo_is_list_nil, __eo_list_concat_rec]
      exact ⟨hNilTy, hTy⟩

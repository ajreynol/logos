...
private theorem eo_to_smt_appSpineRev_dt_sel
...
private theorem eo_type_eq_of_eq_bool_or_same
    (x y : Term) :
    EqBoolOrSame x y ->
    __eo_typeof x = __eo_typeof y := by
  intro h
...
private theorem eo_type_eq_of_eq_true_or_same
    (M : SmtModel) (x y : Term) :
    EqTrueOrSame M x y ->
    __eo_typeof x = __eo_typeof y := by
  intro h
...
private theorem native_string_valid_append_left
  ...
  have hAll :
    ...
    simpa [native_string_valid] using h
  exact by
    have hLeft := (Bool.and_eq_true.mp hAll).1
    simpa [native_string_valid] using hLeft
...
private theorem native_string_valid_append_right
  ...
  have hAll :
    ...
    simpa [native_string_valid] using h
  exact by
    have hRight := (Bool.and_eq_true.mp hAll).2
    simpa [native_string_valid] using hRight

private theorem native_string_valid_drop :
    ∀ (xs : List native_Char) (n : Nat),
      native_string_valid xs = true ->
        native_string_valid (xs.drop n) = true
  | xs, 0, h => by
      simpa using h
  | [], _ + 1, _h => by
      simp [native_string_valid]
  | c :: cs, n + 1, h => by
      exact native_string_valid_drop cs n
        (native_string_valid_cons_parts h).2
...
private theorem native_str_in_re_mk_union
  ...
  cases hValid : native_string_valid str <;>
    simp [native_str_in_re, native_list_in_re, hValid,
      native_list_in_re_mk_union]
...
private theorem native_str_in_re_re_concat_congr
  ...
  ·
    have h₁List :
      ...
      intro ys hys
      simpa [native_str_in_re, native_list_in_re, hys] using h₁ ys
    have h₂List :
        ∀ ys : List native_Char,
          native_string_valid ys = true ->
            native_list_in_re ys r₂ = native_list_in_re ys r₂' := by
...
private theorem native_str_in_re_re_diff
  ...
  cases hValid : native_string_valid s <;>
    simp [native_str_in_re, native_re_diff, native_re_inter,
      native_re_comp, hValid, native_list_in_re, native_list_in_re_mk_inter,
      native_str_in_re_mk_comp_list]
...
private theorem native_list_in_re_mk_star_congr
  ...
  exact native_list_in_re_raw_star_congr xs r r' hExt

private theorem native_str_in_re_re_mult_congr
  ...
  have hList :
    ...
    simpa [native_str_in_re, native_list_in_re] using
      hExt (ys)
  simpa [native_str_in_re, native_list_in_re, native_re_mult] using
    native_list_in_re_mk_star_congr str r r' hList

private theorem native_str_in_re_re_plus_congr
    (str : native_String) (r r' : native_RegLan)
    (hExt :
      ∀ s : native_String,
        native_str_in_re s r = native_str_in_re s r') :
...
private theorem native_str_ext_to_list_ext
    ...
    ∀ ys : List native_Char,
      native_list_in_re ys r = native_list_in_re ys r' := by
  intro ys
  simpa [native_str_in_re, native_list_in_re] using hExt (ys)
...
private theorem native_str_indexof_re_congr
  ...
  · simp [native_str_indexof_re, hLt,
      native_re_find_idx_from_congr r r' s (Int.toNat i) hExtList]
...
private theorem native_str_replace_re_congr
  ...
  simp [native_str_replace_re,
    native_re_find_nonempty_idx_from_congr r r' s 0 hExtList]
...
private theorem native_str_replace_re_all_congr
  ...
  simp [native_str_replace_re_all,
    native_re_replace_all_nonempty_list_congr r r' replacement
      s hExtList]
...
private theorem congTrueSpine_eotype_non_reg_unop_eq_true_of_eval_congr
    (M : SmtModel) (hM : model_total_typed M)
    (eoOp : UserOp)
    (hArgOfNN :
      ∀ a,
  ...
  | Datatype s d =>
      ...
      · subst s
        ...
        | Numeral n =>
            ...
            · exfalso
              apply hNN
              simp [__eo_to_smt_tuple_select, hTy, hIdx, hNonneg, native_ite]
            · have hTerm :
              ...
              exact ⟨SmtType.Datatype (native_string_lit "@Tuple") d, rfl, by simp, by simp⟩
        | _ =>
            exfalso
            apply hNN
            simp [__eo_to_smt_tuple_select, hTy, hIdx]
...
private theorem congTrueSpine_same_generic_head_apply_eq_true
    (M : SmtModel) (hM : model_total_typed M)
    (f x y : Term)
    (hToSmt :
      ∀ a,
...
private theorem mkSmtAppSpineRev_cons_ne_dt_sel
    (F x : SmtTerm) :
    ∀ xs s d i j,
...
private theorem mkSmtAppSpineRev_cons_ne_dt_tester
    (F x : SmtTerm) :
    ∀ xs s d i,
...
private theorem dt_sel_arg_non_reg_of_non_none_core
...
private theorem congTypeSpine_appSpineRev_dt_sel_eq_has_bool_type
...
private theorem congTrueSpine_appSpineRev_dt_sel_eq_true
    (M : SmtModel) (hM : model_total_typed M)
    (s : native_String) (d : Datatype) (i j : native_Nat) (t rhs : Term)
    (hHead : (appSpineRev t).1 = Term.DtSel s d i j)
    (hEqBool : RuleProofs.eo_has_bool_type (mkEq t rhs))
...
private theorem updater_args_non_reg_of_non_none
    (sel t u : SmtTerm) :
...
private theorem eo_to_smt_updater_eval_congr
...
private theorem congTrueSpine_non_reg_indexed_binary_uop1_eq_true
    (M : SmtModel) (hM : model_total_typed M)
    (eoOp : UserOp1) (idx : Term)
    (smtOp : SmtTerm -> SmtTerm -> SmtTerm)
    (hToSmt :
...
private theorem congTrueSpine_tuple_update_eq_true
    (M : SmtModel) (hM : model_total_typed M)
    (idx x₁ x₂ rhs : Term) :
    RuleProofs.eo_has_bool_type
      (mkEq
...
private theorem congTypeSpine_tuple_eq_has_bool_type
    (x₁ x₂ rhs : Term) :
    RuleProofs.eo_has_smt_translation
...
private theorem congTrueSpine_tuple_eq_true
    (M : SmtModel) (hM : model_total_typed M)
    (x₁ x₂ rhs : Term) :
    RuleProofs.eo_has_bool_type
...
private theorem congTrueSpine_re_mult_eq_true
    (M : SmtModel) (hM : model_total_typed M) (x rhs : Term) :
    RuleProofs.eo_has_bool_type
...
private theorem congTrueSpine_re_plus_eq_true
    (M : SmtModel) (hM : model_total_typed M) (x rhs : Term) :
    RuleProofs.eo_has_bool_type
...
private theorem eo_to_smt_strings_deq_diff_ne_dt_sel
    (a b : Term) :
    ∀ s d i j,
      __eo_to_smt (Term.UOp2 UserOp2._at_strings_deq_diff a b) ≠
        SmtTerm.DtSel s d i j := by
  intro s d i j h
  change
          ...
          | @app f g x y hFn hArg =>
                  ...
                  exact congTypeSpine_typecongr_indexed_unop_eq_has_bool_type
                    ...
                    (by
                      ...
                      cases __eo_to_smt_tester (__eo_to_smt c) <;>
                        simp [__smtx_typeof, h])
                    x (Term.Apply g y) hTrans hApp
              | Term.UOp1 UserOp1._at_strings_stoi_result x₁ =>
                  exact congTypeSpine_strings_stoi_result_eq_has_bool_type
              ...
              | Term.UOp1 UserOp1.tuple_update i =>
                  exact False.elim
                    (no_translation_of_eo_apply_none_head
                      ...
                      (by rfl) hTrans)
              | Term.UOp1 UserOp1._at_strings_replace_all_result i =>
                  exact False.elim
                    (no_translation_of_eo_apply_none_head
                      (f :=
                        Term.UOp1 UserOp1._at_strings_replace_all_result i)
                      (x := x) (by rfl) hTrans)
              | Term.UOp2 UserOp2._at_strings_num_occur_re i j =>
                  exact False.elim
                    (no_translation_of_eo_apply_none_head
                      (f := Term.UOp2 UserOp2._at_strings_num_occur_re i j)
                      (x := x) (by rfl) hTrans)
              | Term.UOp2 UserOp2._at_strings_occur_index_re i j =>
                  exact False.elim
                    (no_translation_of_eo_apply_none_head
                      (f :=
                        Term.UOp2 UserOp2._at_strings_occur_index_re i j)
                        ...
                        congTypeSpine_same_generic_head_apply_eq_has_bool_type
                          ...
                          (generic_apply_type_of_non_datatype_head
                            ...
                            (by
                              ...
                              cases h))
                          hTrans hArg
              | Term.UOp1 UserOp1._at_strings_stoi_non_digit z =>
                  cases hFn with
                  | refl _ =>
                      exact
                        congTypeSpine_same_generic_head_apply_eq_has_bool_type
                          ...
                          (generic_apply_type_of_non_datatype_head
                            ...
                            (by
                              ...
                              cases h))
                          hTrans hArg
              | Term.UOp2 UserOp2._at_array_deq_diff q idx =>
                  cases hFn with
                  | refl _ =>
                      exact
                        congTypeSpine_same_generic_head_apply_eq_has_bool_type
                          ...
                          (by intro a; rfl)
                          (generic_apply_type_of_non_datatype_head
                            (eo_to_smt_array_deq_diff_ne_dt_sel q idx)
                            (eo_to_smt_array_deq_diff_ne_dt_tester q idx))
                          (generic_apply_type_of_non_datatype_head
                            (eo_to_smt_array_deq_diff_ne_dt_sel q idx)
                            (eo_to_smt_array_deq_diff_ne_dt_tester q idx))
                          hTrans hArg
                        ...
                        congTypeSpine_same_generic_head_apply_eq_has_bool_type
                          ...
                          (generic_apply_type_of_non_datatype_head
                            ...
                            (eo_to_smt_at_bv_ne_dt_tester
                              (__eo_to_smt n) (__eo_to_smt w)))
                          hTrans hArg
              | Term.UOp2 UserOp2._at_strings_deq_diff a b =>
                  cases hFn with
                  | refl _ =>
                      exact
                        congTypeSpine_same_generic_head_apply_eq_has_bool_type
                          ...
                          (by intro q; rfl)
                          (generic_apply_type_of_non_datatype_head
                            (eo_to_smt_strings_deq_diff_ne_dt_sel a b)
                            (eo_to_smt_strings_deq_diff_ne_dt_tester a b))
                          (generic_apply_type_of_non_datatype_head
                            (eo_to_smt_strings_deq_diff_ne_dt_sel a b)
                            (eo_to_smt_strings_deq_diff_ne_dt_tester a b))
                          hTrans hArg
                      ...
                      exact
                        congTypeSpine_same_generic_head_apply_eq_has_bool_type
                          ...
                          (generic_apply_type_of_non_datatype_head
                            ...
                            (eo_to_smt_quant_skolemize_top_ne_dt_tester q idx))
                          hTrans hArg
              | Term.UOp2 UserOp2._at_sets_deq_diff q idx =>
                  cases hFn with
                  | refl _ =>
                      exact
                        congTypeSpine_same_generic_head_apply_eq_has_bool_type
                          ...
                          (by intro a; rfl)
                          (generic_apply_type_of_non_datatype_head
                            (eo_to_smt_sets_deq_diff_ne_dt_sel q idx)
                            (eo_to_smt_sets_deq_diff_ne_dt_tester q idx))
                          (generic_apply_type_of_non_datatype_head
                            (eo_to_smt_sets_deq_diff_ne_dt_sel q idx)
                            (eo_to_smt_sets_deq_diff_ne_dt_tester q idx))
                          hTrans hArg
              | Term.UOp3 UserOp3._at_re_unfold_pos_component str re idx =>
                  ...
                  | refl _ =>
                      exact
                        congTypeSpine_same_generic_head_apply_eq_has_bool_type
                          (Term.UOp3 UserOp3._at_re_unfold_pos_component
                            str re idx)
                          x y
                          (by intro a; rfl)
                          (generic_apply_type_of_non_datatype_head
                              ...
                              exact congTrueSpine_bv_pred_to_bv_eq_true M hM
                                ...
                                (by
                                  ...
                                  rw [bvPredToBv, bvPredToBvEval,
                                    __smtx_model_eval.eq_133,
                                    __smtx_model_eval.eq_55,
                                    __smtx_model_eval.eq_5,
                                    __smtx_model_eval.eq_5])
                                z x (Term.Apply g y) hEqBool hApp
                            case bvsltbv =>

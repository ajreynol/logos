...
private theorem congTrueSpine_head_eq_of_not_app
    (M : SmtModel) {head rhs : Term}
    (hHead : ∀ f x, head ≠ Term.Apply f x) :
    CongTrueSpine M head rhs ->
    rhs = head := by
  intro h
  cases h with
  | refl _ => rfl
...
private theorem congTypeSpine_head_eq_of_not_app
    {head rhs : Term}
    (hHead : ∀ f x, head ≠ Term.Apply f x) :
    CongTypeSpine head rhs ->
    rhs = head := by
  intro h
  cases h with
  | refl _ => rfl
...
private theorem congTrueSpine_unary_non_app_inv
    (M : SmtModel) (head x rhs : Term)
    (hHead : ∀ f x, head ≠ Term.Apply f x) :
    CongTrueSpine M (Term.Apply head x) rhs ->
    ∃ y,
      rhs = Term.Apply head y ∧
  ...
  | app hHeadSpine hArg =>
      have hg := congTrueSpine_head_eq_of_not_app M hHead hHeadSpine
      subst hg
      exact ⟨_, rfl, Or.inr hArg⟩

private theorem congTypeSpine_unary_non_app_inv
    (head x rhs : Term)
    (hHead : ∀ f x, head ≠ Term.Apply f x) :
    CongTypeSpine (Term.Apply head x) rhs ->
    ∃ y,
      rhs = Term.Apply head y ∧
        EqBoolOrSame x y := by
  intro h
  ...
  | refl _ =>
      exact ⟨x, rfl, Or.inl rfl⟩
  | app hHeadSpine hArg =>
      have hg := congTypeSpine_head_eq_of_not_app hHead hHeadSpine
      subst hg
      exact ⟨_, rfl, Or.inr hArg⟩
...
private theorem congTrueSpine_non_reg_non_app_unop_eq_true
    (M : SmtModel) (hM : model_total_typed M)
    (head : Term) (smtOp : SmtTerm -> SmtTerm)
    (evalOp : SmtValue -> SmtValue)
    (hHead : ∀ f x, head ≠ Term.Apply f x)
    (hToSmt :
      ∀ a,
        __eo_to_smt (Term.Apply head a) =
          smtOp (__eo_to_smt a))
    (hArgOfNN :
      ∀ a,
        __smtx_typeof (smtOp a) ≠ SmtType.None ->
    ...
    (hEval :
      ∀ a,
        __smtx_model_eval M (smtOp a) =
          evalOp (__smtx_model_eval M a))
    (x rhs : Term) :
    RuleProofs.eo_has_bool_type (mkEq (Term.Apply head x) rhs) ->
    CongTrueSpine M (Term.Apply head x) rhs ->
    eo_interprets M (mkEq (Term.Apply head x) rhs) true := by
  intro hEqBool hSpine
  rcases congTrueSpine_unary_non_app_inv M head x rhs hHead hSpine with
    ⟨y, hRhs, hArg⟩
  subst hRhs
  apply RuleProofs.eo_interprets_eq_of_rel M
  · exact hEqBool
  · have hTypes :=
      RuleProofs.eo_eq_operands_same_smt_type_of_has_bool_type
        (Term.Apply head x) (Term.Apply head y) hEqBool
    have hxOpNN :
        __smtx_typeof (smtOp (__eo_to_smt x)) ≠ SmtType.None := by
      rw [← hToSmt x]
      exact hTypes.2
...
private theorem congTypeSpine_typecongr_non_app_unop_eq_has_bool_type
    (head : Term) (smtOp : SmtTerm -> SmtTerm)
    (hHead : ∀ f x, head ≠ Term.Apply f x)
    (hToSmt :
      ∀ a,
    ...
    (hTypeCong :
      ∀ a b,
        __smtx_typeof a = __smtx_typeof b ->
          __smtx_typeof (smtOp a) = __smtx_typeof (smtOp b))
    (x rhs : Term) :
    RuleProofs.eo_has_smt_translation (Term.Apply head x) ->
    CongTypeSpine (Term.Apply head x) rhs ->
    RuleProofs.eo_has_bool_type (mkEq (Term.Apply head x) rhs) := by
  intro hTrans hSpine
  rcases congTypeSpine_unary_non_app_inv head x rhs hHead hSpine with
    ⟨y, hRhs, hArg⟩
  subst hRhs
  have hArgTy :
      __smtx_typeof (__eo_to_smt x) =
  ...
  have hOpTy :
      __smtx_typeof (__eo_to_smt (Term.Apply head x)) =
        __smtx_typeof (__eo_to_smt (Term.Apply head y)) := by
    rw [hToSmt x, hToSmt y]
    exact hTypeCong (__eo_to_smt x) (__eo_to_smt y) hArgTy
  exact RuleProofs.eo_has_bool_type_eq_of_same_smt_type
    (Term.Apply head x) (Term.Apply head y) hOpTy hTrans
...
private theorem smt_tester_apply_type_congr
    (c a b : SmtTerm)
    (h : __smtx_typeof a = __smtx_typeof b) :
    __smtx_typeof (SmtTerm.Apply (__eo_to_smt_tester c) a) =
      __smtx_typeof (SmtTerm.Apply (__eo_to_smt_tester c) b) := by
  by_cases hCons : ∃ s d i, c = SmtTerm.DtCons s d i
  · rcases hCons with ⟨s, d, i, rfl⟩
    change
      __smtx_typeof (SmtTerm.Apply (SmtTerm.DtTester s d i) a) =
        __smtx_typeof (SmtTerm.Apply (SmtTerm.DtTester s d i) b)
    rw [typeof_dt_tester_apply_eq, typeof_dt_tester_apply_eq, h]
  · cases c <;> simp [__eo_to_smt_tester, __smtx_typeof,
      __smtx_typeof_apply, h] at hCons ⊢

private theorem smt_tester_apply_arg_non_reg_of_non_none
    (c a : SmtTerm) :
    __smtx_typeof (SmtTerm.Apply (__eo_to_smt_tester c) a) ≠
      SmtType.None ->
    ∃ A,
      __smtx_typeof a = A ∧ A ≠ SmtType.None ∧ A ≠ SmtType.RegLan := by
  intro hNN
  by_cases hCons : ∃ s d i, c = SmtTerm.DtCons s d i
  · rcases hCons with ⟨s, d, i, rfl⟩
    change __smtx_typeof (SmtTerm.Apply (SmtTerm.DtTester s d i) a) ≠
      SmtType.None at hNN
    rw [typeof_dt_tester_apply_eq] at hNN
    rcases typeof_apply_non_none_cases hNN with
      ⟨A, B, hHead, hArg, hANN, _hBNN⟩
    rcases hHead with hHead | hHead
    · cases hHead
      exact ⟨SmtType.Datatype s d, hArg, by simp, by simp⟩
    · cases hHead
  · cases c <;> simp [__eo_to_smt_tester, __smtx_typeof,
      __smtx_typeof_apply] at hCons hNN

private noncomputable def smtTesterApplyEval
    (M : SmtModel) (c : SmtTerm) (v : SmtValue) : SmtValue :=
  match c with
  | SmtTerm.DtCons s d i => __smtx_model_eval_dt_tester s d i v
  | _ =>
      __smtx_model_eval_apply (__smtx_model_eval M (__eo_to_smt_tester c)) v

private theorem smt_tester_apply_eval_eq
    (M : SmtModel) (c a : SmtTerm) :
    __smtx_model_eval M (SmtTerm.Apply (__eo_to_smt_tester c) a) =
      smtTesterApplyEval M c (__smtx_model_eval M a) := by
  by_cases hCons : ∃ s d i, c = SmtTerm.DtCons s d i
  · rcases hCons with ⟨s, d, i, rfl⟩
    change
      __smtx_model_eval M (SmtTerm.Apply (SmtTerm.DtTester s d i) a) =
        __smtx_model_eval_dt_tester s d i (__smtx_model_eval M a)
    rw [__smtx_model_eval.eq_139]
  · cases c <;> simp [smtTesterApplyEval, __eo_to_smt_tester,
      __smtx_model_eval] at hCons ⊢
...
private theorem bvredand_arg_non_reg_of_non_none (a : SmtTerm) :
  ...
  rcases bv_binop_ret_args_non_reg_of_non_none SmtTerm.bvcomp
      (SmtType.BitVec 1)
      (by intro x y; rw [__smtx_typeof.eq_44])
...
private theorem bvredor_arg_non_reg_of_non_none (a : SmtTerm) :
  ...
  have hInnerNN :
    ...
    rcases bv_unop_args_non_reg_of_non_none SmtTerm.bvnot
        (by intro x; rw [__smtx_typeof.eq_37])
  ...
  congTrueSpine_non_reg_unop_eq_true_of_eval_congr M hM UserOp.bvredand
    ...
    (by
      ...
      rw [bvRedandTerm, bvAllOnesTerm, bvZeroTerm, bvRedandTerm,
        bvAllOnesTerm, bvZeroTerm, __smtx_model_eval.eq_44,
        __smtx_model_eval.eq_44, __smtx_model_eval.eq_37,
...
private theorem congTypeSpine_bvredand_eq_has_bool_type
  ...
  congTypeSpine_typecongr_unop_eq_has_bool_type UserOp.bvredand
    ...
    (by
      intro a b h
      rw [bvRedandTerm, bvAllOnesTerm, bvZeroTerm, bvRedandTerm,
        bvAllOnesTerm, bvZeroTerm, __smtx_typeof.eq_44,
        __smtx_typeof.eq_44, __smtx_typeof.eq_37, __smtx_typeof.eq_37,
  ...
  congTrueSpine_non_reg_unop_eq_true_of_eval_congr M hM UserOp.bvredor
    ...
    (by
      ...
      rw [bvRedorTerm, bvZeroTerm, bvRedorTerm, bvZeroTerm,
        __smtx_model_eval.eq_37, __smtx_model_eval.eq_37,
        __smtx_model_eval.eq_44, __smtx_model_eval.eq_44,
...
private theorem congTypeSpine_bvredor_eq_has_bool_type
  ...
  congTypeSpine_typecongr_unop_eq_has_bool_type UserOp.bvredor
    ...
    (by
      intro a b h
      rw [bvRedorTerm, bvZeroTerm, bvRedorTerm, bvZeroTerm,
        __smtx_typeof.eq_37, __smtx_typeof.eq_37, __smtx_typeof.eq_44,
        __smtx_typeof.eq_44, __smtx_typeof.eq_5, __smtx_typeof.eq_5, h])
    x rhs
...
private def bvAtBitTerm (idx a : SmtTerm) : SmtTerm :=
  SmtTerm.eq (SmtTerm.extract idx idx a) (SmtTerm.Binary 1 1)

private noncomputable def bvAtBitEval (idx a : SmtValue) : SmtValue :=
  __smtx_model_eval_eq (__smtx_model_eval_extract idx idx a)
    (SmtValue.Binary 1 1)

private theorem bvAtBit_arg_non_reg_of_non_none (idx a : SmtTerm) :
    __smtx_typeof (bvAtBitTerm idx a) ≠ SmtType.None ->
      ∃ A,
        __smtx_typeof a = A ∧
          A ≠ SmtType.None ∧ A ≠ SmtType.RegLan := by
  intro hNN
  have hEqNN :
      __smtx_typeof_eq
          (__smtx_typeof (SmtTerm.extract idx idx a))
          (SmtType.BitVec 1) ≠
        SmtType.None := by
    simpa [bvAtBitTerm, typeof_eq_eq, typeof_binary_one_eq] using hNN
  have hExtNN :
      __smtx_typeof (SmtTerm.extract idx idx a) ≠ SmtType.None :=
    (cong_smtx_typeof_eq_non_none hEqNN).2
  have hTerm : term_has_non_none_type (SmtTerm.extract idx idx a) := by
    unfold term_has_non_none_type
    exact hExtNN
  rcases extract_args_of_non_none hTerm with
    ⟨_i, _j, w, _hHi, _hLo, hArg, _hj0, _hji, _hiw⟩
  exact ⟨SmtType.BitVec w, hArg, by simp, by simp⟩

private theorem congTrueSpine_bv_at_bit_eq_true
    (M : SmtModel) (hM : model_total_typed M)
    (idx x rhs : Term) :
    RuleProofs.eo_has_bool_type
      (mkEq (Term.Apply (Term._at_bit idx) x) rhs) ->
    CongTrueSpine M (Term.Apply (Term._at_bit idx) x) rhs ->
    eo_interprets M (mkEq (Term.Apply (Term._at_bit idx) x) rhs) true :=
  congTrueSpine_non_reg_non_app_unop_eq_true M hM (Term._at_bit idx)
    (bvAtBitTerm (__eo_to_smt idx))
    (bvAtBitEval (__smtx_model_eval M (__eo_to_smt idx)))
    (by intro f x h; cases h)
    (by intro a; rfl)
    (bvAtBit_arg_non_reg_of_non_none (__eo_to_smt idx))
    (by
      intro a
      rw [bvAtBitTerm, bvAtBitEval, __smtx_model_eval.eq_133,
        __smtx_model_eval.eq_35, __smtx_model_eval.eq_5])
    x rhs

private theorem congTypeSpine_bv_at_bit_eq_has_bool_type
    (idx x rhs : Term) :
    RuleProofs.eo_has_smt_translation
      (Term.Apply (Term._at_bit idx) x) ->
    CongTypeSpine (Term.Apply (Term._at_bit idx) x) rhs ->
    RuleProofs.eo_has_bool_type
      (mkEq (Term.Apply (Term._at_bit idx) x) rhs) :=
  congTypeSpine_typecongr_non_app_unop_eq_has_bool_type
    (Term._at_bit idx)
    (bvAtBitTerm (__eo_to_smt idx))
    (by intro f x h; cases h)
    (by intro a; rfl)
    (by
      intro a b h
      rw [bvAtBitTerm, bvAtBitTerm, typeof_eq_eq, typeof_eq_eq,
        typeof_extract_eq, typeof_extract_eq, h])
    x rhs
  ...
  · let C : SmtTerm := __eo_to_smt c
    ...
    change
      RuleProofs.smt_value_rel
        ...
        (__smtx_model_eval M (bvIteTerm C' T' E'))
    rw [bvIteTerm, bvIteTerm, __smtx_model_eval.eq_132,
      __smtx_model_eval.eq_132, __smtx_model_eval.eq_133,
      __smtx_model_eval.eq_133, hCondEval]
    ...
    change
      __smtx_model_eval_eq
        ...
        (__smtx_model_eval M (SmtTerm.str_in_re Y₁ Y₂)) =
          SmtValue.Boolean true
    rw [__smtx_model_eval.eq_117, __smtx_model_eval.eq_117,
  ...
  have hEqNN :
    ...
    change
      __smtx_typeof
        ...
        SmtType.None at hNN
    rwa [typeof_eq_eq, __smtx_typeof.eq_120] at hNN
...
private theorem congTypeSpine_seq_unit_eq_has_bool_type
    (x rhs : Term) :
    RuleProofs.eo_has_smt_translation (Term.Apply (Term.UOp UserOp.seq_unit) x) ->
    CongTypeSpine (Term.Apply (Term.UOp UserOp.seq_unit) x) rhs ->
    RuleProofs.eo_has_bool_type
      (mkEq (Term.Apply (Term.UOp UserOp.seq_unit) x) rhs) :=
  congTypeSpine_typecongr_unop_eq_has_bool_type UserOp.seq_unit
    SmtTerm.seq_unit
    (by intro a; rfl)
    (by
      intro a b h
...
private theorem congTypeSpine_set_singleton_eq_has_bool_type
    (x rhs : Term) :
    RuleProofs.eo_has_smt_translation
      (Term.Apply (Term.UOp UserOp.set_singleton) x) ->
    CongTypeSpine (Term.Apply (Term.UOp UserOp.set_singleton) x) rhs ->
    RuleProofs.eo_has_bool_type
      (mkEq (Term.Apply (Term.UOp UserOp.set_singleton) x) rhs) :=
  congTypeSpine_typecongr_unop_eq_has_bool_type UserOp.set_singleton
...
private theorem congTypeSpine_set_member_eq_has_bool_type
    (x₁ x₂ rhs : Term) :
    RuleProofs.eo_has_smt_translation
    ...
    CongTypeSpine
      ...
      rhs ->
    RuleProofs.eo_has_bool_type
      (mkEq (Term.Apply (Term.Apply (Term.UOp UserOp.set_member) x₁) x₂)
        rhs) :=
  congTypeSpine_typecongr_binop_eq_has_bool_type UserOp.set_member
    SmtTerm.set_member
    (by intro a b; rfl)
    (by
      intro a b a' b' ha hb
...
private theorem congTypeSpine_select_eq_has_bool_type
    (x₁ x₂ rhs : Term) :
    RuleProofs.eo_has_smt_translation
    ...
    CongTypeSpine
      ...
      rhs ->
    RuleProofs.eo_has_bool_type
      (mkEq (Term.Apply (Term.Apply (Term.UOp UserOp.select) x₁) x₂)
        rhs) :=
  congTypeSpine_typecongr_binop_eq_has_bool_type UserOp.select
    SmtTerm.select
    (by intro a b; rfl)
    (by
      intro a b a' b' ha hb
...
private theorem congTypeSpine_store_eq_has_bool_type
    (x₁ x₂ x₃ rhs : Term) :
    RuleProofs.eo_has_smt_translation
      (Term.Apply (Term.Apply (Term.Apply (Term.UOp UserOp.store) x₁) x₂)
        x₃) ->
    CongTypeSpine
      (Term.Apply (Term.Apply (Term.Apply (Term.UOp UserOp.store) x₁) x₂)
        x₃) rhs ->
    RuleProofs.eo_has_bool_type
      (mkEq
        (Term.Apply (Term.Apply (Term.Apply (Term.UOp UserOp.store) x₁) x₂)
          x₃) rhs) :=
  congTypeSpine_typecongr_ternop_eq_has_bool_type UserOp.store
    SmtTerm.store
    (by intro a b c; rfl)
    (by
      intro a b c a' b' c' ha hb hc
...
private theorem congTypeSpine_str_replace_re_eq_has_bool_type
    (x₁ x₂ x₃ rhs : Term) :
    RuleProofs.eo_has_smt_translation
      (Term.Apply
    ...
    RuleProofs.eo_has_bool_type
      (mkEq
        (Term.Apply
          (Term.Apply (Term.Apply (Term.UOp UserOp.str_replace_re) x₁) x₂)
          x₃) rhs) :=
  congTypeSpine_typecongr_ternop_eq_has_bool_type UserOp.str_replace_re
    SmtTerm.str_replace_re
    (by intro a b c; rfl)
    (by
      intro a b c a' b' c' ha hb hc
      rw [typeof_str_replace_re_eq, typeof_str_replace_re_eq, ha, hb, hc])
    x₁ x₂ x₃ rhs

private theorem congTypeSpine_str_replace_re_all_eq_has_bool_type
    (x₁ x₂ x₃ rhs : Term) :
    RuleProofs.eo_has_smt_translation
      (Term.Apply
    ...
    RuleProofs.eo_has_bool_type
      (mkEq
        (Term.Apply
          (Term.Apply (Term.Apply (Term.UOp UserOp.str_replace_re_all) x₁)
            x₂) x₃) rhs) :=
  congTypeSpine_typecongr_ternop_eq_has_bool_type UserOp.str_replace_re_all
    SmtTerm.str_replace_re_all
    (by intro a b c; rfl)
    (by
      intro a b c a' b' c' ha hb hc
      rw [typeof_str_replace_re_all_eq, typeof_str_replace_re_all_eq, ha, hb,
        hc])
    x₁ x₂ x₃ rhs

private theorem congTypeSpine_str_indexof_re_eq_has_bool_type
    (x₁ x₂ x₃ rhs : Term) :
    RuleProofs.eo_has_smt_translation
      (Term.Apply
    ...
    RuleProofs.eo_has_bool_type
      (mkEq
        (Term.Apply
          (Term.Apply (Term.Apply (Term.UOp UserOp.str_indexof_re) x₁) x₂)
          x₃) rhs) :=
  congTypeSpine_typecongr_ternop_eq_has_bool_type UserOp.str_indexof_re
    SmtTerm.str_indexof_re
    (by intro a b c; rfl)
    (by
      intro a b c a' b' c' ha hb hc
      rw [typeof_str_indexof_re_eq, typeof_str_indexof_re_eq, ha, hb, hc])
    x₁ x₂ x₃ rhs

private theorem congTypeSpine_re_mult_eq_has_bool_type
    (x rhs : Term) :
    RuleProofs.eo_has_smt_translation (Term.Apply (Term.UOp UserOp.re_mult) x) ->
    CongTypeSpine (Term.Apply (Term.UOp UserOp.re_mult) x) rhs ->
    RuleProofs.eo_has_bool_type
      (mkEq (Term.Apply (Term.UOp UserOp.re_mult) x) rhs) :=
  congTypeSpine_typecongr_unop_eq_has_bool_type UserOp.re_mult
    SmtTerm.re_mult
    (by intro a; rfl)
    (by
      intro a b h
...
private theorem congTypeSpine_re_plus_eq_has_bool_type
    (x rhs : Term) :
    RuleProofs.eo_has_smt_translation (Term.Apply (Term.UOp UserOp.re_plus) x) ->
    CongTypeSpine (Term.Apply (Term.UOp UserOp.re_plus) x) rhs ->
    RuleProofs.eo_has_bool_type
      (mkEq (Term.Apply (Term.UOp UserOp.re_plus) x) rhs) :=
  congTypeSpine_typecongr_unop_eq_has_bool_type UserOp.re_plus
    SmtTerm.re_plus
    (by intro a; rfl)
    (by
      intro a b h
...
private theorem congTrueSpine_strings_stoi_result_eq_true
    ...
    RuleProofs.eo_has_bool_type
      (mkEq (Term.Apply
        (Term._at_strings_stoi_result x₁) x₂)
        rhs) ->
    CongTrueSpine M
      (Term.Apply
        (Term._at_strings_stoi_result x₁) x₂)
    ...
    eo_interprets M
      (mkEq (Term.Apply
        (Term._at_strings_stoi_result x₁) x₂)
        rhs) true :=
  congTrueSpine_non_reg_non_app_unop_eq_true M hM
    (Term._at_strings_stoi_result x₁)
    (fun b => stringsStoiResultTerm (__eo_to_smt x₁) b)
    (stringsStoiResultEval (__smtx_model_eval M (__eo_to_smt x₁)))
    (by intro f x h; cases h)
    (by intro a; rfl)
    (by
      intro b hNN
      rcases strings_stoi_result_args_non_reg_of_non_none
          (__eo_to_smt x₁) b hNN with
        ⟨_A, B, _hA, hB, _hANN, hBNN, _hAReg, hBReg⟩
      exact ⟨B, hB, hBNN, hBReg⟩)
    (by
      intro b
...
private theorem congTrueSpine_strings_itos_result_eq_true
    ...
    RuleProofs.eo_has_bool_type
      (mkEq (Term.Apply
        (Term._at_strings_itos_result x₁) x₂)
        rhs) ->
    CongTrueSpine M
      (Term.Apply
        (Term._at_strings_itos_result x₁) x₂)
    ...
    eo_interprets M
      (mkEq (Term.Apply
        ...
        rhs) true :=
  congTrueSpine_non_reg_non_app_unop_eq_true M hM
    (Term._at_strings_itos_result x₁)
    (fun b => stringsItosResultTerm (__eo_to_smt x₁) b)
    (stringsItosResultEval M (__smtx_model_eval M (__eo_to_smt x₁)))
    (by intro f x h; cases h)
    (by intro a; rfl)
    (by
      intro b hNN
  ...
  congTypeSpine_typecongr_non_app_unop_eq_has_bool_type
    ...
    (by
      ...
      rw [stringsItosResultTerm, stringsItosResultTerm, typeof_mod_eq,
        typeof_mod_eq, typeof_multmult_eq, typeof_multmult_eq, hb])
    x₂ rhs
...
private theorem congTrueSpine_strings_num_occur_eq_true
  ...
  congTrueSpine_non_reg_binop_eq_true M hM UserOp._at_strings_num_occur
    ...
    (by
      intro a b
      rw [stringsNumOccurTerm, stringsNumOccurEval, __smtx_model_eval.eq_23,
        __smtx_model_eval.eq_12, __smtx_model_eval.eq_78,
        __smtx_model_eval.eq_78, __smtx_model_eval.eq_78,
        __smtx_model_eval.eq_98, __smtx_model_eval.eq_77])
    x₁ x₂ rhs
...
private theorem congTrueSpine_strings_stoi_non_digit_eq_true
    (M : SmtModel) (_hM : model_total_typed M) (x rhs : Term) :
    RuleProofs.eo_has_bool_type
      (mkEq (Term._at_strings_stoi_non_digit x) rhs) ->
    CongTrueSpine M
      (Term._at_strings_stoi_non_digit x) rhs ->
    eo_interprets M
      (mkEq (Term._at_strings_stoi_non_digit x) rhs) true := by
  intro hEqBool hSpine
  have hRhs : rhs = Term._at_strings_stoi_non_digit x :=
    congTrueSpine_head_eq_of_not_app M
      (by intro f y h; cases h) hSpine
  subst hRhs
  exact congTrueSpine_refl_eq_true M (Term._at_strings_stoi_non_digit x) hEqBool
...
private theorem congTypeSpine_strings_stoi_non_digit_eq_has_bool_type
    ...
    CongTypeSpine
      (Term._at_strings_stoi_non_digit x) rhs ->
    RuleProofs.eo_has_bool_type
      (mkEq (Term._at_strings_stoi_non_digit x) rhs) := by
  intro hTrans hSpine
  have hRhs : rhs = Term._at_strings_stoi_non_digit x :=
    congTypeSpine_head_eq_of_not_app
      (by intro f y h; cases h) hSpine
  subst hRhs
  exact congTypeSpine_refl_eq_has_bool_type (Term._at_strings_stoi_non_digit x)
    hTrans
...
private theorem false_of_eo_to_smt_none_has_translation
    {t : Term}
    (hToSmt : __eo_to_smt t = SmtTerm.None) :
    RuleProofs.eo_has_smt_translation t -> False := by
  intro hTrans
  unfold RuleProofs.eo_has_smt_translation at hTrans
  rw [hToSmt] at hTrans
  exact hTrans (by simp [__smtx_typeof])

private theorem false_of_eq_bool_left_to_smt_none
    {t rhs : Term}
    (hToSmt : __eo_to_smt t = SmtTerm.None) :
    RuleProofs.eo_has_bool_type (mkEq t rhs) -> False := by
  intro hEqBool
  have hTypes :=
    RuleProofs.eo_eq_operands_same_smt_type_of_has_bool_type
      t rhs hEqBool
  have hLeftNN := hTypes.2
  rw [hToSmt] at hLeftNN
  exact hLeftNN (by simp [__smtx_typeof])
...
private theorem congTypeSpine_eq_has_bool_type (t rhs : Term) :
  ...
  | Term.Apply (Term.is c) x =>
      exact congTypeSpine_typecongr_non_app_unop_eq_has_bool_type
        (Term.is c)
        ...
        (by
          intro a b h
          exact smt_tester_apply_type_congr (__eo_to_smt c) a b h)
        x rhs hTrans hSpine
  | Term.Apply (Term.Apply (Term.UOp UserOp.select) x₁) x₂ =>
      exact congTypeSpine_select_eq_has_bool_type x₁ x₂ rhs hTrans hSpine
  | Term.Apply (Term.Apply (Term.Apply (Term.UOp UserOp.store) x₁) x₂) x₃ =>
      exact congTypeSpine_store_eq_has_bool_type x₁ x₂ x₃ rhs hTrans hSpine
  ...
  | Term.Apply (Term.UOp UserOp.re_mult) x =>
      exact congTypeSpine_re_mult_eq_has_bool_type x rhs hTrans hSpine
  | Term.Apply (Term.UOp UserOp.re_plus) x =>
      exact congTypeSpine_re_plus_eq_has_bool_type x rhs hTrans hSpine
  | Term.Apply (Term.UOp UserOp.re_opt) x =>
  ...
  | Term.Apply (Term.UOp UserOp.seq_unit) x =>
      exact congTypeSpine_seq_unit_eq_has_bool_type x rhs hTrans hSpine
  ...
  | Term.Apply (Term.Apply (Term.Apply (Term.UOp UserOp.str_replace_re) x₁) x₂) x₃ =>
      exact congTypeSpine_str_replace_re_eq_has_bool_type x₁ x₂ x₃ rhs
        hTrans hSpine
  | Term.Apply (Term.Apply (Term.Apply (Term.UOp UserOp.str_replace_re_all) x₁) x₂) x₃ =>
      exact congTypeSpine_str_replace_re_all_eq_has_bool_type x₁ x₂ x₃ rhs
        hTrans hSpine
  | Term.Apply (Term.Apply (Term.Apply (Term.UOp UserOp.str_indexof_re) x₁) x₂) x₃ =>
      exact congTypeSpine_str_indexof_re_eq_has_bool_type x₁ x₂ x₃ rhs
        hTrans hSpine
  | Term.Apply (Term.UOp UserOp.set_is_empty) x =>
      exact congTypeSpine_set_is_empty_eq_has_bool_type
        x rhs hTrans hSpine
  | Term.Apply (Term.UOp UserOp.set_singleton) x =>
      exact congTypeSpine_set_singleton_eq_has_bool_type x rhs hTrans hSpine
  ...
  | Term.Apply (Term.Apply (Term.UOp UserOp.set_minus) x₁) x₂ =>
      exact congTypeSpine_set_binop_eq_has_bool_type UserOp.set_minus
        ...
        x₁ x₂ rhs hTrans hSpine
  | Term.Apply (Term.Apply (Term.UOp UserOp.set_member) x₁) x₂ =>
      exact congTypeSpine_set_member_eq_has_bool_type x₁ x₂ rhs hTrans
        hSpine
  ...
  | Term.Apply (Term.Apply (Term.UOp UserOp.set_insert) Term.__eo_List_nil) x =>
      exact False.elim
        (false_of_eo_to_smt_none_has_translation (t :=
          Term.Apply (Term.Apply (Term.UOp UserOp.set_insert)
            Term.__eo_List_nil) x) (by rfl) hTrans)
  | Term.Apply (Term.Apply (Term.UOp UserOp.concat) x₁) x₂ =>
      exact congTypeSpine_bv_concat_eq_has_bool_type x₁ x₂ rhs hTrans hSpine
  | Term.Apply (Term.extract hi lo) x =>
      exact congTypeSpine_typecongr_non_app_unop_eq_has_bool_type
        (Term.extract hi lo)
        (fun a => SmtTerm.extract (__eo_to_smt hi) (__eo_to_smt lo) a)
        (by intro f x h; cases h)
        (by intro a; rfl)
        (by
          intro a b h
        ...
        x rhs hTrans hSpine
  | Term.Apply (Term.repeat n) x =>
      exact congTypeSpine_typecongr_non_app_unop_eq_has_bool_type
        (Term.repeat n)
        (fun a => SmtTerm.repeat (__eo_to_smt n) a)
        (by intro f x h; cases h)
        (by intro a; rfl)
        (by
          intro a b h
        ...
        x rhs hTrans hSpine
  | Term.Apply (Term.zero_extend n) x =>
      exact congTypeSpine_typecongr_non_app_unop_eq_has_bool_type
        (Term.zero_extend n)
        (fun a => SmtTerm.zero_extend (__eo_to_smt n) a)
        (by intro f x h; cases h)
        (by intro a; rfl)
        (by
          intro a b h
        ...
        x rhs hTrans hSpine
  | Term.Apply (Term.sign_extend n) x =>
      exact congTypeSpine_typecongr_non_app_unop_eq_has_bool_type
        (Term.sign_extend n)
        (fun a => SmtTerm.sign_extend (__eo_to_smt n) a)
        (by intro f x h; cases h)
        (by intro a; rfl)
        (by
          intro a b h
        ...
        x rhs hTrans hSpine
  | Term.Apply (Term.rotate_left n) x =>
      exact congTypeSpine_typecongr_non_app_unop_eq_has_bool_type
        (Term.rotate_left n)
        (fun a => SmtTerm.rotate_left (__eo_to_smt n) a)
        (by intro f x h; cases h)
        (by intro a; rfl)
        (by
          intro a b h
        ...
        x rhs hTrans hSpine
  | Term.Apply (Term.rotate_right n) x =>
      exact congTypeSpine_typecongr_non_app_unop_eq_has_bool_type
        (Term.rotate_right n)
        (fun a => SmtTerm.rotate_right (__eo_to_smt n) a)
        (by intro f x h; cases h)
        (by intro a; rfl)
        (by
          intro a b h
  ...
  | Term.Apply (Term.int_to_bv w) x =>
      exact congTypeSpine_typecongr_non_app_unop_eq_has_bool_type
        ...
        x rhs hTrans hSpine
  | Term.Apply (Term._at_bit idx) x =>
      exact congTypeSpine_bv_at_bit_eq_has_bool_type
  ...
  | Term.Apply (Term.Apply (Term.UOp UserOp.exists) Term.__eo_List_nil) x =>
      exact False.elim
        (false_of_eo_to_smt_none_has_translation (t :=
          Term.Apply (Term.Apply (Term.UOp UserOp.exists)
            Term.__eo_List_nil) x) (by rfl) hTrans)
  | _ =>
      cases hSpine with
      | refl _ =>
...
private theorem congTrueSpine_eq_true
  ...
  | Term.Apply (Term.is c) x =>
      exact congTrueSpine_non_reg_non_app_unop_eq_true M hM
        (Term.is c)
        (fun a => SmtTerm.Apply (__eo_to_smt_tester (__eo_to_smt c)) a)
        (smtTesterApplyEval M (__eo_to_smt c))
        (by intro f x h; cases h)
        (by intro a; rfl)
        (by
          intro a hNN
          exact smt_tester_apply_arg_non_reg_of_non_none
            (__eo_to_smt c) a hNN)
        (by
          intro a
          exact smt_tester_apply_eval_eq M (__eo_to_smt c) a)
  ...
  | Term.Apply (Term.Apply (Term.UOp UserOp.set_insert) Term.__eo_List_nil) x =>
      exact False.elim
        (false_of_eq_bool_left_to_smt_none (t :=
          Term.Apply (Term.Apply (Term.UOp UserOp.set_insert)
            Term.__eo_List_nil) x) (by rfl) hEqBool)
  | Term.Apply (Term.Apply (Term.UOp UserOp.concat) x₁) x₂ =>
      ...
      exact congTrueSpine_non_reg_non_app_unop_eq_true M hM
        ...
        (fun v =>
          __smtx_model_eval_extract
            ...
            (__smtx_model_eval M (__eo_to_smt lo)) v)
        (by intro f x h; cases h)
        (by intro a; rfl)
        (by
          intro a hNN
          have hTerm :
              term_has_non_none_type
                (SmtTerm.extract (__eo_to_smt hi) (__eo_to_smt lo) a) := by
            unfold term_has_non_none_type
            exact hNN
          rcases extract_args_of_non_none hTerm with
            ⟨_i, _j, w, _hHi, _hLo, hArg, _hj0, _hji, _hiw⟩
      ...
      exact congTrueSpine_non_reg_non_app_unop_eq_true M hM
        ...
        (fun v =>
          __smtx_model_eval_repeat
            (__smtx_model_eval M (__eo_to_smt n)) v)
        (by intro f x h; cases h)
        (by intro a; rfl)
        (by
          intro a hNN
          have hTerm :
              term_has_non_none_type
                (SmtTerm.repeat (__eo_to_smt n) a) := by
            unfold term_has_non_none_type
            exact hNN
  ...
  | Term.Apply (Term.zero_extend n) x =>
      exact congTrueSpine_non_reg_non_app_unop_eq_true M hM
        ...
        (by
          ...
          have hTerm :
              term_has_non_none_type
                (SmtTerm.zero_extend (__eo_to_smt n) a) := by
            unfold term_has_non_none_type
            exact hNN
  ...
  | Term.Apply (Term.sign_extend n) x =>
      exact congTrueSpine_non_reg_non_app_unop_eq_true M hM
        ...
        (by
          ...
          have hTerm :
              term_has_non_none_type
                (SmtTerm.sign_extend (__eo_to_smt n) a) := by
            unfold term_has_non_none_type
            exact hNN
  ...
  | Term.Apply (Term.rotate_left n) x =>
      exact congTrueSpine_non_reg_non_app_unop_eq_true M hM
        ...
        (by
          ...
          have hTerm :
              term_has_non_none_type
                (SmtTerm.rotate_left (__eo_to_smt n) a) := by
            unfold term_has_non_none_type
            exact hNN
  ...
  | Term.Apply (Term.rotate_right n) x =>
      exact congTrueSpine_non_reg_non_app_unop_eq_true M hM
        ...
        (by
          ...
          have hTerm :
              term_has_non_none_type
                (SmtTerm.rotate_right (__eo_to_smt n) a) := by
            unfold term_has_non_none_type
            exact hNN
      ...
      exact congTrueSpine_non_reg_non_app_unop_eq_true M hM
        ...
        (fun v =>
          __smtx_model_eval_int_to_bv
            (__smtx_model_eval M (__eo_to_smt w)) v)
        (by intro f x h; cases h)
        (by intro a; rfl)
        (by
          intro a hNN
          have hTerm :
              term_has_non_none_type
                (SmtTerm.int_to_bv (__eo_to_smt w) a) := by
            unfold term_has_non_none_type
            exact hNN
          rcases int_to_bv_args_of_non_none hTerm with
            ⟨_i, _hw, hArg, _hi⟩
          exact ⟨SmtType.Int, hArg, by simp, by simp⟩)
        (by intro a; rw [__smtx_model_eval.eq_129])
        x rhs hEqBool hSpine
  | Term.Apply (Term._at_bit idx) x =>
      exact congTrueSpine_bv_at_bit_eq_true M hM

...
private theorem listIdentity_is_list (w : Nat) :
    __eo_is_list op (listIdentity w) = Term.Boolean true := by
...
private theorem typeof_bvand_args_of_result_bitvec
  ...
  have hNe :
    ...
    simpa [band] using (show __eo_typeof (band x y) ≠ Term.Stuck by
      rw [h]
      intro hBad
      cases hBad)
...
private theorem list_smt_type_or_nil_of_concat_type
  ...
  | case3 f x xs z hz ih =>
      ...
      have hNonNone :
        ...
        simpa [RuleProofs.eo_has_smt_translation, band, op] using hATrans
...
private theorem lhs_component_types
  ...
  have hInsertedOpNe :
    ...
    simpa [inserted, band, op] using hInsertedNe
...
private theorem lhs_smt_type
  ...
  have hSecondZsList :
      __eo_is_list op (band second zs) = Term.Boolean true :=

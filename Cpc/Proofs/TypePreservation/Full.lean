...
private theorem tp_smt_term_result_seq_components_wf_of_non_none
  ...
  let rec go (x : SmtTerm) (hxNN : term_has_non_none_type x) :
    ...
    case String s =>
      ...
      simpa [__smtx_typeof, tp_result_seq_components_wf, native_ite, hs] using
        tp_seq_char_wf
    case Var s T =>
...
private axiom type_has_no_none_components_of_wf :
    {T : SmtType} ->
    __smtx_type_wf T = true ->
    type_has_no_none_components T

private axiom type_has_no_none_components_of_wf_rec :
    {T : SmtType} ->
    {refs : RefList} ->
...
private theorem dt_cons_wf_head_type_substitute_has_no_none_components
  ...
  cases T <;> simp [__smtx_dt_cons_wf_rec, __smtx_type_wf_rec,
    __smtx_type_substitute, native_reflist_contains, native_reflist_insert,
    native_reflist_nil, native_ite, native_streq,
    type_has_no_none_components] at h ⊢
  case TypeRef r =>
    have hr : r = s := h.1
    subst r
  ...
  let rec go (t : SmtTerm) (ht : term_has_non_none_type t) :
    ...
    | SmtTerm.String s =>
        have hs : native_string_in_cpc_range s = true := by
          unfold term_has_non_none_type at ht
          cases h : native_string_in_cpc_range s <;>
            simp [__smtx_typeof, native_ite, h] at ht ⊢
        simp [__smtx_typeof, type_has_no_none_components, native_ite, hs]
    | SmtTerm.Binary _ _ =>
